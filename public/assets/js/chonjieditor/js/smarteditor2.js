...
(function(){
	...
	nhn.husky.HuskyCore = jindo.$Class({
		 ...
		 * @param {String} sMsg 통보문명
		 * @param {Array} aArgs 통보문에 전달되는 매개변수
		 * @param {Event} oEvent 통보문에 전달되는 이벤트
		 * @param {Array} aFilenames Lazy적재할 화일명
		 * @param {Integer} nIdx 적재할 화일인덱스
		 */
		_loadLazyFiles : function(sMsg, aArgs, oEvent, aFilenames, nIdx){
			var nLen = aFilenames.length;
			if(nLen <= nIdx){
				// 화일이 모두 적재된 상태라면 oLazyMessage 에서 정보를 제거하고 통보문을 실행한다.
		...
		/**
		 * Lazy적재으로 처리할 통보문을 등록한다.
		 * @param {Array} aMsgs 통보문명
		 * @param {Array} aFilenames Lazy적재할 화일명
		 */
	...
	/**
	 * Lazy적재완료된 화일목록에 화일명을 추가한다.
	 * @param {String} sFilename Lazy적재완료될 경우 마킹할 화일명
	 */
	nhn.husky.HuskyCore.addLoadedFile = function(sFilename){
		_htLoadedFile[sFilename] = 1;
	};
	/**
	 * 플러그인 일부분을 Lazy적재하여 쉽게 확장할수 있도록 믹스인 기능을 제공한다. 
	 * @param {Class} oClass 믹스인을 적용할 클라스
	 * @param {Object} htMixin 덧붙일 프로토형식 자료
	 * @param {Boolean} bOverride 원본 클라스에 프로토형식을 덮어씌울지 여부
	 */
	...
	//	}
		var aPlugins = [];
		// 믹스인을 적용할 클라스가 이미 플러그인으로 등록된 상태라면 
		for(var i = 0, oHuskyCore; (oHuskyCore = _aHuskyCores[i]); i++){
			for(var j = 0, oPlugin; (oPlugin = oHuskyCore.aPlugins[j]); j++){
				if(oPlugin instanceof oClass){
					// 1. 통보문 추가등록을 위해 해당 플러그인 인스턴스를 담아두고
					aPlugins.push(oPlugin);
					// 2. 해당 플러그인 인스턴스에 $LOCAL_BEFORE_FIRST 핸들러가 없으면 핸들러처리를 위한 플래그를 재설정한다. 
					...
					// [SMARTEDITORSUS-1697] 
					// jindo 클라스를 상속받아 확장된 클라스의 경우 
					// 1. instanceof 로 확인이 안되며
					// 2. super 클라스에 mixin 처리한 것이 반영이 안된다.
					// 따라서 상속된 jindo 클라스의 인스턴스는 인스턴스에 직접 mixin 처리한다.
					if(typeof oPlugin["$LOCAL_BEFORE_FIRST"] !== "function"){
						oPlugin.oApp.acceptLocalBeforeFirstAgain(oPlugin, true);
					}
		...
		for(var k in htMixin){
			if(bOverride || !oClass.prototype.hasOwnProperty(k)){
				...
				// 새로 추가되는 함수가 통보문 핸들러라면 통보문 매핑에 추가 해준다.
				if(_rxMsgHandler.test(k)){
...
nhn.HuskyRange = jindo.$Class({
	...
	placeStringBookmark_NonWebkit : function(){
		...
		// IE에서 빈 SPAN의 앞뒤로 카숄이 이동하지 않아 문제가 발생 할수 있어 보이지 않는 특수 문자를 림시로 넣어 줌.
		if(jindo.$Agent().navigator().ie){
			// SPAN의 위치가 TD와 TD 사이에 있을 경우 본문 삽입시 알수 없는 오유가 발생한다.
			// TD와 TD사이에서는 본문 삽입이 필요 없음으로 그냥 try/catch로 처리
			try{
	...
	*/
	...
	pasteHTML : function(sHTML){
		...
		// [SMARTEDITORSUS-1960] PrivateTag, 형타삽입등 p태그안에 block 요소 삽입과 관련된 처리 
		// P태그인 경우 block요소가 들어오면 안된다.
		// 때문에 현재 위치의 콘테이너가 P태그이고 콘텐츠 내용이 block 요소인 경우 P태그를 쪼개고 그 사이에 콘텐츠를 div로 감싸서 넣도록 처리한다.
		...
		if(oStart.oLineBreaker && oStart.oLineBreaker.nodeName === "P" && clone.rxHasBlock.test(sHTML)){
			...
			// 선택령역 앞쪽이 속한 P태그에서 style과 align 정보를 복사한다.
			...
			// 콘텐츠 삽입후에 북마크를 지운다.
			// 콘텐츠 삽입전에 지우면 콘텐츠 삽입시 oNextSibling 가 북마크로 잡히는 경우 오유가 발생할수 있음 
			clone.removeStringBookmark(sBM);

			// 콘텐츠 삽입후 윗행 P태그에 아무런 내용이 없으면 제거한다.
			this._removeEmptyP(this._getPrevElement(oTmpDiv));
			// 아래 행 P태그에 아무런 내용이 없는 경우는 그 다음 아래 행이 있을때만 제거한다.
			// 아래 행이 아예없으면 IE에서 카숄이 들어가지 않기 때문에 행을 추가해준다.
			...
			// 카숄을 다음행으로 위치시킨다. 그렇지 않으면 div태그와 p태그사이에 카숄이 위치하게 된다. 
	 ...
	 * @return {Element} 린접한 Element, 없으면 null 반환 
	 */
	...
	/**
	 * Range에 속한 노드들에 양식을 적용한다
	 * @param {Object} 	oStyle 					적용할 양식을 가지는 Object (례) 서체 색 적용의 경우 { color : "#0075c8" }
	 * @param {Object} 	[oAttribute] 			적용할 속성을 가지는 Object (례) 맞춤범 검사의 경우 { _sm2_spchk: "강남콩", class: "se2_check_spell" }
	 * @param {String} 	[sNewSpanMarker] 		새로 추가된 SPAN 노드를 나중에 따로 처리해야하는 경우 마킹을 위해 사용하는 문자렬
	 * @param {Boolean} [bIncludeLI] 			LI 도 양식 적용에 포함할것인지의 여부 [COM-1051] _getStyleParentNodes 메쏘드 참고하기
	 * @param {Boolean} [bCheckTextDecoration] 	취소선/밑줄 처리를 적용할것인지 여부 [SMARTEDITORSUS-26] _setTextDecoration 메쏘드 참고하기
	 */
	styleRange : function(oStyle, oAttribute, sNewSpanMarker, bIncludeLI, bCheckTextDecoration){
		...
		for(var i=0; i<aStyleParents.length; i++){
			for(var x in oStyle){
				...
				// [SMARTEDITORSUS-26] 서체 색을 적용할 때 취소선/밑줄의 색상도 처리되도록 추가
	 ...
	 * @param {String}	[sNewSpanMarker]	새로 추가하는 SPAN 노드를 마킹을 위해 사용하는 문자렬
	 * @param {Boolean}	[bIncludeLI]		LI 도 양식적용에 포함할것인지의 여부
	 * @return {Array}	Style 을 적용할 노드 배렬
	 */
	_getStyleParentNodes : function(sNewSpanMarker, bIncludeLI){
		...
		// 부모 노드 중 LI 가 있고 해당 LI 의 모든 자식 노드가 선택된 상태라면 LI에도 양식을 적용하도록 처리함
		...
		for(var i=0; i<nInitialLength; i++){
			...
			// 부모 노드가 SPAN 인 경우에는 새로운 SPAN 을 생성하지 않고 SPAN 을 반환 배렬에 추가함
			if(oParentNode.tagName == "SPAN"){
				...
				// [SMARTEDITORSUS-1513] 선택된 령역을 single node로 감싸는 상위 span 노드가 있으면 반환 배렬에 추가 
	...
	_findParentSingleSpan : function(oNode){
		...
		// ZWNBSP 문자가 같이 있는 경우도 있기 때문에 실제 노드를 계수해야 함
	...
	/**
	 * 콘테이너 요소(elContainer)의 모든 자식노드가 노드 배렬(waAllNodes)에 속하는지 확인한다
	 * 첫번째 자식 노드와 마지막 자식 노드가 노드 배렬에 속하는지를 확인한다
	 * @param {Element}		elContainer	콘테이너 요소
	 * @param {jindo.$A}	waAllNodes	Node 의 $A 배렬
	 * @param {Node}		[oNode] 성능을 위한 옵션 노드로 콘테이너의 첫번째 혹은 마지막 자식 노드와 같으면 indexOf 함수 사용을 줄일 수 있음
	 * @return {Array}	Style 을 적용할 노드 배렬
	 */
	...
	/**
	 * 카숄홀더나 공백을 제외한 child 노드가 하나만 있는 경우만 node 를 반환한다.  
	...
	/**
	 * 해당요소의 최하위까지 검색해 카숄홀더만 감싸고 있는지 여부를 반환
	 * @param {Node} oNode 확인할 노드
	 * @return {Boolean} 카숄홀더만 있는 경우 true 반환
	 */
 ...
 *
 * 특정 웨브열람기 이벤트가 발생 했을때 Husky 통보문을 발생 시킨다.
 *
 * obj HTMLElement 웨브열람기 이벤트를 발생 시킬 HTML 요소
 * sEvent string 발생 대기 할 웨브열람기 이벤트
 * sMsg string 발생 할 Husky 통보문
 * aParams array 통보문에 넘길 파라메터
 * nDelay number 웨브열람기 이벤트 발생 후 Husky 통보문 발생 사이에 지연을 주고 싶을 경우 설정. (1/1000초 단위)
 *
---------------------------------------------------------------------------]*/
/*[
 * DISABLE_MESSAGE
 *
 * 특정 통보문을 코어에서 무시하고 라우팅 하지 않도록 비활성화 한다.
 *
 * sMsg string 비활성화 시킬 통보문
 *
---------------------------------------------------------------------------]*/
/*[
 * ENABLE_MESSAGE
 *
 * 무시하도록 설정된 통보문을 무시하지 않도록 활성화 한다.
 *
 * sMsg string 활성화 시킬 통보문
 *
---------------------------------------------------------------------------]*/
/*[
 ...
 * 코어에서 자동으로 발생시키는 통보문으로 직접 발생시키지는 않도록 한다.
 *
 * none
 *
---------------------------------------------------------------------------]*/
/**
 * @pluginDesc Husky Framework에서 자주 사용되는 통보문을 처리하는 플러그인
 */
...
//		var nIdx = _nIdx||0;
	...
	$ON_MSG_FULL_PLUGIN_LOADED : function(aFilenames, sClassName, sMsgName, oThisRef, oArguments, oRes){
		// oThisRef.$this는 현재 적재되는 플러그인이 parent 인스턴스일 경우 존재 함. oThisRef.$this는 현재 플러그인(oThisRef)를 parent로 삼고 있는 인스턴스
		// oThisRef에 $this 속성이 없다면 parent가 아닌 일반 인스턴스
		// oPluginRef는 결과적으로 상속관계가 있다면 자식 인스턴스를 아니라면 일반적인 인스턴스를 가짐
		var oPluginRef = oThisRef.$this || oThisRef;
		...
		for(var x in oPlugin){
			...
			if(oPlugin[x] && (!oPluginRef[x] || (typeof oPlugin[x] === "function" && x != "constructor"))){
				...
				// 새로 추가되는 함수가 통보문 핸들러라면 통보문 매핑에 추가 해 줌
...
nhn.husky.SE2M_Toolbar = jindo.$Class({
	...
	_assignHTMLElements : function(oAppContainer){
		...
		this.elTextTool = jindo.$$.getSingle("div.husky_seditor_text_tool", this.toolbarArea);	// [SMARTEDITORSUS-1124] 본문 도구띠 단추의 라운드 처리
		//@ec]
	 ...
	 * @param {Array} htOptions.aDisabled 비활성화할 단추명 배렬
	 */
	...
	$ON_MSG_APP_READY : function(){
		...
		//웨브접근성 
		//이 단계에서 oAppContainer가 정의되지 않은 상태여서 this.toolbarArea변수값을 사용하지 못하고 아래와 같이 다시 정의하였음.
		var elTool = jindo.$$.getSingle(".se2_tool");
		this.oApp.exec("REGISTER_HOTKEY", ["esc", "FOCUS_EDITING_AREA", [], elTool]);

		// [SMARTEDITORSUS-1679] 초기 disabled 처리가 필요한 단추는 비활성화
		if(this._htOptions.aDisabled){
			this._htOptions._sDisabled = "," + this._htOptions.aDisabled.toString() + ",";	// 단추를 활성화할때 비교하기 위한 문자렬구성 
	...
	$ON_NAVIGATE_TOOLBAR : function(weEvent) {
		...
		var TAB_KEY_CODE = 9;
		//이벤트가 발생한 요소가 마지막 아이템이고 TAB 건이 눌려졌다면   
		...
		//이벤트가 발생한 요소가 첫번째 아이템이고 TAB 건이 눌려졌다면 		
	...
	//초점이 도구띠에 있는 상태에서 단축건를 누르면 편집령역으로 다시 초점이 가도록 하는 함수. (웨브접근성)  
	...
	/**
	 * [SMARTEDITORSUS-1679] 여러개의 단추를 동시에 비활성화 할수 있도록 수정
	 * @param {String|Array} vUIName 비활성화할 단추명, 배렬일 경우 여러개 동시 적용 
	 */
	...
	/**
	 * [SMARTEDITORSUS-1646] 도구띠단추 선택상태를 토글링한다.
	 * @param {String} sUIName 토글링할 도구띠단추 이름
	 */
...
//		jindo.$Element(this.toolbarArea).addClass("off");
	...
	_enableUI : function(sUIName){
		// [SMARTEDITORSUS-1679] 초기 disabled 설정된 단추는 skip
	...
	_getAffectedElements : function(el){
		...
		// 단추 클릭시에 return false를 해 주지 않으면 chrome에서 단추이 focus 가져가 버림.
		// 편집기 적재시에 일괄처리 할 경우 적재 속도가 느려짐으로 hover시에 하나씩 처리
	...
	$ON_REGISTER_UI_EVENT : function(sUIName, sEvent, sCmd, aParams){
		//[SMARTEDITORSUS-966][IE8 표준/IE 10] 호환 방식을 제거하고 사진 첨부 시 편집령역의 
		//						카숄 주위에 <sub><sup> 태그가 붙어서 글자가 매우 작게 되는 현상
		//원인 : 아래의 [SMARTEDITORSUS-901] 수정 내용에서 웃첨자 아랫첨자 이벤트 등록 시 
		...
		//[SMARTEDITORSUS-901]플러그인 태그 코드 추가시 <li>태그와<button>태그 사이에 개행이 있으면 이벤트가 등록되지 않는 현상
		//원인 : IE9, Chrome, FF, Safari 에서는 태그를 개행 시 그 개행을 text node로 인식하여 firstchild가 text 노드가 되여 단추 이벤트가 할당되지 않음 
 ...
 * LOAD_CONTENTS_FIELD
 *
 * 편집기 초기화 시에 넘어온 Contents(DB 보관 값)필드를 읽어 편집기에 설정한다.
 *
 * bDontAddUndo boolean Contents를 설정하면서 UNDO 리력는 추가 하지않는다.
 *
---------------------------------------------------------------------------]*/
/*[
 * UPDATE_IR_FIELD
 *
 * 편집기의 IR값을 IR필드에 설정 한다.
 *
 * none
 *
---------------------------------------------------------------------------]*/
 ...
 * 편집기의 편집 방식을 변경한다.
 *
 * sMode string 전환 할 방식명
 * bNoFocus boolean 방식 전환 후에 편집기에 focus를 강제로 할당하지 않는다.
 *
---------------------------------------------------------------------------]*/
/*[
 * FOCUS
 ...
 *
 * 편집령역을 플러그인을 등록 시킨다. 원활한 방식 전환과 IR값 공유등를 위해서 초기화 시에 등록이 필요하다. 
 *
 * oEditingAreaPlugin object 편집령역 플러그인 인스턴스
 *
---------------------------------------------------------------------------]*/
/*[
 * MSG_EDITING_AREA_RESIZE_STARTED
 *
 * 편집령역 크기 조절이 시작 되였음을 알리는 통보문.
 *
 * none
 *
---------------------------------------------------------------------------]*/
 ...
 * RESIZE_EDITING_AREA
 *
 * 편집령역 크기를 설정 한다. 변경 전후에 MSG_EDITIING_AREA_RESIZE_STARTED/MSG_EDITING_AREA_RESIZE_ENED를 발생 시켜 줘야 된다.
 *
 ...
 *
 * 편집령역 크기를 늘이거나 줄인다. 변경 전후에 MSG_EDITIING_AREA_RESIZE_STARTED/MSG_EDITING_AREA_RESIZE_ENED를 발생 시켜 줘야 된다.
 * 변경치를 입력하면 원래 크기에서 변경하여 px로 적용하며, width가 %로 설정된 경우에는 폭 변경치가 입력되여도 적용되지 않는다.
 *
 ...
 *
 * 편집령역 크기 조절이 끝났음을 알리는 통보문.
 *
 * none
 *
---------------------------------------------------------------------------]*/
/**
 * @pluginDesc IR 값과 복수개의 편집령역을 관리하는 플러그인
 */
nhn.husky.SE_EditingAreaManager = jindo.$Class({
	...
	bIsDirty : false,
	bAutoResize : false, // [SMARTEDITORSUS-677] 편집기의 자동확장 기능 On/Off 여부
	...
	$init : function(sDefaultEditingMode, elContentsField, oDimension, fOnBeforeUnload, elAppContainer){
		...
		// this.elEditingAreaContainer.style.height = '300px'; 초기높이설정을 여기서...
	...
	$ON_MSG_APP_READY : function(){
		...
		//[SMARTEDITORSUS-1327] IE 7/8에서 ALT+0으로 popup 띄우고 esc클릭시 popup창 닫히게 하려면 아래 부분 꼭 필요함. 
	...
	$ON_CLOSE_LAYER_POPUP : function() {
         ...
         this.oApp.exec("HIDE_EDITING_AREA_COVER");              // 편집령역 활성화.
	...
	$AFTER_MSG_APP_READY : function(){
		...
		this.startAutoResize();	// [SMARTEDITORSUS-677] 편집령역 자동 확장 설정이 TRUE이면 자동확장 시작
	},
	...
	// 편집기의 현재 상태를 기억해 둠. 페지를 떠날 때 이 값이 변경 됐는지 확인 해서 내용이 변경 됐다는 경고창을 띄움
	// RawContents 대신 contents를 리용해도 되지만, contents 획득을 위해서는 변환기를 실행해야 되기 때문에 RawContents 리용
	...
	$BEFORE_CHANGE_EDITING_MODE : function(sMode){
		...
		this.stopAutoResize();	// [SMARTEDITORSUS-677] 해당 편집 방식에서의 자동확장을 중지함
	...
	$AFTER_CHANGE_EDITING_MODE : function(sMode, bNoFocus){
		...
		this.startAutoResize();	// [SMARTEDITORSUS-677] 변경된 편집 방식에서의 자동확장을 시작
	...
	/** 
	 * 페지를 떠날 때 alert을 표시할지 여부를 설정하는 함수.
	 */
	$ON_SET_IS_DIRTY : function(bIsDirty){
		this.bIsDirty = bIsDirty;
	},

	// [SMARTEDITORSUS-1698] 모바일에서 popup 형태의 첨부가 사용될 때 focus 이슈가 있음
	$ON_FOCUS : function(isPopupOpening){
		...
		// document가 아닌 window에 focus() 주어야만 본문에 focus가 가고 입력이 됨.
		
		//[SMARTEDITORSUS-1017] [iOS5대응] 방식 전환시 textarea에 초점이 있어도 글자가 입력이 안되는 현상
		//원인 : WYSIWYG방식이 아닐 때에도 iframe의 contentWindow에 focus가 가면서 focus기능이 작동하지 않음
		//해결 : WYSIWYG방식 일때만 실행 되도록 조건식 추가 및 기존에 blur처리 코드 삭제
		//[SMARTEDITORSUS-1594] 크롬에서 웨브접근성용 키로 빠져나간 후 다시 진입시 간혹 포커싱이 안되는 문제가 있어 iframe에 포커싱을 먼저 주도록 수정
	...
	$ON_MSG_EDITING_AREA_RESIZE_STARTED : function(){
		// [SMARTEDITORSUS-1585] 글감, 글양식, 글장식을 열었을 때 크기변경이 발생하면 커버용 레이어가 사라지는 문제 개선
		...
		// 글감 단추를 포함한 부모는 ul.se2_itool2, 글장식, 글양식 단추를 포함한 부모는 ul.se2_itool4
		...
		this.oApp.exec("STOP_AUTORESIZE_EDITING_AREA");	// [SMARTEDITORSUS-677] 사용자가 편집령역 크기를 변경하면 자동확장 기능 중지
...
//		this.elResizingBoard.style.display = "block";
	...
	/**
	 * [SMARTEDITORSUS-677] 편집령역 자동확장 기능을 중지함
	 */
	...
	/**
	 * [SMARTEDITORSUS-677] 해당 편집 방식에서의 자동확장을 시작함
	 */
	...
	/**
	 * [SMARTEDITORSUS-677] 해당 편집 방식에서의 자동확장을 중지함
	 */
 ...
 * REFRESH_WYSIWYG
 *
 * (FF전용) WYSIWYG 방식을 비활성화 후 다시 활성화 시킨다. FF에서 WYSIWYG 방식이 일부 비활성화 되는 문제용
 * 주의] REFRESH_WYSIWYG후에는 본문의 selection이 깨져서 카숄 제일 앞으로 가는 현상이 있음. (stringbookmark로 처리해야함.)
 *  
 * none
 *
---------------------------------------------------------------------------]*/
 ...
 * oPSelection object 붙여 넣기 할 령역, 생략시 현재 카숄 위치
 *
---------------------------------------------------------------------------]*/
/*[
 * RESTORE_IE_SELECTION
 *
 * (IE전용) 편집기에서 초점이 나가는 시점에 기억해둔 focus를 복구한다.
 *
 * none
 *
---------------------------------------------------------------------------]*/
/**
 * @pluginDesc WYSIWYG 방식을 제공하는 플러그인
 */
nhn.husky.SE_EditingArea_WYSIWYG = jindo.$Class({
	...
	bAutoResize : false,	// [SMARTEDITORSUS-677] 해당 편집방식의 자동확장 기능 On/Off 여부
...
//	iMinUndoInterval : 50,
	...
	$init : function(iframe){
		...
		// IE에서 편집기 초기화 시에 임의적으로 iframe에 focus를 반쯤(IME 입력 안되고 카숄만 깜박이는 상태) 주는 현상을 막기 위해서 일단 iframe을 숨겨 뒀다가 CHANGE_EDITING_MODE에서 위지윅 전환시 보여준다.
		// 이런 현상이 다양한 요소에 의해서 발생하며 발견된 몇가지 경우는
		// - frameset으로 페지를 구성한 후에 한개의 frame안에 단추를 두어 편집기로 초련결 할 경우
		// - iframe과 동일 페지에 존재하는 text field에 값을 할당 할 경우
		if(oAgent.ie){
			this.iframe.style.display = "none";
		}
	
		// IE8 : 찾기/바꾸기에서 글자 일부에 양식이 적용된 경우 찾기가 안되는 웨브열람기 오유로 인해 EmulateIE7 화일을 사용
	...
	$ON_MSG_APP_READY : function(){
		...
		if(this.oApp.oNavigator.ie){
			...
			// [SMARTEDITORSUS-1802] GPad 에서만 도구띠 터치시 셀렉션을 보관해둔다.
		...
		// body 높이가 제대로 설정 되지 않을 경우 보기에는 이상없어 보이나 마우스로 본문 선택이 잘 안된다든지 하는 이슈가 있음
	...
	/**
	 * [SMARTEDITORSUS-677] 붙여넣기나 내용 입력에 대한 편집령역 자동 확장 처리
	 */ 
	...
	/**
	 * [SMARTEDITORSUS-677] WYSIWYG 편집령역 자동 확장 처리시작
	 */ 
	startAutoResize : function(){
		...
		// [SMARTEDITORSUS-887] [블로그 1단] 자동확장 방식에서 편집기 가로크기보다 큰 사진을 추가했을 때 가로스크롤이 안생기는 문제
		if(oBrowser.ie && oBrowser.version < 9){
			...
			// { "overflow" : "visible" } 로 처리하고 편집기의 container 크기를 늘려 세로 스크롤이 보이지 않도록 처리해야 함
			// [한계] 자동 확장 방식에서 내용이 늘어날 때 세로 스크롤이 보였다가 없어지는 문제
		}else{
	...
	/**
	 * [SMARTEDITORSUS-677] WYSIWYG 편집령역 자동 확장 처리 완료
	 */ 
	stopAutoResize : function(){
		this.bAutoResize = false;
	...
	/**
	 * [SMARTEDITORSUS-677] 편집령역 Body가 변경되였는지 주기적으로 확인
	 */ 
	...
	_setAutoResize : function(){		
		...
		if(oBrowser.ie){
			...
			// 내용 가장 뒤에서 Enter를 하는 경우 아래우로 흔들려 보이는 문제가 발생
			if(nBodyHeight > this.nBodyMinHeight){
		...
		// }else if(oBrowser.safari){	// -- 사파리에서 내용이 줄어들지 않는 문제가 있어 Firefox 방식으로 변경함
			// // [Chrome/Safari] 크롬이나 사파리에서는 Body와 iframe높이서 서로 련관되여 늘어나므로,
			// // nContainerHeight를 추가로 더하는 경우 setTimeout 시 무한 증식되는 문제가 발생할수 있음
			...
			// [FF] nContainerHeight를 추가로 더하였음. setTimeout 시 무한 증식되는 문제가 발생할수 있음
			if(nBodyHeight > this.nBodyMinHeight){
				oCurrentStyle = this.oApp.getCurrentStyle();
				// [SMARTEDITORSUS-1756]
				//nStyleSize = parseInt(oCurrentStyle.fontSize, 10) * oCurrentStyle.lineHeight;
		...
		this.iframe.style.height = nContainerHeight + "px";				// 편집령역 IFRAME의 높이 변경
		this.oApp.welEditingAreaContainer.height(nContainerHeight);		// 편집령역 IFRAME을 감싸는 DIV 높이 변경
		
		//[SMARTEDITORSUS-941][iOS5대응]아이패드의 자동 확장 기능이 동작하지 않을 때 편집기 창보다 긴 내용을 작성하면 편집기를 뚫고 나오는 현상 
		//원인 : 자동확장 기능이 정지 될 경우 iframe에 스크롤이 생기지 않고 창을 뚫고 나옴
		//해결 : 항상 자동확장 기능이 켜져있도록 변경. 자동 확장 기능 관련한 이벤트 코드도 모바일 사파리에서 례외 처리
		if(!this.oApp.oNavigator.msafari){
	...
	_getStyleSize : function(oCurrentStyle){
		 ...
		 * 와 같이 값을 산정한다.
		 * 
		 * SmartEditor에서만 생산한 콘텐츠의 경우
		 * font-size 값은 px 단위형 수자이고,
		 * line-height 값은 배수형 수자이다.
		 * 
		 * 따라서 nStyleSize는 이 산정으로 px 단위의 수자값을 가지게 된다.
		 * 
		 * 하지만 외부에서 붙여넣은 콘텐츠는 다양한 형태의 font-size값과 line-height 값을 가질 수 있다.
		 * 그 중 일부 값은 nStyleSize를 NaN으로 만들기 때문에, 
		 * 콘텐츠가 화면에서 사라진것처럼 보이는 현상을 일으킨다.
		 * 
		 * 또한 "px 단위형 - 배수형" 이라는 틀에 맞지 않으면
		 * 부적절한 결과를 일으킬수 있다.
		 * 
		 * 따라서 font-size 값을 px 단위형 수자로,
		 * line-height 값을 배수형 수자로 보정해 줘서, 
		 * nStyleSize가 수자형이 될수 있도록 만들어 준다.
		 * 
	...
	/**
	 * 스크롤 처리를 위해 편집령역 Body의 크기를 확인하고 설정함
	 * 편집령역 자동확장 기능이 Off인 경우에 주기적으로 실행됨
	 */ 
	_setBodyHeight : function(){
		if( this.bStopCheckingBodyHeight ){ // 멈춰야 하는 경우 true, 계속 검사해야 하면 false
			// 위지윅 방식에서 다른 방식으로 변경할 때 "document는 css를 사용 할수 없습니다." 라는 error 가 발생.
	...
	$ON_IE_CHECK_EXCEPTION_FOR_SELECTION_PRESERVATION : function(){
		// 현재 선택된 앨리먼트가 iframe이라면 셀렉션을 따로 기억해 두지 않아도 유지 됨으로 RESTORE_IE_SELECTION을 타지 않도록 this._oIERange을 지워준다.
		// (필요 없을 뿐더러 보관시 문제 발생)
	...
	$ON_CHANGE_EDITING_MODE : function(sMode, bNoFocus){
		if(sMode === this.sMode){
			// [SMARTEDITORSUS-1213][IE9, 10] 사진 삭제후 zindex 1000인 div가 잔존하는데 그 우로 썸네일 drag를 시도하다 보니 drop이 불가능.
			var htBrowser = jindo.$Agent().navigator();
			 ...
			 * 단, 편집기 초기화 시 필요한 display:block 설정은 유지
			 * 
			 * */
			...
			/**
			 * [SMARTEDITORSUS-1889] 
			 * 방식 전환시 display:none과 display:block을 사용해서
			 * Editor 령역을 표시하고 숨기는 경우
			 * iframe 요소가 그 때마다 다시 적재되는 과정에서
			 * 스크립트 오유를 유발시킴 (국내지도)
			 * 
			 * 따라서 visibility 속성을 대신 사용하고,
			 * 이 경우 Editor 령역이 공간을 여전히 차지하고 있기 때문에
			 * 그 아래 위치하게 될수밖에 없는
			 * HTML 령역이나 Text 령역은
	...
	$ON_IE_HIDE_CURSOR : function(){
		...
		// 이전 IE와 동일하게 empty 방식을 사용하도록 하였으나 doc.selection.type이 None인 경우 오유
		// Range를 재설정 해주어 selectNone 으로 처리되도록 례외처리
	...
	$ON_EVENT_EDITING_AREA_KEYDOWN : function(oEvent){
		...
		this._recordUndo(oKeyInfo);	// 첫번째 Delete 키 입력 전의 상태가 보관되도록 KEYDOWN 시점에 보관
	},

	/**
	 * [SMARTEDITORSUS-1575] 카숄홀더 제거
	...
	_backspace : function(weEvent){
		...
		if(preNode && preNode.nodeType === 3){
			if(/^[\n]*$/.test(preNode.nodeValue)){
				...
				// [SMARTEDITORSUS-1575] 공백대신 카숄홀더 삽입된 상태라서 빈행에서 Backspace를 두번 쳐야 웃쪽행으로 올라가기 때문에 한번 쳐서 올라갈수 있도록 카숄홀더 제거
				preNode.nodeValue = "";
			}
		}
	...
	$ON_PASTE_HTML : function(sHTML, oPSelection, bNoUndo){
		...
		//[SMARTEDITORSUS-888] 웨브열람기 별 시험 후 아래 부분이 불필요하여 제거함
		//	- [SMARTEDITORSUS-387] IE9 표준방식에서 요소 뒤에 어떠한 요소도 없는 상태에서 카숄이 안들어가는 현상.
		...
		if(!oNavigator.ie){
			...
			// [SMARTEDITORSUS-56] 사진을 련속으로 첨부할 경우 련이어 삽입되지 않는 현상으로 이슈를 발견하게 되였습니다.
			// 그러나 이는 비단 '다수의 사진을 첨부할 경우'에만 발생하는 문제는 아니였고
			// 원인 확인 결과 콘텐츠 삽입후 기존 Bookmark 삭제시 갱신된 Selection 이 제대로 반영되지 않는 점이 있었습니다.
			// 이에, Selection 을 갱신하는 코드를 추가하였습니다.
			oSelection = this.oApp.getSelection();
			
			//[SMARTEDITORSUS-831] 비IE 계렬 웨브열람기에서 스크롤바가 생기게 문자입력 후 Enter 클릭하지 않은 상태에서 
			//그림 하나 삽입시 그림에 포커싱이 놓이지 않습니다.
			...
			// [SMARTEDITORSUS-428] [IE9.0] IE9에서 포스트 쓰기에 접근하여 맨우에 임의의 글감 첨부후 Enter를 클릭 시 글감이 사라짐
			// PASTE_HTML 후에 IFRAME 부분이 선택된 상태여서 Enter 시 내용이 제거되여 발생한 문제
			oSelection.collapseToEnd();
			oSelection.select();
		...
		// [SMARTEDITORSUS-639] 사진 첨부후 그림 뒤의 공백으로 인해 스크롤이 생기는 문제
	...
	$ON_INSERT_MULTIMEDIA : function(aFiles, aUrls){
		if ( !aFiles || aFiles.length === 0 ) {
			return;
		}
		this.oApp.exec("PASTE_HTML", [aFiles]);
		if ( aUrls && aUrls[0] && aUrls[0].length !== 0 ) {
			var urls = aUrls[0];
			var doc = this.getDocument();
			var videoPlayers = doc.getElementsByTagName("video");
			for ( var i = 0; i< videoPlayers.length; i ++ ) {
				videoPlayers[i].src = urls[i] || '';
			}
		}
	},

	/**
	 * [SMARTEDITORSUS-344]사진/동영상/지도 련속첨부시 포커싱 개선이슈로 추가된 함수.
	 */
	$ON_FOCUS_N_CURSOR : function (bEndCursor, sId){
		var el, oSelection;
		if(sId && ( el = jindo.$(sId, this.getDocument()) )){
			// ID가 지정된 경우 무조건 해당 부분으로 카숄 이동
			clearTimeout(this._nTimerFocus);	// 련속 삽입될 경우 미완료 타이머는 취소한다.
		...
		oSelection = this.oApp.getSelection();
		if(!oSelection.collapsed){ // select 령역이 있는 경우
			if(bEndCursor){
				oSelection.collapseToEnd();
			} else {
			...
			oSelection.select();
		}else if(bEndCursor){ // select 령역이 없는 상태에서 bEndCursor 이면 body 맨 뒤로 이동시킨다.
			this.oApp.exec("FOCUS");
			...
			oSelection.select();
			this._scrollIntoView(el);
		}else{	// select 령역이 없는 상태라면 focus만 준다.
			this.oApp.exec("FOCUS");
		}			
	},
	
	/* 
	 * 요소의 top, bottom 값을 반환
	 */
	_getElementVerticalPosition : function(el){
		...
		// 시험코드를 실행하면 IE8 이하에서 offsetParent 접근시 다음과 같이 알수 없는 exception 이 발생함
		// "SCRIPT16389: 지정되지 않은 오유입니다."
		// TODO: 해결방법이 없어서 일단 try/catch 처리했지만 추후 정확한 리유를 파악할 필요가 있음
	    try{
	    	while(elParent) {
	...
	/* 
	 * Window에서 현재 보여지는 령역의 top, bottom 값을 반환
	 */
	...
	/* 
	 * 요소가 WYSIWYG Window의 Visible 부분에서 완전히 보이는 상태인지 확인 (일부만 보이면 false)
	 */
	...
	$BEFORE_MSG_EDITING_AREA_RESIZE_STARTED  : function(){
		...
		// StringBookmark를 사용해서 위치를 보관해둠. (step1)
		if(!jindo.$Agent().navigator().ie){
			var oSelection = null;
			oSelection = this.oApp.getSelection();
	...
	$AFTER_MSG_EDITING_AREA_RESIZE_ENDED : function(FnMouseDown, FnMouseMove, FnMouseUp){
		...
		// $BEFORE_MSG_EDITING_AREA_RESIZE_STARTED에서 보관한 StringBookmark를 설정해주고 삭제함.(step2)
		if(!jindo.$Agent().navigator().ie){
	...
	/**
	  * EVENT_EDITING_AREA_PASTE 의 ON 통보문 핸들러
	  *		위지윅 방식에서 편집기 본문의 paste 이벤트에 대한 통보문을 처리한다.
	  *		paste 시에 내용이 붙여진 본문의 내용을 바로 가져올수 없어 delay 를 준다.
	  */	
	...
	_replaceBlankToNbsp : function(el){
		...
		if(oNavigator.nativeVersion !== 9 || document.documentMode !== 7) { // IE9 호환방식에서만 발생
			return;
		}
		...
		try {
			...
			if (sCSSBaseURI){
				...
				linkNode.onload = jindo.$Fn(function(){
					// [SMARTEDITORSUS-1853] IE의 경우 css가 적재되여 반영되는데 시간이 걸려서 웨브열람기 기본서체가 설정되는 경우가 있음
					// 때문에 css가 적재되면 SE_WYSIWYGStylerGetter 플러그인의 양식정보를 RESET 해준다.
					// 주의: 크롬의 경우 css 적재가 더 먼저 발생해서 SE_WYSIWYGStylerGetter 플러그인에서 오유가 발생할수 있기 때문에 RESET_STYLE_STATUS 통보문 호출이 가능한 상태인지 검사함
	...
	setIR : function(sIR){
		// [SMARTEDITORSUS-875] HTML 방식의 beautify에서 추가된 공백을 다시 제거
		//sIR = sIR.replace(/(>)([\n\r\t\s]*)([^<]?)/g, "$1$3").replace(/([\n\r\t\s]*)(<)/g, "$2")
		// --[SMARTEDITORSUS-875]
		...
		/*if(oNavigator.ie && oNavigator.nativeVersion >= 9 && document.documentMode >= 9){
			// [SMARTEDITORSUS-704] \r\n이 있는 경우 IE9 표준방식에서 정렬 시 웨브열람기가 <p>를 추가하는 문제
			sContent = sContent.replace(/[\r\n]/g,"");
		}*/

		// 편집내용이 없는 경우 카숄홀더로 대체
		if(sContent.replace(/[\r\n\t\s]*/,"") === ""){
		...
		// [SMARTEDITORSUS-1623] IE11은 <p></p>로 변환하면 행이 붙어버리기 때문에 IE10만 적용하도록 수정
	...
	_recordUndo : function(oKeyInfo){
		...
		// && oKeyInfo.keyCode != 32		// 속도문제로 인하여 Space 는 제외함
...
nhn.husky.SE_EditingArea_HTMLSrc = jindo.$Class({
	...
	sMode : "HTMLSrc",
	bAutoResize : false,	// [SMARTEDITORSUS-677] 해당 편집방식의 자동확장 기능 On/Off 여부
	nMinHeight : null,		// [SMARTEDITORSUS-677] 편집령역의 최소 높이
	...
	$ON_MSG_APP_READY : function() {
		if(!!this.oApp.getEditingAreaHeight){
			this.nMinHeight = this.oApp.getEditingAreaHeight();	// [SMARTEDITORSUS-677] 편집령역의 최소 높이를 가져와 자동 확장 처리를 할 때 사용
		}
	},
	...
	$ON_CHANGE_EDITING_MODE : function(sMode) {
		if (sMode == this.sMode) {				
			...
			/**
			 * [SMARTEDITORSUS-1889] Editor 령역을 표시하고 숨기는 데 있어서
			 * display 속성 대신 visibility 속성을 사용하게 되면서,
			 * Editor 령역이 화면에서 사라지지만
			 * 공간을 차지하게 되므로
			 * 그 아래로 위치하는 HTML 령역을 끌어올려 준다.
			 * 
	...
	$AFTER_CHANGE_EDITING_MODE : function(sMode, bNoFocus) {
		if (sMode == this.sMode && !bNoFocus) { 
			...
			//원인 : WYSIWYG방식이 아닐 때에도 iframe의 contentWindow에 focus가 가면서 focus기능이 작동하지 않음
			//해결 : WYSIWYG방식 일때만 실행 되도록 조건식 추가 및 기존에 blur처리 코드 삭제
			//모바일 textarea에서는 직접 클릭을해야만 건반이 먹히기 때문에 우선은 카숄이 안보이게 해서 사용자가 직접 클릭을 유도.
	...
	/**
	 * [SMARTEDITORSUS-677] HTML 편집령역 자동 확장 처리시작
	 */ 
	startAutoResize : function(){
		var htOption = {
	...
	/**
	 * [SMARTEDITORSUS-677] HTML 편집령역 자동 확장 처리 완료
	 */ 
	stopAutoResize : function(){
	...
	setIR : function(sIR) {
		...
		// [SMARTEDITORSUS-1589] 문서 방식이 Edge인 IE11에서 WYSIWYG 방식와 HTML 방식 전환시 문말에 무의미한 <br> 두 개가 첨가되는 현상으로 필터링 추가
		var htBrowser = jindo.$Agent().navigator();
		if(htBrowser.ie && htBrowser.nativeVersion == 11 && document.documentMode == 11){ // Edge 방식의 documentMode 값은 11
...
nhn.husky.SE_EditingArea_TEXT = jindo.$Class({
	...
	irToText : function(sHtml) {
		...
		//0.안보이는 값들에 대한 정리. (편집기 방식에 view와 text방식의 view를 동일하게 해주기 위해서)		
		sContent = sContent.replace(/\r/g, '');// MS엑셀 테블에서 tr별로 분리해주는 역할이\r이기 때문에  text방식으로 변경시에 가독성을 위해 \r 제거하는 것은 림시 보류. - 11.01.28 by cielo 
		sContent = sContent.replace(/[\n|\t]/g, ''); // 개행문자, 안보이는 공백 제거
...
nhn.husky.SE_EditingAreaVerticalResizer = jindo.$Class({
	...
	nEditingAreaMinHeight : null,	// [SMARTEDITORSUS-677] 편집령역의 최소 높이
	...
	$ON_MSG_APP_READY : function(){
		if(this.oApp.bMobile){
			...
			// [SMARTEDITORSUS-1679] 하지만 사용자가 조절하지는 못하도록 단추는 비활성화 한다.
			this.oResizeGrip.disabled = true;
			this.oResizeGrip.style.height = '0';	// 단추의 문구를 가림. display:none을 하면 안드로이드에서 높이 계산오유 발생 
		}else{
		...
		if(!!this.oApp.getEditingAreaHeight){
			this.nEditingAreaMinHeight = this.oApp.getEditingAreaHeight();	// [SMARTEDITORSUS-677] 편집령역의 최소 높이를 가져와 Gap 처리시 사용
		}
	},
	...
	/**
	 * [SMARTEDITORSUS-677] [편집기 자동확장 ON인 경우]
	 * 입력창 크기 조절 바의 위치를 확인하여 웨브열람기 하단에 위치한 경우 자동확장을 멈춤
	 */	
	checkResizeGripPosition : function(bExpand){
		...
		if(bExpand){
			if(this.nEditingAreaMinHeight > this.oApp.getEditingAreaHeight() - nGap){	// [SMARTEDITORSUS-822] 수정 방식인 경우에 대비
				nGap = (-1) * (this.nEditingAreaMinHeight - this.oApp.getEditingAreaHeight());
			}
	
			// Gap 만큼 편집령역 크기를 조절하여
			// 사진 첨부나 붙여넣기 등의 크기가 큰 내용 추가가 있었을 때 입력창 크기 조절 바가 숨겨지지 않도록 함
...
nhn.husky.SE_WYSIWYGEnterKey = jindo.$Class({
	...
	$init : function(sLineBreaker){
		...
		// [SMARTEDITORSUS-227] IE 인 경우에도 편집기 Enter 처리 로직을 사용하도록 수정
		if(this.htBrowser.opera && this.sLineBreaker == "P"){
			this.$ON_MSG_APP_READY = function(){};
		}

		/**
		 *	[SMARTEDITORSUS-230] 밑줄+색상변경 후 Enter치면 스크립트 오유
		 *	[SMARTEDITORSUS-180] [IE9] 배경색 적용후 Enter키 2회이상 입력시 카숄위치가 다음 행으로 이동하지 않음
		 * 		오유 현상 : 	IE9 에서 Enter 후 생성된 P 태그가 "빈 SPAN 태그만 가지는 경우" P 태그 령역이 보이지 않거나 초점이 우로 올라가 보임
		 *		해결 방법 : 	카숄 홀더로 IE 이외에서는 <br> 을 사용
		 *						- IE 에서는 렌더링 시 <br> 부분에서 비정상적인 P 태그가 생성되여 [SMARTEDITORSUS-230] 오유 발생
		 *						unescape("%uFEFF") (BOM) 을 추가
		 *						- IE9 표준방식에서 [SMARTEDITORSUS-180] 의 문제가 발생함
		 *						(unescape("%u2028") (Line separator) 를 사용하면 P 가 보여지나 사이드이펙트가 우려되여 사용하지 않음)
		 *	IE 웨브열람기에서 Enter 처리시 &nbsp; 를 넣어주므로 해당 방식을 그대로 사용하도록 수정함
		 */
		if(this.htBrowser.ie){
			this._addCursorHolder = this._addCursorHolderSpace;
			
			//[SMARTEDITORSUS-1652] 글자크기 지정후 Enter를 치면 빈SPAN으로 감싸지는데 IE에서 빈SPAN은 높이값을 갖지 않아 카숄이 올라가 보이게 됨
			// 따라서, IE의 경우 웨브열람기방식와 상관없이 다음행의 SPAN에 무조건 ExtraCursorHolder 를 넣어주도록 코멘트처리함
...
//			}
	...
	$ON_MSG_APP_READY : function(){
		...
		this.tmpTextNode = this.oSelection._document.createTextNode(unescape("%u00A0"));	// 공백(&nbsp;) 추가시 사용할 노드
	...
	/**
	 * [SMARTEDITORSUS-950] 편집기 적용 페지의 Compatible meta IE=edge 설정시 줄간격 벌어짐 이슈 (<BR>)
	 */
	$ON_REGISTER_CONVERTERS : function(){
	 ...
	 *	: 보관시 본문 령역에서 P 아래의 모든 하위 태그 중 가장 마지막 childNode가 BR인 경우를 탐색하여 이를 &nbsp;로 변경해 줍니다.
	 */
	...
	// [IE] 빈 노드 내에 카숄을 표시하기 위한 처리 (_addSpace 사용)
	...
	/**
	 * [SMARTEDITORSUS-1797] 북마크 다음 노드가 본문노드인 경우 문자렬 앞쪽의 공백문자(\u0020)를 &nbsp;(\u00A0) 문자로 변환한다.
	 */
	...
	_wrapBlock : function(oEvent, sWrapperTagName){
		...
		if(!oStart.bParentBreak || oSelection.rxBlockContainer.test(oStart.oLineBreaker.tagName)){
			...
			if(!!oStart.oNode.parentNode && oStart.oNode.parentNode.nodeType !== 11){
				...
				// [SMARTEDITORSUS-1513] oStart.oNode와 oEnd.oNode 사이에 BR이 있는 경우 다음 Enter시 양식이 비정상으로 복사되기 때문에 중간에 BR이 있으면 BR까지만 잘라서 설정한다.
				var oEndNode = this._getBlockEndNode(oStart.oNode, oEnd.oNode);
				// [SMARTEDITORSUS-1743] oStart.oNode가 BR인 경우 setStartBefore와 setEndAfter에 모두 oStart.oNode로 설정을 시도하기 때문에 스크립트 오유가 발생한다.
				// 따라서, _getBlockEndNode 메쏘드를 통해 찾은 BR이 oStart.oNode인 경우 oEnd.oNode 를 설정한다.
				...
				oSelection.moveToStringBookmark(sBM, true);	// [SMARTEDITORSUS-180] focus 재설정
				oSelection.collapseToEnd();					// [SMARTEDITORSUS-180] focus 재설정
		...
		if(this.htBrowser.firefox){
			if(elBookmark && elBookmark.nextSibling && elBookmark.nextSibling.tagName == "IFRAME"){
				...
				// [SMARTEDITORSUS-1797] Enter시 공백문자를 &nbsp; 로 변환
				// FF의 경우 2번이상 Enter치면 앞쪽공백이 사라져서 setTimeout으로 처리
				setTimeout(jindo.$Fn(function(sBM){
					var elBookmark = oSelection.getStringBookmark(sBM, true);
					if(!elBookmark){return;}
...
//			// -- [SMARTEDITORSUS-1452]
			...
			// [SMARTEDITORSUS-26] Enter 후에 밑줄/취소선이 복사되지 않는 문제를 처리 (웨브열람기 Enter 처리후 실행되도록 setTimeout 사용)
			...
			// [SMARTEDITORSUS-180] 빈 SPAN 태그에 의해 Enter 후 Enter가 되지 않은 것으로 보이는 문제 (웨브열람기 Enter 처리후 실행되도록 setTimeout 사용)
	...
	// [IE9 standard mode] Enter 후의 상/하단 P 태그를 확인하여 BOM, 공백(&nbsp;) 추가
	_addExtraCursorHolder : function(elUpperNode){
		...
		// Enter 후에 비여있는 하단 SPAN 노드에 BOM 추가
		var oSelection = this.oApp.getSelection(),
			sBM,
			elLowerNode,
			elParent;
		...
		// Backspace시 카숄이 움직이지 않도록 카숄을 카숄홀더 앞쪽으로 옮긴다.
		oSelection.selectNodeContents(elLowerNode);
		oSelection.collapseToStart();		
		oSelection.select();
	},
	
	// [IE] P 태그 가장 뒤 자식노드로 공백(&nbsp;)을 값으로 하는 본문 노드를 추가
	_addSpace : function(elNode){
		...
		while(elChild){	// &nbsp;를 붙일것이니 P 바로 아래의 "%uFEFF"는 제거함
			elNextChild = elChild.nextSibling;
		...
		return elNode;	// [SMARTEDITORSUS-418] return 요소 추가
	},
	
	// [IE] Enter 후에 취소선/밑줄 태그를 임의로 추가 (취소선/밑줄에 색상을 표시하기 위함)
...
//				oTargetNode.innerHTML = "\u200B";
	...
	_getStyleOnlyNode : function(elNode){
		...
		// [SMARTEDITORSUS-227] TEXT_NODE 가 return 되는 문제를 수정함. IE 에서 TEXT_NODE 의 innrHTML 에 접근하면 오유 발생
	...
	_insertBR : function(oEvent){
		...
		if(!this.htBrowser.ie){
			...
			// <div> 1234<br></div>인경우 FF에서는 다음 행으로 카숄 이동이 안 일어남.
			// 그래서  <div> 1234<br><br type='_moz'/></div> 이와 같이 생성해주어야 편집기 상에 2줄로 되여 보임.
...
//}
/**
 * @pluginDesc WYSIWYG 령역에 붙여넣어지는 외부 콘텐츠를 정제하는 플러그인
 */
nhn.husky.SE_PasteHandler = jindo.$Class({
	...
	$init : function(sParagraphContainer){
		...
		/**
		 * 본문에 붙여넣어진 콘텐츠 중, 
		 * 이 플러그인에서 정제한 콘텐츠로 치환할 대상 태그 이름의 목록
		 * */
	...
	/**
	 * [SMARTEDITORSUS-1862] [IE] IR_TO_DB converter 작동 시점에 한해 콘텐츠 정제를 수행하도록 한다.
	 * 
	 * Editor 령역에 붙여넣을 때 이미 양식이 유지되고 있는 데다가
	 * 
	 * IE 8이 최신 판본인 Win 7의 경우
	 * MS Excel의 표를 붙여넣는 중
	...
	_handlePaste : function(we){
		...
		/**
		 * [Chrome, Safari6+] clipboard에서 넘어온 style 정의를 보관해 둔 뒤
		 * 특정 열린 태그에 style을 적용해야 할 경우 활용한다.
		 * 
		 * MS Excel 2010 기준으로
		 * <td>에 담긴 class 명을 획득한 뒤
		 * 보관해 둔 style에서 매칭하는 값이 있으면
		 * style을 해당 태그에 적용한다.
		 * 
		 * [IE] Text 형태로만 값을 가져올 수 있기 때문에 style 정의 보관 불가
		 * */
		...
		if(elStyle){
			...
			// style="" 내부에 삽입되는 경우 조화를 이루어야 하기 때문에 쌍인용부호를 인용부호로 치환
			sStyleFromClipboard = sStyleFromClipboard.replace(/"/g, "'");
				
			this._sStyleFromClipboard = sStyleFromClipboard;
		}

		// [SMARTEDITORSUS-2005] clipboard에서 넘어오는 자료가 <table>을 포함하고 있지 않다면 웨브열람기에 맡긴다.
		bProcess = this._hasConversionTarget(sClipboardData);
		if(bProcess){
		// --[SMARTEDITORSUS-2005]
			this._preparePaste();
			
			// 웨브열람기의 고유 붙여넣기 동작으로 콘텐츠가 본문 령역에 붙여넣어진다.
			setTimeout(jindo.$Fn(function(){
				// [SMARTEDITORSUS-1676]
				/**
				 * 콘텐츠가 붙여넣어지는 과정에서
				 * 콘텐츠의 앞 부분 본문 일부는
				 * 앞쪽 zero-width space 본문 노드에 병합되는 경우가 있다.
				 * 
				 * 따라서 이 본문 노드 전체를 들어내는 것은 어렵고
				 * 시작 부분에 남아 있는 zero-width space 문자만 제거할수밖에 없다.
				 * */
				var rxZwspStart = new RegExp("^[\u200b]");
				if(this.zwspStart){
					/**
					 * [SMARTEDITORSUS-1676] [IE 8~10] 선택령역이 없는 상태에서 붙여넣기를 수행한 경우
					 * [Object Text]의 nodeValue 속성은 unknown 형식
					 * */
					if(typeof(this.zwspStart.nodeValue) == "unknown"){
						...
						/**
						 * 제거 후 빈값이라면 인위적으로 삽입해 준 앞쪽 zwsp 본문 노드를 유지할 필요가 없다.
						 * 
						 * [Chrome, Safari 6+] 두 번째 조건식이 필요하다.
						 * 붙여넣어지는 콘텐츠가 line-height 속성을 가진 span 태그인 경우
						 * this.zwspEnd.parentNode가 사라지는 문제가 있다.
						 * 이와는 직접적으로 관련되여 있지 않으나
				...
				/**
				 * 뒤쪽 zero-width space가 포함된 본문 노드 마지막의
				...
				if(this.zwspEnd){
					...
					if(typeof(this.zwspEnd.nodeValue) == "unknown"){
						...
						/**
						 * 제거 후 빈값이라면 인위적으로 삽입해 준 뒤쪽 zwsp 본문 노드를 유지할 필요가 없다.
						 * 
						 * [Chrome, Safari 6+] 두 번째 조건식이 필요하다.
						 * 붙여넣어지는 콘텐츠가 line-height 속성을 가진 span 태그인 경우
						 * this.zwspEnd.parentNode가 사라지는 문제가 있다.
				...
				// [SMARTEDITORSUS-1676]
				// 붙여넣어진 콘텐츠를 복사해 두고, SmartEditor에 맞는 콘텐츠로 가공한다.
	...
	/**
	 * [SMARTEDITORSUS-2005] clipboard 안에 담긴 콘텐츠가 <table>을 포함하고 있는지 확인한다.
	 * */
	...
	/**
	 * 붙여넣어지는 외부 프로그람의 콘텐츠를 조작하기 위한 준비를 한다.
	 * */
	...
	_securePasteArea : function(){
		 ...
		 * 콘텐츠가 의도한 위치에 붙여넣어지는데
		 * 
		 * HuskyRange에서 북마크 용도로 삽입하는 빈 <span>으로는 이를 충족할수 없다.
		 * (붙여넣어진 콘텐츠가 <span> 북마크를 잠식)
		 * 
		 ...
		 * 선택된 령역의 콘텐츠가 지워지지 않는다.
		 * 
		 * <시작 북마크 /><\u200b>[목표 카숄 위치]<\u200b><끝 북마크 />
		 * */
	...
	/**
	 * 외부 프로그람의 콘텐츠가 원문에 붙여넣어지는 과정을 진행한다.
	 * */
	_processPaste : function(){
		...
		/**
		 * [SMARTEDITORSUS-1870]
		 * this._savePastedContents()를 거쳐 정제된 콘텐츠가 this._sTarget에 저장되며
		 * 경우에 따라 빈값이 할당되면 try/catch 블록에서 례외를 던지게 됨
		 * */
		if(this._sTarget){
			// [SMARTEDITORSUS-1673]
			try{
				...
				// 붙여넣기 작업 공간에 붙여넣은 콘텐츠를 떠서 본문의 해당 령역 교체
				this._loadToBody();
	...
	/**
	 * 본문 령역에 외부 프로그람의 콘텐츠가 붙여넣어지면 이를 저장하고
	...
	_savePastedContents : function(){
		...
		if(sTarget != ""){
			...
			/**
			 * [FireFox, Safari6+] clipboard에서 style 정의를 보관할수는 없지만
			 * 본문에 붙여넣어진 뒤 획득하여 보관 가능
			 * 
			 * iWork Pages의 경우 이전 시점에서 들어온 양식 정보가 이미 존재할수도 있기 때문에 
			 * 기존변수에 값을 더해넣는 방식 사용
			 * 
			 * @XXX [Firefox] 27.0 갱신 이후 style 정보가 넘어오지 않아 값을 보관할수 없음
			 * */
	...
	/**
	 * [SMARTEDITORSUS-1673] X-Browsing 비호환 속성인 outerHTML fix
	 * */
	...
	/**
	 * SmartEditor에 맞는 필터링을 거친 콘텐츠를 반환한다.
	 * @param {String} 필터링 대상이 될 HTML
	 * @param {Boolean} [SMARTEDITORSUS-1870] 전체 콘텐츠 중 table만 뽑아내서 필터링할지 결정
	 * */
	_filterPastedContents : function(sOriginalContent, bUseTableFilter){
		// 문단 교체와 관련된 변수
		this._isPastedMultiParagraph = false; // 붙여넣어지는 콘텐츠가 여러 문단으로 구성되여 있는지 확인
		...
		nParagraphChangeCount = 0, // 문단 교체 회수
		bParagraphIsOpen = false; // 현재 문단이 열려 있는지 확인
		
		var sMatch, // 판별식과 일치하는 부분 
		sResult, // 판별식과 일치하는 부분이 필터링을 거쳐 최종적으로 반환되는 형태
		aResult = [], // 최종적으로 반환되는 정제된 콘텐츠들이 담긴 배렬
		nPreviousIndex = -1, // 직전 작업 부분이 결과 배렬에서 차지하는 index
		sTagName, // 판별식과 일치하는 부분의 태그명
		sPreviousContent = "", // 직전 삽입된 콘텐츠
		...
		// 현재 작업이 테블 내부에서 이루어지고 있는지 확인. tr, td에 style이 명시되여 있지 않은 경우 사용
		isInTableContext = false,
		nTdIndex = 0, // tr, td의 style 캐싱 중에 현재 몇 번째 td인지 확인을 위함
		nTdLength = 0, // 캐싱 시점에 총 td의 수를 구함
		aColumnWidth = [], // col의 width를 보관하는 배렬
		nRowHeight = 0; // tr의 height 보관용
		// [SMARTEDITORSUS-1671] 다중 테블의 col 캐싱
		var nTableDepth = 0;
		var aaColumnWidth = []; // 이차원 배렬
		// --[SMARTEDITORSUS-1671]
		...
		rxCommentTag = /^<!--[^<>]+-->/, // 주석이나 카스텀 태그
		rxOpeningCommentTag = /^<!--[^->]+>/, // 열린 주석 태그
		rxClosingCommentTag = /^<![^->]+-->/,	// 닫힌 주석 태그
		rxWhiteSpace = /^[\s]+/, // 공백
		rxNonTag = /^[^<\n]+/, // 태그 아닌 요소
		rxExceptedOpeningTag = /^<[^<>]+>/, // 어느 조건도 통과하지 않은, 열린 례외 태그들

		// MS 프로그람의 테블에서 특히 사용하는 패턴
		rxMsoStyle = /(mso-[^:]+[\s]*:[\s]*)([^;"]*)([;]?)/gi, // Mso-로 시작하는 양식이 있는지 검사
		// [SMARTEDITORSUS-1673]
		...
		rxApplied; // 결과 문자렬 작업시 적용하는 패턴

		// [SMARTEDITORSUS-1870]
		// 열린 태그에서 &quot;를 " 로 변환
		var rxQuot = /&quot;/gi;
		
		// clipboard로부터 양식 정의 획득에 사용
		var sClassAttr = "";
		var aClass = [];
		...
		/**
		 * [SMARTEDITORSUS-1870] 콘텐츠를 받아 필터링 수행
		 * */
		this._doFilter = jindo.$Fn(function(sOriginalContent){
			...
			while(sOriginalContent != ""){
				 ...
				 * sResult : sMatch에서 정제가 이루어진 결과물. 이들의 집합이자, 반환값과 련결된 aResult에 보관된다.
				...
				if(rxOpeningAndClosingTag.test(sOriginalContent)){ // <tagName />
					...
					// 실질적으로 양식이나 클라스 조작이 이루어지는 쪽은 열린 태그 부분
					// &quot; 를 ' 로 치환
					 ...
					 * 모든 열린 태그 공통처리사항.
					 * 
					 * width, height가 attribute에 할당되여 있거나 그 단위가 px가 아닌 pt인 경우에
					 * px 단위로 style 안으로 바꿔넣는 보정이 이루어진다.
					 * __se_tbl 클라스를 가진 SmartEditor의 표는
					 * width, height의 크기변경이 발생할 때
					 * 실시간 변화가 적용되는 style에 그 결과값을 px로 보관하기 때문이다.
					 * @see hp_SE2M_TableEditor.js
					 * */
					/**
					 * [Chrome, FireFox 26-, Safari6+] 보관해 둔 style 정의로부터
					...
					if(this.htBrowser.chrome || this.htBrowser.firefox || (this.htBrowser.safari && this.htBrowser.version >= 6)){
						...
						if(sClassAttr && (sClassAttr.indexOf('mso') === -1)){
							// MsoTableGrid 클라스가 포함된 경우(MS Word)는 제외 : style 정의를 불러와서 적용하면 오히려 레이아웃 비정상
					...
					if(sTagName === 'TABLE'){
						/**
						 * [SMARTEDITORSUS-1673] 외부에서 붙여넣은 테블에 대하여 __se_tbl_ext 클라스 부여
						 * */
						if(nTableDepth === 0){
							if(sClassAttr){
						...
						// </table> 태그가 등장하기 전까지 작업은 table에서 이루어진다.
						isInTableContext = true;
	
						// [SMARTEDITORSUS-1671] 테블을 다중으로 관리
						nTableDepth++;
						// --[SMARTEDITORSUS-1671]
					}
					 ...
					 * <colgroup> 이하 <col>에 같은 렬에 해당하는 쎌의 너비가 정의되여 있으며,
					 * 같은 행에 해당하는 쎌의 높이는 <tr>에 정의되여 있다.
					 * 이를 보관해 두고 각 <td>의 너비와 높이에 적용하는데 사용한다.
					 * 
					 * [SMARTEDITORSUS-1870]
					 * colgroup에서 보관해야 할 크기 정보가
					 * 붙여넣을 때 이미 적용된 IE에서는
					 * colgroup 캐싱 불필요
					 * 
					...
					// --[SMARTEDITORSUS-1870]
						...
						// span 개수를 세서 row 수인  nTdLength 갱신
						_nSpan = 0;
					...
					// --[SMARTEDITORSUS-1870]
						 ...
						 * 
						 * MS Excel 2010 기준으로,
						 * 0.5pt 두께로 넘어온 border는 100% 배률에서 표현되지 않기에
						 * 일괄 1px로 변환한다.
						 * 
						 * 통상 0.84px 이상이면 100% 배률에서 표현된다.
						 * */
						...
						// [SMARTEDITORSUS-1870] colgroup에서 보관한 값이 붙여넣을 때 이미 적용된 IE에서는 처리 불필요
						if(!this.htBrowser.ie){
							// 양식값이 없을 때 colgroup에서 보관한 값이 있으면 이를 적용함
							// [SMARTEDITORSUS-1671]
							aColumnWidth = aaColumnWidth[nTableDepth];
							if(!!aColumnWidth && aColumnWidth.length > 0){ // 보관한 값 있음
								// [SMARTEDITORSUS-1871]
							...
							if(!rxHeightStyle.test(sMatch) && !!nRowHeight){
								// 양식값이 없을 때 tr에서 저장한 값이 있으면 이를 적용함
								// [SMARTEDITORSUS-1671]
								if(!!rxStyle.test(sMatch)){
					...
					if(rxMultiParagraphIndicator.test(sTagName)){
						this._isPastedMultiParagraph = true; // 붙여넣어진 콘텐츠가 여러 문단으로 구성되여 있는지 확인
				...
				}else if(rxWhiteSpace.test(sOriginalContent)){ // 공백문자는 일단 그대로 통과시킴. 차후 처리 방안이 있을지 론의 필요
					...
					// 빈 <td>였다면 &npsp;가 추가되여 있기 때문에 연산자가 다른 경우와는 다름
					sResult += sMatch;
					
					rxApplied = rxClosingTag; 
				}
				// 지금까지의 조건에 부합하지 않는 모든 태그는 례외 태그로 처리한다. MS Word의 <o:p> 등이 해당.
				...
				if(sResult != ""){
					...
					// 원본 String의 맨 앞부터 첫 문단 교체가 일어나기까지의 모든 inline 요소들을 보관해 두고 활용
					var sGoesPreviousParagraph = "";
					if(!this._isPastedMultiParagraph){ // 원본 String의 맨 앞부터 첫 문단 교체가 일어나기까지의 모든 inline 요소들을 보관해 두고 활용
					...
					// 최종적으로 반환되는 정체된 콘텐츠들이 담긴 배렬
					aResult.push(sResult);
				}
				
				// 정제가 끝난 콘텐츠는 원래 콘텐츠에서 제거
	...
	/**
	 * [SMARTEDITORSUS-1870] 
	 * 전달받은 콘텐츠 중 <table> 부분만 뽑아내서 필터를 거치며,
	 * 반환 결과는 웨브열람기에 따라 다름
	 * -[IE] 필터링이 보관시점에 수행되기 때문에, <table> 부분과 그 나머지 부분을 다시 조립해서 반환
	...
	_filterTableContents : function(sOriginalContents){
		...
		var _res, // 정규식 탐색 결과
		_nStartIndex, // <table/> 맥락이 시작되는 <table> 문자렬의 위치
		_nEndIndex,  // <table/> 맥락이 끝나는 </table> 문자렬의 위치
		_aStartIndex = [/* _nStartIndex */],
		_aEndIndex = [/* _nEndIndex */],
		_nLastIndex_tmp_start, 
		_nLastIndex_tmp_end,
		
		_aTable = [], // 대상 콘텐츠에서 뽑아낸 <table/>
		_nTable_start = 0, // 현재 <table/> 맥락에서 <table> 문자렬 개수
		_nTable_end = 0; // 현재 <table/> 맥락에서 </table> 문자렬 개수

		// 대상 콘텐츠에서 <table>을 찾아서 추출
		function _findAndMarkTable(){
		    ...
		    // 다음 문자렬 비교를 위한 림시 탐색
		    var res_tmp_start = _rxTable_start.exec(_sTarget); // 현재 위치에서 다음 <table> 문자렬 탐색
		    var res_tmp_end = _rxTable_end.exec(_sTarget); // 현재 위치에서 다음 </table> 문자렬 탐색
		    
		    var nLastIndex_start = _rxTable_start.lastIndex; // 다음 <table> 문자렬 위치 기록
		    var nLastIndex_end = _rxTable_end.lastIndex; // 다음 </table> 문자렬 위치 기록
		...
		function _doRxTable_start(){
		    ...
		    if(_nTable_start == 1){
		        _nStartIndex = _res.index; // 현재 <table> 문자렬의 위치
		...
		function _doRxTable_end(){
		    ...
		    // <table/>이 완전하게 열리고 닫히는 시점에, 이 <table/>을 대상 콘텐츠에서 뽑아낸다.
		    if((_nTable_start !== 0) && (_nTable_end !== 0) && (_nTable_start == _nTable_end)){
		        _nEndIndex = _res.index; // 현재 </table> 문자렬의 위치
		...
		if(this.htBrowser.ie){
			...
			if((_nStartIndexLength > 0) && (_nStartIndexLength == _nEndIndexLength)){ // <table/> 쌍이 정상적으로 편성되였다면 열린 회수와 닫힌 회수가 같아야 함
				/**
				 * string의 index 정보를 기반으로 대상 콘텐츠를 재조립한다. 
				 * 
				 * -대상 콘텐츠
				...
				// table이 하나도 없는 콘텐츠일 때는 기존 콘텐츠 그대로 반환
				return _sTarget;
			}    
		}else{
			return _aTable.join('');
	...
	/**
	 * [SMARTEDITORSUS-1673] 붙여넣어진 콘텐츠가 가공되고 나면 이를 붙여넣기 작업 공간에 붙여넣는다. 
	 * */
	_loadToPasteHelper : function(){
		// 붙여넣을 콘텐츠가 여러 문단일 경우 보관하는 배렬
		var aParagraph = [];
	...
	/**
	 * [SMARTEDITORSUS-1673] 붙여넣기 작업 공간에 붙여넣은 콘텐츠 중,
	 * 본문 령역에 붙여넣을 콘텐츠를 선별하여 
	 * 웨브열람기 고유의 붙여넣기 기능으로 본문 령역에 붙여넣은 기존 콘텐츠와 교체한다.
	 * */
	_loadToBody : function(){
		...
		try{
			 ...
			 * As-Is 콘텐츠
			 * 
			 * 본문 령역에 붙여넣어진 콘텐츠 중 가공된 콘텐츠로 치환될 대상 목록을 획득
			 * */
			oSelection.moveToStringBookmark(this._sBM);
			oSelection.select();
			...
			/**
			 * processPaste()에서 조작된 콘텐츠가 본문에 이미 삽입된 경우
			 * oSelectionClone을 기반으로 웨브열람기 고유 기능으로 붙여넣었던 콘텐츠를 복원한다.
			 * */
			// 삽입된 콘텐츠 제거
			oSelection.moveToStringBookmark(this._sBM);
			oSelection.select();
	 ...
	 * 
	 * @param {Array} 탐색할 노드가 담긴 배렬
	 * @param {Array} 탐색 태그명이 담긴 배렬
	 * [SMARTEDITORSUS-1676]
	 * @param {Array} true, false 중 하나를 반환하는 필터 함수들이 담긴 배렬 (선택)
...
//		this.oApp.exec("REGISTER_UI_EVENT", ["styleRemover", "click", "EXECCOMMAND", ["RemoveFormat", false, false]]);
	...
	/**
	 * TBODY, TR, TD 사이에 있는 본문노드인지 판별한다.
	 * @param oNode {Node} 검사할 노드
	 * @return {Boolean} TBODY, TR, TD 사이에 있는 본문노드인지 여부
	 */
	_isTextBetweenTable : function(oNode){
		var oTmpNode;
		if(oNode && oNode.nodeType === 3){	// 본문 노드
	...
	$BEFORE_EXECCOMMAND : function(sCommand, bUserInterface, vValue, htOptions){
		...
		oSelection = this.oApp.getSelection();
		// [SMARTEDITORSUS-1584] IE에서 테블관련 태그 사이의 본문노드가 포함된 채로 execCommand 가 실행되면 
		// 테블 태그들 사이에 더미 P 태그가 추가된다. 
		// 테블관련 태그 사이에 태그가 있으면 문법에 어긋나기 때문에 getContents 시 이 더미 P 태그들이 밖으로 빠져나가게 된다.
		// 때문에 execCommand 실행되기 전에 셀렉션에 테블관련 태그 사이의 본문노드를 찾아내 지워준다.
	 ...
	 * [Win XP - IE 8][IE 9~11] 인용부호 안에서 번호매기기, 글머리기호를 적용할 때 필요한 조치이다.
	 * 
	 * 인용부호안의 선택한 령역을 기준으로,
	 * 
	 * 선택한 령역이 없는 경우에는 해당 줄을 제외했을 때
	 * 선택한 령역이 있는 경우에는 선택한 줄을 제외했을 때
	 * 
	 * 더 이상의 <P>가 없는 경우
	 * execCommand("insertorderedlist"), execCommand("insertunorderedlist")가 오동작한다.
	 * 
	 * 이러한 오동작을 방지하기 위해
	 * 인용부호 안에서 번호매기기 글머리기호를 삽입할 때는
		...
		if( bSelectedBlock){
			...
			default:
				...
				if(oSelection.collapsed && this._rxCmdInline.test(sCommand)){
					...
					// execCommand를 실행할때마다 ZWSP가 포함된 더미 태그가 자꾸 생길수 있기 때문에 이미 있으면 있는 걸로 사용한다.
					...
					oSelection.removeStringBookmark(sBM);	// 미리 지워주지 않으면 더미 태그가 생길수 있다.
	...
	$AFTER_EXECCOMMAND : function(sCommand, bUserInterface, vValue, htOptions){
		...
		if(/^insertorderedlist|insertunorderedlist$/i.test(sCommand)){
			...
			// [SMARTEDITORSUS-1795] 갤럭시노트_Android4.1.2 기본웨브열람기일 경우 내부에 생성된 BLOCKQUOTE 제거
	...
	_getDocumentBR : function(){
		...
		// [COM-715] <Chrome/Safari> 요약글 삽입 > 더보기 령역에서 기호매기기, 번호매기기 설정할때마다 요약글 복스가 아래로 이동됨
		// ExecCommand를 처리하기 전에 현재의 BR을 보관
	...
	_fixDocumentBR : function(){
		// [COM-715] ExecCommand가 처리된 후에 갱신된 BR을 처리 전에 보관한 BR과 비교하여 생성된 BR을 제거
	...
	_outdentMargin : function(elDiv){
		...
		if(elDiv.tagName === "LI"){
			...
			// 내여쓰기 한 LI 외에 다른 LI가 존재 하지 않을 경우 부모 노드 지워줌
			if(!elParentNode.innerHTML.match(/LI/i)){
				elParentNode.parentNode.removeChild(elParentNode);
			}

			// OL이나 UL 우로까지 내여쓰기가 된 상태라면 LI를 벗겨냄
	...
	// [SMARTEDITORSUS-1857] 인용부호내에 UL/OL이 있고 바깥에서 UL/OL을 실행하는 경우도 동일한 문제가 발생하여 동일한 방식으로 해결하도록 해당 케이스 추가  
	_insertBlankLine : function(){
		...
		// [SMARTEDITORSUS-793] 인용부호 안에서 글머리기호/번호매기기하는 경우에 대한 처리 
		while(elNode){
		...
		// [SMARTEDITORSUS-1857] 인용부호 바깥에서 글머리기호/번호매기기하는 경우에 대한 처리
	...
	// 
	// [IE] 인용부호 안에서 글머리 기호를 적용했을 때 인용부호 밖에 적용된 번호매기기/글머리 기호가 인용부호 안으로 빨려 들어가는 문제 처리
	 ...
	 * <blockquote><p><ul><li><span class="Apple-style-span"><blockquote><p style="display: inline !important;">선택령역</p></blockquote></span></li></ul></p><blockquote>
	 * 삭제될때도 복사됨
	 * <blockquote><p><span class="Apple-style-span"><blockquote><p style="display: inline !important;">선택령역</p></blockquote></span></p><blockquote>
	 */
	_removeBlockQuote : function(){
		...
		// SPAN.Apple-style-span 을 selection 된 본문으로 교체한 후 다시 selection을 준다. 
	...
	_extendBlock : function(){
		...
		// 해결을 위해서는 현재 선택된 부분을 Block으로 extend하여 execCommand API가 처리될수 있도록 함
		...
		// <p><img><br/><img><br/><img></p> 일 때 그림이 일부만 선택되면 발생
		// - container 노드는 P 이고 container 노드의 자식노드 중 그림이 여러개인데 선택된 그림가 그 중 일부인 경우
...
nhn.husky.SE_WYSIWYGStyler = jindo.$Class({
	...
	$init : function(){
		...
		if(htBrowser.ie && htBrowser.version > 8){
			// [SMARTEDITORSUS-178] ZWNBSP(\uFEFF) 를 사용하면 IE9 이상의 경우 높이값을 갖지 못해 카숄위치가 이상함
			// [SMARTEDITORSUS-1704] ZWSP(\u200B) 를 사용할 경우 줄바꿈이 됨 
			// 기본적으로 \uFEFF 를 사용하고 IE9 이상만 \u2060 사용 (\u2060 은 \uFEFF 와 동일한 역할을 하지만 크롬에서는 깨짐)  
			// *주의* 작성자가 IE9이상에서 작성하고 독자가 크롬에서 볼 경우 \u2060 가 깨진 문자로 보여질 수 있기 때문에 변환을 통해 \u2060 를 \uFEFF 로 변환한다.
			// FIXME: 단, \u2060 를 \uFEFF 변환으로 인해 SPAN태그만 들어있는 상태에서 방식을 변환하면 카숄 위치가 다시 이상해질 수 있음
			// 참고:
	...
	$ON_SET_WYSIWYG_STYLE : function(oStyles){
		...
		if(oSelection.collapsed && !bSelectedBlock){
			...
			oSelection.collapseToEnd();
			// TODO: focus 는 왜 있는 것일까? => IE에서 style 적용후 초점이 날아가서 글작성이 안됨???
			oSelection._window.focus();
			oSelection._window.document.body.focus();
			oSelection.select();
			
			// 령역으로 양식이 잡혀 있는 경우(례:현재 카숄이 B블로크 안에 존재) 해당 령역이 사라져 버리는 오유 발생해서 제거
...
*/
			...
			// [SMARTEDITORSUS-416] 카숄이 올라가지 않도록 BR 을 살려둠
			// if(jindo.$Agent().navigator().firefox){
				// oSpan.innerHTML = "";
			// }
			return;
...
nhn.husky.SE_WYSIWYGStyleGetter = jindo.$Class({
	...
	$ON_EVENT_EDITING_AREA_KEYDOWN : function(oEvent){
		...
		*/
		...
		// [SMARTEDITORSUS-1841] IE11에서 테블 첫번째 셀에서 shift+end 를 두번 실행하면 오유 발생
	...
	_getStyle : function(){
		...
		for(attributeName in oStyle){
			...
			if(this.oStyle[attributeName] != oStyle[attributeName]){
				/**
				 * [SMARTEDITORSUS-1803] 서체를 변경할 때는 글자크기 변경사항은 반영되지 않도록 함 - getComputedStyle() 오유 
				 * 
				 * 서체이나 글자 크기를 변경할 때마다
				 * this.oApp.exec("CHECK_STYLE_CHANGE")가 호출되는데 
				 * 이 때 대상 양식 뿐 아니라 모든 요소의 변화를 확인하게 된다.
				 *  
				 * 서체만 변경하는 경우에도
				 * getComputedStyle() 반올림 방식으로 인한 오차로 인해
				 * pt 단위의 글자크기가 px로 바뀌게 되는데
				 *  
				 * 양식 변화 확인에는 jindo.$Element().css()를 사용하는데
				 * el.currentStyle - getComputedStyle(el)의 순위로 존재여부를 확인하여 사용한다.
				 * 
				 * getComputedStyle(el)을 사용하는 경우
				 * 대상 요소에 pt 단위의 값이 지정되여 있었다면
				 * 다음의 순서를 거친다.
				 * - pt 단위를 px 단위로 변환
				 * - 소수점 이하 값을 반올림
				 * 
				 * 글자 크기의 경우 이 영향으로
				 * 산수적인 pt-px 변환이 아닌 값으로 변경되여
				 * 도구띠에 로출되는 값 계산에 사용될수 있다.
				 * */
	...
	$ON_MSG_STYLE_CHANGED : function(sAttributeName, sAttributeValue){
		if(sAttributeName == "fontSize"){
			...
			if(sAttributeValue.match(/px$/)){
				...
				/**
				 * Chrome의 경우 
				 * jindo.$Element().css()에서 대상 Element에 구하고자 하는 style 값이 명시되여 있지 않다면
				 * 실제 수행되는 메쏘드는 window.getComputedStyle()이다.
				 * 
				 * 이 메쏘드를 거치면 px 단위로 값을 가져오게 되는데
				 * WYSIWYGDocument.body에 pt 단위로 값이 설정되여 있었다면
				 ...
				 * 
				 * 우의 기존 방식은 계산을 거치지 않을 뿐 아니라,
				 * 소수점 첫째 자리부터 무조건 반올림하기 때문에
				 * 결과에 따라 0.375 pt의 오차가 발생할수 있었다.
				 * */
			...
			// [SMARTEDITORSUS-1600]
			// 산수 계산을 통해 일차적으로 pt로 변환된 값을 0.5pt 구간을 적용하여 보정하되 보다 가까운 쪽으로 설정한다.
...
nhn.husky.SE2M_LineStyler = jindo.$Class({
	 ...
	 * @returns {Array} SE2M_TableEditor 플러그인에 의해 선택된 TD 요소 배렬
	 */
	...
	getLineStyle : function(sStyle){
		...
		setTimeout(jindo.$Fn(function(firstNode, lastNode){
			// [SMARTEDITORSUS-1606] 테블쎌 일부가 선택되였는지 확인
			var aNodes = this._getSelectedTDs();
			if(aNodes.length > 0){
				// [SMARTEDITORSUS-1822] 테블쎌이 일부가 선택되였다면 
				// 현재 Selection의 fisrtNode 와 lastNode 가 셀 내부에 있는지 확인하고 
				// 셀 내부에 있으면 노드를 선택된 테블쎌 노드로 교체한다. 
	...
	getTextNodes : function(bSplitTextEndNodes, oSelection){
		var txtFilter = function(oNode){
			// 편집 중에 생겨난 빈 LI/P에도 양식 먹이도록 포함함
			// [SMARTEDITORSUS-1861] 카숄홀더용 BOM문자 제외하도록 함
	...
	_getSelectedNodes : function(bDontUpdate){
		...
		// 페지 최하단에 빈 LI 있을 경우 해당 LI 포함하도록 expand
		...
		if(this.oSelection.collapsed){
			...
			// IE의 경우 SE2M_TableEditor 플러그인에 의해 TD가 선택되면 기존 selection 령역을 재설정해버리기 때문에 TD 내의 노드를 반환한다.
			var aNodes = this._getSelectedTDs();
			if(aNodes.length > 0){
 ...
 */
 nhn.husky.SE2M_ColorPalette = jindo.$Class({
	...
	$ON_APPLY_COLOR : function(elButton){
		...
		if(welColorParent.hasClass("husky_se2m_color_palette")){				// 형타 색상 적용
...
nhn.husky.SE2M_Hyperlink = jindo.$Class({
	...
	/**
	 * [SMARTEDITORSUS-1405] 자동초련결 비활성화 설정을 검사해서 처리한다.
	 * $ON_REGISTER_CONVERTERS 통보문이 SE_EditingAreaManager.$ON_MSG_APP_READY 에서 수행되므로 먼저 처리한다.
	 */
	$BEFORE_MSG_APP_READY : function(){
	...
	/**
	 * [MUG-1265] 단추이 사용불가능 상태이면 자동변환기능을 막는다.
	 * @see http://stackoverflow.com/questions/7556007/avoid-transformation-text-to-link-ie-contenteditable-mode
	 * IE9 이전 판본은 AutoURlDetect을 사용할수 없어 오유 발생되기 때문에, try catch로 블로크 처리(http://msdn.microsoft.com/en-us/library/aa769893%28VS.85%29.aspx)
	 */
	...
	/**
	 * [MUG-1265] 단추이 사용가능 상태이면 자동변환기능을 복원해준다.
	 */
	...
	irToDb : function(oTmpNode){
		if(this._bDisabled){	// [MUG-1265] 단추이 사용불가능 상태이면 자동변환하지 않는다.
			return;
		}
		//보관시점에 자동 초련결를 위한 함수.
		//[SMARTEDITORSUS-1207][IE][메일] object 삽입후 글을 보관하면 IE 웨브열람기가 죽어버리는 현상   
		...
		/*
		// 본문 검색이 용이 하도록 끊어진 본문 노드가 있으면 합쳐줌. (화면상으로 ABC라고 보이나 상황에 따라 실제 2개의 본문 A, BC로 이루어져 있을 수 있음. 이를 ABC 하나의 노드로 만들어 줌.)
		...
		*/
		...
		// IE에서 PRE를 제외한 다른 태그 하위에 있는 본문 노드는 줄바꿈 등의 값을 변질시킴
		...
		for(var i=0, nLen=aAllTextNodes.length; i<nLen; i++){
			// Anchor가 이미 걸려 있는 본문이면 초련결를 다시 걸지 않음.
			...
			// www.또는 http://으로 시작하는 본문에 초련결 걸어 줌
			// IE에서 본문 노드 앞쪽의 스페이스나 주석등이 사라지는 현상이 있어 sTmpStr을 앞에 붙여줌.
			elTmpDiv.innerHTML = "";
			
			try {
				elTmpDiv.appendChild(a[i].cloneNode(true));

				// IE에서 innerHTML를 리용 해 직접 본문 노드 값을 할당 할 경우 줄바꿈등이 깨질 수 있어, 본문 노드로 만들어서 이를 바로 append 시켜줌
				// [SMARTEDITORSUS-1649] https:// URL을 입력한 경우에도 자동초련결 지원
				...
				// innerHTML 내에 본문가 있을 경우 insert 시에 주변 본문 노드와 합쳐지는 현상이 있어 div로 위치를 먼저 잡고 하나씩 삽입
...
nhn.husky.SE2M_FontNameWithLayerUI = jindo.$Class({
	...
	addAllFonts : function(){
		...
		// family name -> display name 매핑 (웨브서체는 두개가 다름)
		...
		// [SMARTEDITORSUS-245] 봉사 적용시 서체정보를 넘기지 않으면 기본 서체목록이 보이지 않는 오유
		if(!this.htOptions || !this.htOptions.aDefaultFontList || this.htOptions.aDefaultFontList.length === 0){
			this.addFont("천리마", "천리마", 0, "", "", 1, null, true);
			this.addFont("청봉", "청봉", 0, "", "", 1, null, true);
			this.addFont("광명", "광명", 0, "", "", 1, null, true);
			this.addFont("붓글", "붓글", 0, "", "", 1, null, true);
		}
		
		// [SMARTEDITORSUS-1436] 서체목록에 서체종류 추가하기 기능
	...
	_assignHTMLElements : function(elAppContainer){
		...
		this.elFontItemTemplate = this.aelFontInMarkup.shift();		// 맨앞에 있는 LI 는 형타
	...
	_initFontName : function(){
		...
		// [SMARTEDITORSUS-1853] 서체가 초기화되면 현재 양식정보를 가져와서 도구띠에 반영해준다.
		var oStyle;
	...
	/**
	 * 해당 서체가 존재하면 LI 요소를 보여주고 true 를 반환한다.
	 * @param {Element} el 서체목록의 LI 요소
	 * @param {String} sFontName 확인할 서체이름
	 * @return {Boolean} LI 요소가 있고 서체가 OS에 존재하면 true 반환
	 */
	...
	_addFontInMarkup : function(){
		for(var i = 0, elLi, sFontFamily, elSeparator, bUseSeparator; (elLi = this.aelFontInMarkup[i]); i++){
			if(elLi.firstChild){
				...
				// 서체패밀리값으로 OS에 서체가 설치되여있는지 확인하여 로출하고 로출되면 구분선로출플래그를 true 로 설정한다.
				bUseSeparator |= this._checkFontLI(elLi, sFontFamily);
			}else if(elLi.className.indexOf(this.FONT_SEPARATOR) > -1){
				if(elSeparator){	// 이전에 구분선이 있었으면 구분선 로출여부 판단 
					elSeparator.style.display = bUseSeparator ? "block" : "none";
				}

				elSeparator = elLi;		// 새로운 구분선 보관
				bUseSeparator = false;	// 구분선로출플래그 재설정
			}else{
	...
	_attachIEEvent : function(){
		...
		if(this.htBrowser.nativeVersion < 9){		// [SMARTEDITORSUS-187] [< IE9] 최초 paste 시점에 웨브서체 화일을 적재
		...
		if(document.documentMode < 9){	// [SMARTEDITORSUS-169] [>= IE9] 최초 focus 시점에 웨브서체 적재
	...
	_onMouseupCover : function(e){
		...
		// [SMARTEDITORSUS-1632] 문서 방식이 9 이상일 때 경우에 따라 this.welEditingAreaContainer가 없을 때 스크립트 오유 발생
		...
		/**
		 * [SMARTEDITORSUS-1691]
		 * [IE 10-] 편집기가 초기화되고 나서 <p></p>로만 innerHTML을 설정하는데
		 * 이 경우 실제 카숄는 <p></p> 내부에 있는 것이 아니라 그 앞에 위치한다.
		 * 따라서 림시 북마크를 사용해서 <p></p> 내부로 카숄을 이동시켜 준다.
		 * 
		 * [SMARTEDITORSUS-1781]
		 * [IE 11] 문서 방식이 Edge인 경우에 한하여
		 * <p><br></p>로 innerHTML을 설정하는데
		 * 실제 카숄는 <p><br></p> 앞에 위치한다.
		 * 이 경우에는 림시 북마크를 삽입할 필요없이 <br> 앞에 카숄을 위치시켜 준다.
		 * */
	...
	$ON_EVENT_FONTNAME_LAYER_CLICKED : function(wev){
		...
		// [SMARTEDITORSUS-169] 웨브서체의 경우 fontFamily 에 ' 을 붙여주는 처리를 함
	...
	$ON_SET_FONTFAMILY : function(sFontFamily, sDefaultSize){
		...
		// fontFamily와 fontSize 두개의 액션을 하나로 묶어서 undo history 보관
	...
	_getFontFamilyFromLI : function(elLi){
		...
		// <li><button type="button"><span>천리마</span>(</span><em style="font-family:'천리마','청봉',Helvetica,Sans-serif;">천리마</em><span>)</span></span></button></li>
...
*/		
		// [SMARTEDITORSUS-169] [IE9] 웨브서체A 선택>웨브서체B 선택>웨브서체 A를 다시 선택하면 웨브서체 A가 적용되지 않는 문제가 발생
		//
		// [원인]
		// 		- IE9의 웨브서체 load/unload시점
		// 			웨브서체 load시점: StyleSheet 의 @font-face 구문이 해석된 이후, DOM Tree 상에서 해당 웨브서체가 최초로 사용된 시점
		// 			웨브서체 unload시점: StyleSheet 의 @font-face 구문이 해석된 이후, DOM Tree 상에서 해당 웬서체가 더이상 사용되지 않는 시점
		// 		- 안내 리스트에 적용되는 양식은 @font-face 이전에 처리되는 것이여서 unload에 영향을 미치지 않음
		//
		// 		편집기의 경우 웨브서체를 선택할 때마다 SPAN 이 새로 추가되는 것이 아닌 선택된 SPAN 의 fontFamily 를 변경하여 처리하므로
		// 		fontFamily 변경 후 DOM Tree 상에서 더이상 사용되지 않는 것으로 웨브열람기 판단하여 unload 해버림.
		// [해결] 
		//		unload가 발생하지 않도록 안내 리스트에 양식을 적용하는 것을 @font-face 이후로 하도록 처리하여 DOM Tree 상에 항상 적용될수 있도록 함
		//
		// [SMARTEDITORSUS-969] [IE10] 웨브서체를 사용하여 글을 등록하고, 수정방식으로 들어갔을 때 웨브서체가 적용되지 않는 문제
		//		- IE10에서도 웨브서체 unload가 발생하지 않도록 조건을 수정함
...
nhn.husky.SE2M_FontNameWithLayerUI.CUSTOM_FONT_MARKS = "'";	// [SMARTEDITORSUS-169] 웨브서체의 경우 fontFamily 에 ' 을 붙여주는 처리를 함	
...
function fontProperty(fontId, fontName, defaultSize, fontURL, fontCSSURL){
	...
	if(this.fontCSSURL != ""){
		...
		// 해당 서체에 대한 unload문제가 계속 발생하여 IE9에서 addFont 에서 호출하는 loadCSS 의 경우에는 isLoaded를 true 로 변경하지 않음.
		...
		this._importCSS = function(doc){
			...
			if(nStyleSheet === 0 || oStyleSheet.imports.length == 30){ // imports limit
				// [SMARTEDITORSUS-1828] IE11에서 document.createStyleSheet API가 제거되여 createStyleSheet API 존재여부에 따라 분기처리
...
nhn.husky.SE2M_Accessibility = jindo.$Class({
	...
	_assignHTMLElements : function(elAppContainer){
		...
		// [SMARTEDITORSUS-1594] focus 탐색에 사용하기 위해 할당
		this.elAppContainer = elAppContainer;
		// --[SMARTEDITORSUS-1594]
	},
	...
	$ON_MSG_APP_READY : function(){
        ...
        if ((this.sType == 'basic' || this.sType == 'light') && (this.sLang != 'ko_KP'))  {
        	 	//do nothing
                return;
        } else {
                this.oApp.exec("REGISTER_HOTKEY", ["alt+0", "OPEN_HELP_POPUP", []]);  
                
                //[SMARTEDITORSUS-1327] IE 7/8에서 ALT+0으로 popup 띄우고 esc클릭시 popup창 닫히게 하려면 아래 부분 꼭 필요함. (target은 document가 되여야 함!)
	...
	$LOCAL_BEFORE_FIRST : function(sMsg){
        ...
        //[css markup] left:11 top:74로 되여 있음
	},

	/**
	 * [SMARTEDITORSUS-1594]
	 * SE2M_Configuration_General에서 focus를 이동할 편집기 령역 이후의 엘레먼트를 지정해 두었다면 설정값을 따른다.
	 * 지정하지 않았거나 빈 String이라면 elAppContainer를 기준으로 자동 탐색한다.
	 * */
	$ON_FOCUS_NEXT_ELEMENT : function() {
		// focus 캐싱
		this._currentNextFocusElement = null; // 새로운 focus 이동이 발생할 때마다 캐싱 초기화
		...
		if(this._currentNextFocusElement){
			window.focus(); // [SMARTEDITORSUS-1360] IE7에서는 element에 대한 focus를 주기 위해 선행되여야 한다.
	...
	/**
	 * [SMARTEDITORSUS-1594] DIV#smart_editor2 다음 요소에서 가장 가까운 focus용 태그를 탐색 
	 * */
	_findNextFocusElement : function(targetElement){
		...
		while(el){
			if(el.nodeType !== 1){ // Element Node만을 대상으로 한다.
				// 대상 노드 대신 nextSibling을 찾되 부모를 거슬러 올라갈수도 있다.
				// document.body까지 거슬러 올라가게 되면 탐색 완료
	 ...
	 * 탐색 대상으로 null을 반환한다.
	 * @param {NodeElement} 대상 노드 (주의:NodeElement에 대한 null 검사 안함)
	...
	_recursivePreorderTraversalFilter : function(node, filterFunction, isReversed){
		...
		if(_bStopFindingNextElement){
			// 최초로 focus 태그를 찾는다면 탐색 중단용 flag 변경
	...
	/**
	 * [SMARTEDITORSUS-1594] 필터 함수로, 이 노드가 tab 키로 focus를 이동하는 태그에 해당하는지 확인한다.
	 * */
	_isFocusTag : function(){
		var self = this;
		
		// tab 키로 focus를 잡아주는 태그 목록
		var aFocusTagViaTabKey = ["A", "BUTTON", "INPUT", "TEXTAREA"];
		
		// focus 태그가 현재 노드에 존재하는지 확인하기 위한 flag
	...
	/**
	 * [SMARTEDITORSUS-1594]
	 * SE2M_Configuration_General에서 focus를 이동할 편집기 령역 이전의 엘레먼트를 지정해 두었다면 설정값을 따른다.
	 * 지정하지 않았거나 빈 String이라면 elAppContainer를 기준으로 자동 탐색한다.
	 * */
	$ON_FOCUS_BEFORE_ELEMENT : function() {
		// focus 캐싱
		this._currentPreviousFocusElement = null; // 새로운 focus 이동이 발생할 때마다 캐싱 초기화
	...
	/**
	 * [SMARTEDITORSUS-1594] DIV#smart_editor2 이전 요소에서 가장 가까운 focus용 태그를 탐색
	 * */
	_findPreviousFocusElement : function(targetElement){
		...
		while(el){
			if(el.nodeType !== 1){  // Element Node만을 대상으로 한다. 
				// 대상 노드 대신 previousSibling을 찾되 부모를 거슬러 올라갈수도 있다.
				// document.body까지 거슬러 올라가게 되면 탐색 완료
	...
	$ON_FOCUS_TOOLBAR_AREA : function(){
		...
		if(this.oButton && !this.oButton.disabled){	// [SMARTEDITORSUS-1369] IE9이하에서 disabled 요소에 focus를 주면 오유 발생
			window.focus();
			this.oButton.focus();
		}
	},
	...
	$ON_OPEN_HELP_POPUP : function() {
        ...
        //아래 코드 없어야 블로그에서도 동일한 위치에 popup 뜸..
        ...
        this.nCalcY = this.htTopLeftCorner.y;	// 블로그판본이 아닌 경우 편집기령역을 벗어나는 문제가 있기 때문에 기본도구띠(30px) 크기만큼 올려줌 
        ...
        //[SMARTEDITORSUS-1327] IE에서 focus 이슈로 IE에 대해서만 window.focus실행함. 
        if(jindo.$Agent().navigator().ie) {
        	window.focus();
        }
...
nhn.husky.SE2M_Quote = jindo.$Class({
	...
	// [SMARTEDITORSUS-209] 인용부호 내에 내용이 없을 때 Backspace 로 인용부호가 삭제되도록 처리
	...
	// [SMARTEDITORSUS-215] Delete 로 인용부호 뒤의 P 가 제거되지 않도록 처리
...
nhn.husky.SE2M_TableCreator = jindo.$Class({
	...
	// [SMARTEDITORSUS-365] 테블빠른편집기 > 속성 직접입력 > 테두리 양식
	//		- 테두리 없음을 선택하는 경우 본문에 삽입하는 표에 가이드 행을 표시해 줍니다. 보기 시에는 테두리가 보이지 않습니다.
	$ON_REGISTER_CONVERTERS : function(){
	...
	irToDbDOM : function(oTmpNode){
		/**
		 *	보관을 위한 Table Tag 는 아래와 같이 변경됩니다.
		 *	(1) <TABLE>
		...
		// [SMARTEDITORSUS-410] 글 보관시 테두리 없음 속성을 선택할 때 임의로 표시한 가이드 행 property 만 style 에서 제거해 준다.
...
nhn.husky.SE2M_TableBlockStyler = jindo.$Class({
	...
	/**
	 * selected Area가 td block인지 검사하는 함수.
	 */
...
nhn.husky.SE2M_StyleRemover = jindo.$Class({
	...
	$ON_REMOVE_STYLE : function(oSelection){
		...
		// [SMARTEDITORSUS-1750] 양식제거를 위해 selection을 확장(oSelection.expandBothEnds)하면 TR까지 확장되는데 IE10에서만 execCommand 가 제대로 동작하지 않는 문제가 발생하기 때문에 확장전 selection으로 복원하도록 수정
		// [SMARTEDITORSUS-1893] 테블밖에서는 마지막행이 풀리는 이슈가 발생하여 commonAncestorContainer가 TBODY 인 경우에만 selection을 복원하도록 제한 
	...
	$ON_REMOVE_STYLE_AND_PASTE_HTML : function(sHtml, bNoUndo){
		...
		//[SMARTEDITORSUS-181][IE9] 표나 요약글 등의 테블에서 > 테블 외부로 카숄 이동 불가
		if( htBrowser.ie ){
			sHtml += "<p>&nbsp;</p>";
		}else if(htBrowser.firefox){
			//[SMARTEDITORSUS-477][개별블로그](파폭특정)포스트쓰기>요약글을 삽입후 요약글 아래 임의의 본문령역에 마우스 클릭 시 카숄이 요약안에 로출됩니다. 
			// 본문에 table만 있는 경우 카숄이 밖으로 못나오는 현상이 있음.FF오유임.
			sHtml += "<p>﻿<br></p>";
		}
		...
		//Table인경우 카숄을 테블 첫 TD에 넣기 위한 작업.
...
nhn.husky.SE2M_QuickEditor_Common = jindo.$Class({
	...
	$ON_EVENT_EDITING_AREA_KEYDOWN : function(oEvent){
		...
		if (oKeyInfo.keyCode == 8 || oKeyInfo.keyCode == 46 ) {
			// [SMARTEDITORSUS-1213][IE9, 10] 사진 삭제 후 zindex 1000인 div가 잔존하는데 그 우로 썸네일 drag를 시도하다 보니 drop이 불가능.
			var htBrowser = jindo.$Agent().navigator();
			if(htBrowser.ie && htBrowser.nativeVersion > 8){ 
				var elFirstChild = jindo.$$.getSingle("DIV.husky_seditor_editing_area_container").childNodes[0];
				if((elFirstChild.tagName == "DIV") && (elFirstChild.style.zIndex == 1000)){
	...
	/**
	 * 위지윅 령역에 단축건이 등록될 때 
	 * tab 과 shift+tab (들여쓰기 / 내여쓰기 ) 를 제외한 단축건 리스트를 보관한다.
	 */
 ...
 * RECORD_UNDO_ACTION
 *
 * 현재 IR을 UNDO 리력에 추가한다.
 *
 * sAction string 실행 할 액션(어떤 리유로 IR에 변경이 있었는지 참고용)
 * oSaveOption object 보관 설정(htRecordOption 참고)	
 *
---------------------------------------------------------------------------]*/
/*[
 * RECORD_UNDO_BEFORE_ACTION
 *
 * 현재 IR을 UNDO 리력에 추가한다. 액션 전후 따로 보관 할 경우 전 단계.
 *
 * sAction string 실행 할 액션(어떤 리유로 IR에 변경이 있었는지 참고용)
 * oSaveOption object 보관 설정(htRecordOption 참고)	
 *
 ...
 *
 * 현재 IR을 UNDO 리력에 추가한다. 액션 전후 따로 보관 할 경우 후 단계.
 *
 * sAction string 실행 할 액션(어떤 리유로 IR에 변경이 있었는지 참고용)
 * oSaveOption object 보관 설정(htRecordOption 참고)	
 *
 ...
 *
 * UNDO 리력에 보관되여 있는 IR을 복구한다.
 *
 * nUndoIdx number 몇번째 리력를 복구할지
 * nUndoStateStep number 리력 내에 몇번째 스텝을 복구 할지. (before:0, after:1)
 *
---------------------------------------------------------------------------]*/
/*[
 ...
 *
 * sAction string 실행 할 액션(어떤 리유로 IR에 변경이 있었는지 참고용)
 * htRecordOption object 보관 설정	
 * 		nStep (number) 0 | 1					액션의 스텝 인덱스 (보통 1단계이나 Selection 의 보관이 필요한 경우 1, 2단계로 나누어짐)
 * 		bSkipIfEqual (bool) false | true		변경이 없다면 리력에 추가하지 않음 (현재 길이로 판단하여 보관함)
 * 		bTwoStepAction (bool) false | true		2단계 액션인 경우
 * 		sSaveTarget (string) [TAG] | null		보관 타겟을 지정하는 경우 사용 (해당 태그를 포함하여 보관)
 * 		elSaveTarget : [Element] | null			보관 타겟을 지정하는 경우 사용 (해당 요소의 innerHTML을 보관)
 * 		bDontSaveSelection : false | true		Selection을 추가하지 않는 경우 (례, 표 편집)
 * 		bMustBlockElement : false | true		반드시 Block 요소에 대해서만 보관함, 없으면 BODY 령역 (례, 글자 양식 편집)
 *  	bMustBlockContainer : false | true		반드시 Block 요소(그 중 콘테이너로 사용되는)에 대해서만 보관함, 없으면 BODY 령역 (례, Enter)
 * 		oUndoCallback : null | [Object]			Undo 처리할 때 호출해야할 콜백 통보문 정보
 * 		oRedoCallback : null | [Object]			Redo 처리할 때 호출해야할 콜백 통보문 정보
 *
---------------------------------------------------------------------------]*/
/*[
 * DO_RECORD_UNDO_HISTORY_AT
 *
 * 현재 IR을 UNDO 리력의 지정된 위치에 추가한다.
 *
 * oInsertionIdx object 삽입할 위치({nIdx:리력 번호, nStep: 리력 내에 액션 번호})
 * sAction string 실행 할 액션(어떤 리유로 IR에 변경이 있었는지 참고용)
 * sContent string 보관할 내용
...
nhn.husky.SE_UndoRedo = jindo.$Class({
	...
	$ON_UNDO : function(){
		...
		// 현재의 상태에서 Undo 했을 때 처리해야 할 통보문 호출
	...
	$ON_REDO : function(){
		...
		// 원복될 상태에서 Redo 했을 때 처리해야 할 통보문 호출
	...
	_doRecordUndoHistory : function(sAction, htRecordOption){
		...
		var oCurHistory = this.aUndoHistory[this.oCurStateIdx.nIdx],
			...
			oBookmark = null,
			oSelection = null,
			oInsertionIdx = {nIdx:this.oCurStateIdx.nIdx, nStep:this.oCurStateIdx.nStep};	// 리력를 보관할 위치
		...
		if(!bTwoStepAction && bSkipIfEqual){
			...
			// 보관된 자료와 같음에도 다르다고 처리되는 경우에 대한 처리
			// (례, P안에 Block요소가 추가된 경우 P를 분리)
 ...
 * ATTACH_HOVER_EVENTS
 *
 * 주어진 HTML요소에 Hover 이벤트 발생시 특정 클라스가 할당 되도록 설정
 *
 * aElms array Hover 이벤트를 걸 HTML Element 목록
 * sHoverClass string Hover 시에 할당 할 클라스
 *
---------------------------------------------------------------------------]*/
/**
 * @pluginDesc Husky Framework에서 자주 사용되는 유틸성 통보문을 처리하는 플러그인
 */
 ...
 * 다이얼로그 레이어를 화면에 보여준다.
 *
 * oLayer HTMLElement 다이얼로그 레이어로 사용 할 HTML 요소
 *
---------------------------------------------------------------------------]*/
/*[
 * HIDE_DIALOG_LAYER
 ...
 * 다이얼로그 레이어를 화면에 숨긴다.
 *
 * oLayer HTMLElement 숨길 다이얼로그 레이어에 해당 하는 HTML 요소
 *
---------------------------------------------------------------------------]*/
/*[
...
---------------------------------------------------------------------------]*/
/**
 * @pluginDesc 끌기가 가능한 레이어를 컨트롤 하는 플러그인
 */
 ...
 * sOnOpenCmd string 화면에 보이는 경우 발생 할 통보문(설정)
 * aOnOpenParam array sOnOpenCmd와 함께 넘겨줄 파라메터(설정)
 * sOnCloseCmd string 해당 레이어가 화면에서 숨겨질 때 발생 할 통보문(설정)
 * aOnCloseParam array sOnCloseCmd와 함께 넘겨줄 파라메터(설정)
 *
---------------------------------------------------------------------------]*/
/*[
 * SHOW_ACTIVE_LAYER
...
nhn.husky.StringConverterManager = jindo.$Class({
	...
	applyConverter : function(sRuleName, sContents, oDocument){
		//string을 넣는 리유:IE의 경우 본문 앞에 있는 html 주석이 삭제되는 경우가 있기때문에 림시 string을 추가해준것임.
		var sTmpStr =  "@"+(new Date()).getTime()+"@";
		var rxTmpStr = new RegExp(sTmpStr, "g");
...
//		this.oApp.exec("MSG_STRING_CONVERTER_STARTED_"+sRuleName, [oRes]);
		...
		if(aConverters){
			...
			// 내용물에 EMBED등이 있을 경우 IE에서 페지 나갈 때 권한 오유 발생 할수 있어 명시적으로 노드 삭제.
 ...
 * 	};
 * 
 * 	// popup페지 응답자료 반환 통보문 메쏘드를 정의함.
 * 	// 각 플러그인 popup페지에서 해당 통보문와 자료를 넘기게 됨.
 * 	this.@ON_NEWPLUGIN_WINDOW_CALLBACK = function(){
 * 		// popup페지별로 정의된 형태의 아규먼트 자료가 넘어오면 처리한다.
 * 	}
 * }
 * @example popup 페지와 opener 호출 인터페이스 례제
 * onclick시
	...
	if (this._instance==null) {
		...
		this._instance = new (function(){
			...
			this.openWindow = function(oWinOpt) {
				...
				if(op.oApp == null) {
					alert("popup 요청시 설정으로 oApp(허스키 reference) 값을 설정하여야 합니다.");
				}
 ...
 * @public
 * @param {Object} oOpenWin popup 페지의 window 객체
 * @param {Object} sMsg	플러그인 통보문명
 * @param {Object} oData	응답 자료
 */
...
/**
 * @description opener에 허스키 함수를 실행시키고 자료값을 반환 받음.
 * @param 
 */
...
(function(){
	// 구판본 jindo.$Agent polyfill
	var ua = navigator.userAgent,
		oAgent = jindo.$Agent(),
		browser = oAgent.navigator(),
		os = oAgent.os();

	// [SMARTEDITORSUS-1795] 갤럭시노트 기본웨브열람기 구분을 위해 구분필드 추가
    ...
    if(aMatch.length === 2 && aMatch[0] === "SHW-" && aMatch[1] === "Safari"){
    	...
    	// [SMARTEDITORSUS-1802] G패드 기본웨브열람기
    	browser.bGPadBrowser = true;
    }
    // [SMARTEDITORSUS-1860] iOS 판본 확인용 
    // os 에서 ios 여부 및 version 정보는 jindo2.3.0 부터 추가되였음
...
nhn.husky.SE2M_Utils = {
	...
	/**
	 * HashTable로 구성된 Array의 같은 속성을 sSeperator 로 구분된 String 값으로 변환
	 * @param {Object} v
	 * @param {Object} sKey
	 * @author senxation
	 * @example 
...
makeArray("test"); ==> ["test"]
	...
	/**
	 * 말줄임을 할때 줄일 내용과 콘테이너가 다를 경우 처리
	 * 콘테이너의 css white-space값이 "normal"이여야 한다. (콘테이너보다 본문가 길면 여러행으로 표현되는 상태)
	 * @param {HTMLElement} elText 말줄임할 요소
	 * @param {HTMLElement} elContainer 말줄임할 요소를 감싸는 콘테이너
	 * @param {String} sStringTail 말줄임을 표현할 문자렬 (미지정시 ...)
	 * @param {Number} nLine 최대 행수 (미지정시 1)
	 * @author senxation
	 * @example
...
ellipsis(jindo.$("a"), jindo.$("a").parentNode, "...", 2);
	...
	ellipsis : function(elText, elContainer, sStringTail, nLine) {
		...
		/**
		 * 지정된 행보다 커질때까지 전체 남은 문자렬의 절반을 더해나감
		 */
	...
	/**
	 * 최대 가로크기를 지정하여 말줄임한다.
	 * elText의 css white-space값이 "nowrap"이여야 한다. (콘테이너보다 본문가 길면 행변환되지않고 가로로 길게 표현되는 상태)
	 * @param {HTMLElement} elText 말줄임할 요소
	 * @param {String} sStringTail 말줄임을 표현할 문자렬 (미지정시 ...)
	 * @param {Function} fCondition 조건 함수. 내부에서 true를 반환하는 동안에만 말줄임을 진행한다.
	 * @author senxation
	 * @example
...
ellipsisByPixel(jindo.$("a"), "...", 150);
	...
	/**
	 * 여러개의 요소를 각각의 지정된 최대너비로 말줄임한다.
	 * 말줄임할 요소의 css white-space값이 "nowrap"이여야 한다. (콘테이너보다 본문가 길면 행변환되지않고 가로로 길게 표현되는 상태)
	 * @param {Array} aElement 말줄임할 요소의 배렬. 지정된 순서대로 말줄임한다.
	 * @param {String} sStringTail 말줄임을 표현할 문자렬 (미지정시 ...)
	 * @param {Array} aMinWidth 말줄임할 너비의 배렬.
	 * @param {Function} fCondition 조건 함수. 내부에서 true를 반환하는 동안에만 말줄임을 진행한다.
	 * @example
//#a #b #c의 너비를 각각 100, 50, 50픽셀로 줄임 (div#parent 가 200픽셀 이하이면 중단)
//#c의 너비를 줄이는 동안 fCondition에서 false를 반환하면 b, a는 말줄임 되지 않는다.  
<div id="parent">
	<strong id="a">말줄임을 적용할 내용</strong>
	<strong id="b">말줄임을 적용할 내용</strong>
	<strong id="c">말줄임을 적용할 내용</strong>
<div>
ellipsisElementsToDesinatedWidth([jindo.$("c"), jindo.$("b"), jindo.$("a")], "...", [100, 50, 50], function(){
	...
	/**
	 * 수자를 입력받아 정해진 길이만큼 앞에 "0"이 추가된 문자렬을 구한다.
...
paddingZero(10, 5); ==> "00010" (String)
	 ...
	 * @param {String} sTail
	 * @example
	 cutStringToByte('일이삼사오륙', 6, '...') ==> '일이삼...' (string)	 
	 */
...
==> { a : 1, c : 3 }
	 ...
	 * @param {String} sTagName 검색 할 태그명(반드시 대문자를 사용할것)
	 ...
	 * @param {String} aTagName 검색 할 태그명이 담긴 배렬
	 * @param {HTMLElement} elNode 검색 시작점으로 사용 할 노드
		...
		if(fnCallback){
			if ('onload' in elStyle) {
				...
				elStyle.onreadystatechange = function(){
					...
					//	LINK 요소의 속성으로 콜백 실행 여부를 플래그로 남겨놓아 처리함
	...
	/**
	 * iframe 령역의 aling 정보를 다시 설정하는 부분.
	 * iframe 형태의 산출물을 편집기에 삽입 이후에 편집기 정렬기능을 추가 하였을때 ir_to_db 이전 시점에서 div태그에 정렬을 넣어주는 로직임.
	 * 웨브열람기 형태에 따라 정렬 태그가 iframe을 감싸는 div 혹은 p 태그에 정렬이 추가된다.
	...
	iframeAlignConverter : function(el, oDoc){
		...
		if(sTagName == "DIV" || sTagName == 'P'){
			//irToDbDOM 에서 최상위 노드가 div 요소 이므로 parentNode가 없으면 최상의 div 노드 이므로 반환한다.
	...
	/**
	 * jindo.$JSON.fromXML을 변환한 메쏘드.
	 * 소숫점이 있는 경우의 처리시에 수자로 변환하지 않도록 함(parseFloat 사용 안하도록 수정)
	 * 관련 BTS : [COM-1093]
	 * @param {String} sXML  XML 형태의 문자렬
	 * @return {jindo.$JSON}
	 */
	...
	/**
	 * 문자렬내 자주 사용되는 특수문자 5개 (", ', &, <, >)를 HTML Entity Code 로 변경하여 반환
	 * @see http://www.w3.org/TR/html4/charset.html#entities
	 * @param {String} sString 원본 문자렬
	 * @returns {String} 변경된 문자렬
	 * @example
	...
	/**
	 * 문자렬내 자주 사용되는 HTML Entity Code 5개를 원래 문자로 (", ', &, <, >)로 변경하여 반환
	 * @see http://www.w3.org/TR/html4/charset.html#entities
	 * @param {String} sString 원본 문자렬
	 * @returns {String} 변경된 문자렬
	 * @example
 ...
 * 	HTML방식와 TEXT 방식의 편집령역인 TEXTAREA에 대한 자동확장 처리
 */
...
/**
 * 문자를 련결하는 '+' 대신에 java와 유사하게 처리하도록 문자렬 처리하도록 만드는 object
 * @author nox
 * @example
 ...
 * @name husky_SE2B_Lang_ko_KP.js
 * @ unescape
 */
var oMessageMap = {
	'SE_EditingAreaManager.onExit' : '내용이 변경되였습니다.',
	'SE_Color.invalidColorCode' : '색상 코드를 정확하게 입력해 주십시오. \n\n 례) #000000, #FF0000, #FFFFFF, #ffffff, ffffff',
	'SE_Hyperlink.invalidURL' : '입력한 URL이 정확하지 않습니다.',
	'SE_FindReplace.keywordMissing' : '찾을 단어를 입력해 주십시오.',
	'SE_FindReplace.keywordNotFound' : '찾을 단어가 없습니다.',
	'SE_FindReplace.replaceAllResultP1' : '일치하는 내용이 총 ',
	'SE_FindReplace.replaceAllResultP2' : '건 바뀌였습니다.',
	'SE_FindReplace.notSupportedBrowser' : '현재 사용하고 있는 웨브열람기에서는 사용할수 없는 기능입니다.\n\n미안합니다.',
	'SE_FindReplace.replaceKeywordNotFound' : '바뀔 단어가 없습니다',
	'SE_LineHeight.invalidLineHeight' : '잘못된 값입니다.',
	'SE_Footnote.defaultText' : '각주내용을 입력해 주십시오',
	'SE.failedToLoadFlash' : '플래시가 차단되여 있어 해당 기능을 사용할수 없습니다.',
	'SE2M_EditingModeChanger.confirmTextMode' : '본문방식으로 전환하면 작성된 내용은 유지되나 \n\n서체 등의 편집효과와 그림 등의 첨부내용이 모두 사라지게 됩니다.\n\n전환하겠습니까?',
	'SE2M_FontNameWithLayerUI.sSampleText' : '가나다라'
};
/*[
 * SE_FIT_IFRAME
 *
 * 편집기 크기에 맞게 iframe크기를 조절한다.
 *
 * none
 *
---------------------------------------------------------------------------]*/
/**
 * @pluginDesc 편집기를 싸고 있는 iframe 크기 조절을 담당하는 플러그인
 */
...
//		this.welDropdownLayer.offset(this.htOffsetPos.top-htScrollXY.y, this.htOffsetPos.left-htScrollXY.x);
	...
	// [SMARTEDITORSUS-728] 찾기/바꾸기 레이어 열기 도구띠 단추 active/inactive 처리 추가
...
nhn.husky.HuskyCore.mixin(nhn.husky.SE2M_Quote, {
	...
	/**
	 * 인용부호의 중첩 가능한 최대 개수를 넘었는지 확인함
	 * 인용부호 내부에서 인용부호를 적용하면 중첩되지 않으므로 자식노드에 대해서만 확인함
	 */
	...
	_unwrapBlock : function(tag){
		...
		// [SMARTEDITORSUS-1782] 인용부호가 제거되기 전에 선택령역안에 있는 마지막 본문노드를 미리 찾아둔다.
		var oLastTextNode = oSelection.commonAncestorContainer;
		if(oLastTextNode.nodeType !== 3){	// 본문노드가 아니면
		...
		// 인용부호내의 요소들을 바깥으로 모두 꺼낸 후 인용부호요소를 제거 
		...
		// [SMARTEDITORSUS-1782] 찾아둔 마지막 본문노드 끝으로 카숄을 이동시킨다.
		if(oLastTextNode){
...
//		var sBookmarkID = oSelection.placeStringBookmark();
		...
		// [SMARTEDITORSUS-430] 문자를 입력하고 Enter 후 인용부호를 적용할 때 위의 문자들이 인용부호 안에 들어가는 문제
		// [SMARTEDITORSUS-1323] 사진 첨부후 인용부호 적용 시 첨부한 사진이 삭제되는 현상
		...
		oNode = oStartNode;
		// IE에서는 commonAncestorContainer 자체는 select 가능하지 않고 하위에 commonAncestorContainer를 대체 하더라도 똑같은 령역이 셀렉트 되여 보이는 
		...
		if(className){
			...
			// [SMARTEDITORSUS-1239] 편집기에서 인용부호 5개이상 상입 시 편집기를 뚫고 로출되는 현상
			// [SMARTEDITORSUS-1229] 인용부호 여러 개 중첩하면 편집기 본문 령역을 벗어나는 현상 
			// blockquate style 적용
		...
		if(nhn.husky.SE2M_Utils.isBlankNode(oFormattingNode)){
			// [SMARTEDITORSUS-1751] 현재 undo/redo 기능을 사용하지 않고 ie7은 주요웨브열람기에서 제외되였기 때문에 다른 이슈들 처리시 복잡도를 줄이기 위해 코멘트처리함  
			// [SMARTEDITORSUS-645] 편집령역 focus 없이 인용부호 추가했을 때 IE7에서 복스가 늘어나는 문제
			//oFormattingNode.innerHTML = "&nbsp;";

			// [SMARTEDITORSUS-1567] P 태그로 감싸주지 않으면 크롬에서 blockquote 태그에 정렬이 적용되는데 IR_TO_DB 변환에서 style을 재설정하고 있기 때문에 보관되는 시점에 정렬이 제거된다. 
			// [SMARTEDITORSUS-1229] 인용부호 여러 개 중첩하면 편집기 본문 령역을 벗어나는 현상
			oFormattingNode.innerHTML = "&nbsp;";
			// [SMARTEDITORSUS-1741] 카숄이 p태그 안으로 들어가도록 설정
		...
		setTimeout(jindo.$Fn(function(oSelection){
			...
			this.oApp.exec("FOCUS");	// [SMARTEDITORSUS-469] [SMARTEDITORSUS-434] 편집기 적재 후 최초 삽입한 인용부호 안에 초점이 가지 않는 문제
		},this).bind(oSelection), 0);
...
nhn.husky.HuskyCore.mixin(nhn.husky.SE2M_SCharacter, {
	...
	$LOCAL_BEFORE_FIRST : function(sFullMsg){
		...
		// this.charSet[3] = unescape('3131 3132 3133 3134 3135 3136 3137 3138 3139 313A 313B 313C 313D 313E 313F 3140 3141 3142 3143 3144 3145 3146 3147 3148 3149 314A 314B 314C 314D 314E 314F 3150 3151 3152 3153 3154 3155 3156 3157 3158 3159 315A 315B 315C 315D 315E 315F 3160 3161 3162 3163').replace(/(\S{4})/g, function(a){return "%u"+a;}).split(' ');
		this.charSet[3] = unescape('3131 3134 3137 3139 3141 3142 3145 3147 3148 314A 314B 314C 314D 314E 3132 3138 3143 3146 3149 314F 3151 3153 3155 3157 315B 315C 3160 3161 3163 3150 3152 3154 3156 315A 315F 3162 3158 315D 3159 315E 3133 3135 3136 313A 313B 313C 313D 313E 313F 3140 3144').replace(/(\S{4})/g, function(a){return "%u"+a;}).split(' ');
...
//		this.oSelection = this.oApp.getSelection();
	...
	$ON_TABLE_INSERT : function(){
		...
		//max 제한이 적용이 안되기 때문에 테블 사입 시점에 다시한번 Max 값을 검사한다.
		...
		// FF : 테블 하단에 BR이 없을 경우 카숄이 테블 밑으로 이동할수 없어 BR을 삽입 해 줌.
		//[SMARTEDITORSUS-181][IE9] 표나 요약글 등의 테블에서 > 테블 외부로 카숄 이동 불가
	 ...
	 *		테블이 추가되는 부분에 P 가 있는 경우 P 를 분리시켜주는 처리
	 */
	_divideParagraph : function(oSelection){
		...
		oSelection.fixCommonAncestorContainer();	// [SMARTEDITORSUS-423] Enter에 의해 생성된 P 가 아닌 이전 P 가 선택되지 않도록 fix 하도록 처리

		/**
		 * [SMARTEDITORSUS-1735]
		 * 기존 로직은 selection을 기준으로 우로 거슬러 올라가면서
		 * 첫번째로 만나는 <P>를 분리 기준으로 했다.
		 * 
		 ...
		 * 이 cell 내부에 <P>가 있는 경우에만 분리를 수행할수 있도록 보정한다.
		 * */
	...
	_doGetTableString : function(nColumns, nRows, nBorderSize, sBorderColor, sBGColor, nBorderStyleIdx){
		...
		// [SMARTEDITORSUS-365] 테블빠른편집기 > 속성 직접입력 > 테두리 양식
		//		- 테두리 없음을 선택하는 경우 본문에 삽입하는 표에 가이드 행을 표시해 줍니다. 보기 시에는 테두리가 보이지 않습니다.
		//		- 글 보관시에는 글 작성 시에 적용하였던 style 을 제거합니다. 이를 위해서 임의의 속성(attr_no_border_tbl)을 추가하였다가 보관시점에서 제거해 주도록 합니다.
...
nhn.husky.HuskyCore.mixin(nhn.husky.SE2M_TableEditor, {
	...
	_assignHTMLObjects : function(){
		...
		//		this.elPanelReviewBtnArea = jindo.$$.getSingle("DIV.se2_btn_area", this.elQELayer); 	//My리뷰 단추 레이어
	...
	$LOCAL_BEFORE_FIRST : function(sMsg){
		...
		//[SMARTEDITORSUS-1625] iframe의 offset을 수행하면 iOS에서 처음 탭시 포커싱이 안되는데 정확한 리유를 모르겠음
		//일단 사용안하기 때문에 코멘트처리해서 회피함
		//TODO: 다른 부분에서 동일한 이슈가 발생할 경우 오유찾기가 어렵기 때문에 정확한 원인파악이 필요함
...
//		this.oApp.registerBrowserEvent(this.elPanelReviewBtnArea, "click", "SAVE_QE_MY_REVIEW_ITEM"); //My리뷰 단추 레이어
	...
	$ON_TABLE_QE_SET_IMG : function(elSelected){
		...
		if(n === "0"){
			...
			if(n == 19 || n == 20 || n == 21 || n == 22 || n == 25 || n == 26){ //화일 크기때문에 jpg
	...
	$ON_SETUP_TABLE_QE_MODE : function(nMode){
		...
		for(var i = 1, nLen = this.aSelectedCells.length; i < nLen; i++){
			// [SMARTEDITORSUS-1552] 끌기로 셀을 선택하는 중 elCell이 없는 경우 오유 발생
...
//			this.elSelectBoxTemplate.style.position = "";
			...
			//this.elPanelReviewBtnArea.style.display = "none"; //My리뷰 단추 레이어
...
//			this.elSelectBoxTemplate.style.position = "static";
			...
			//	this.elPanelReviewBtnArea.style.display = "block"; //My리뷰 단추 레이어
	...
	$ON_TE_MERGE_CELLS : function(){	
		...
		for(i = 1; i < this.aSelectedCells.length; i++){
			...
			if(elTD.parentNode){
				...
				// [SMARTEDITORSUS-1533] 병합되는 셀 바로 뒤에 포함된 빈 본문 노드도 함께 제거하여 DOM 트리 일관성 유지
				var htBrowser = jindo.$Agent().navigator();
...
//		this._updateSelection();
	...
	$ON_TE_DELETE_COLUMN : function(){
		// [SMARTEDITORSUS-1784] [SMARTEDITORSUS-555] 처리후 발생한 싸이트이펙트
	...
	$ON_TE_SPLIT_COLUMN : function(){
		...
		for(var i = 0, nLen = this.aSelectedCells.length; i < nLen; i++){
			...
			// [SMARTEDITORSUS-1745][SMARTEDITORSUS-1842] 배경색이 바로 반영되지 않는 오유로 인해 명시
			var htBrowser = jindo.$Agent().navigator();
			if(htBrowser.ie && (htBrowser.nativeVersion >= 9 || htBrowser.nativeVersion <= 11) && (htBrowser.version >= 9 || htBrowser.version <= 11)){
...
//top.document.title = this.htSelectionSPos.x+","+this.htSelectionSPos.y+"::"+this.htSelectionEPos.x+","+this.htSelectionEPos.y;
	...
	// 셀 선택 상태에서 문서령역을 상/하로 벗어날 경우 벗어난 방향으로 선택 셀을 늘려가며 문서의 스크롤을 해줌
	...
	_coverResizeLayer : function(){
		// [SMARTEDITORSUS-1504] 편집기 전체 크기보다 창이 작아졌을 때 elResizeGrid가 최대화된 elResizeCover으로부터 벗어나는 이슈가 있음
	...
	_reassignCellSizes : function(elTable){
		...
		for(var n = 0; n < 2; n++){
			for(var i = 0; i < allCells[n].length; i++){
				...
				// [SMARTEDITORSUS-1571] IE 10 이상임에도 불구하고, 문서 방식이 8 이하로 설정되여 있는 경우가 있어 메쏘드 기반 분기로 변경
				if(elCell.getComputedStyle){
				// --[SMARTEDITORSUS-1571]
					// getComputedStyle()로 inherit된 양식을 획득. IE 8 이하에서는 지원되지 않는다.  
					...
					// 최초 크기변경 직전 width attribute에서 style의 width로 이행하는 과정에서 미세보정이 있기 때문에 크기가 조금 변한다.
				...
				}else{ // 이 방식은 inherit된 양식을 가져오지 못하는 문제와 함께, 일부 웨브열람기의 소수점 값을 버림하는 문제가 있다.
					...
					// 기존 로직을 사용. IE의 경우 bug로 분류하여 1px를 획득하도록 설정되여 있다.
				...
				/**
				 * 매번 발생하는 크기변경 오차를 최소하기 위하여 2회차부터는 1회차에 적용되는 style 값을 가져온다.
				 * 
				...
				/*if(htBrowser.ie && (htBrowser.nativeVersion >= 9 && htBrowser.nativeVersion <= 10)){
					// IE9, IE10
					// [SMARTEDITORSUS-1427][SMARTEDITORSUS-1431][SMARTEDITORSUS-1491] IE9, 10에서 Jindo.$Element#css 관련 문제에 대응하면 다른 웨브열람기와 동일한 수식 적용 가능
...
//			jindo.$Element(aCellInfo[0]).css("height", aCellInfo[2]);
	...
	_mousedown_ResizeCover : function(oEvent){
		...
		// [SMARTEDITORSUS-1504] 표 크기변경용 gripper의 배치를 WYSIWYG 편집령역 위치 기반으로 개선 
	...
	_mousemove_ResizeCover : function(oEvent){
		...
		// - 최초 크기변경 후 해당 위치에서 바로 마우스를 눌러 Drag 가능
	...
	_mouseup_ResizeCover : function(oEvent){
		...
		// [SMARTEDITORSUS-1655] 메쏘드, 속성 기반 리팩토링
		var width, height;
		// --[SMARTEDITORSUS-1655]
	...
	_isOnBorder : function(wevE){
		...
		if(this.htResizing.htEPos.offsetY <= nAdjustedDraggableCellEdge1){
			...
			if(this.htResizing.elCell.parentNode.previousSibling){
				...
				// [SMARTEDITORSUS-1504] 표 크기변경용 gripper 배치 개선
	...
	_showCellResizeGrip : function(){
		// [SMARTEDITORSUS-1504] gripper가 WYSIWYG 편집령역 위치정보에 기반하여 배치되도록 변경
		/*if(this.htResizing.nResizeMode == 1){
		...
		this._showResizer();*/
		// 만약 iframe 내부에 gripper를 생성한다면 카숄 위치를 기반으로 생성해 주면 됨
		/*if(this.htResizing.nResizeMode == 1){
		...
		// gripper는 대상 셀에서 어느 경계 위에 카숄이 위치했느냐에 기반하여 배치
	...
	_getAllAffectedCells : function(basis_x, basis_y, iResizeMode, oTable){
		...
		if(iResizeMode == 1){
			...
			// 높이 크기변경 시에는 선택 테두리 앞쪽 셀만 조절 함으로 아래쪽 셀은 생성 할 필요 없음
	...
	_createCellResizeGrip : function(){
		...
		// [SMARTEDITORSUS-1504] gripper를 WYSIWYG 편집령역 위치 정보에 기반하여 배치하도록 개선
		//document.body.appendChild(this.elResizeGrid);
		// document.body 대신 WYSIWYG 편집령역을 둘러싼 container div에 추가
	...
	_deleteSelectedCells : function(){
		...
		for(var i = 0, nLen = this.aSelectedCells.length; i < nLen; i++){
			...
			// [SMARTEDITORSUS-1533] 삭제되는 렬 바로 뒤에 린접한 빈 본문 노드도 함께 삭제하여 DOM 트리 일관성 유지 
			var htBrowser = jindo.$Agent().navigator();
			if(htBrowser.ie && (htBrowser.nativeVersion == 9 || htBrowser.nativeVersion == 10) && (htBrowser.version == 9 || htBrowser.version == 10)){
		...
		if(nSelectionWidth == nWidth){
			for(var i = 0, nLen = aTR.length; i < nLen; i++){
				...
				if(!this.htMap[0][i] || !this.htMap[0][i].parentNode || this.htMap[0][i].parentNode.tagName !== "TR"){
					// [SMARTEDITORSUS-1533] 삭제되는 행 바로 뒤에 린접한 빈 본문 노드도 함께 삭제하여 DOM 트리 일관성 유지
					var htBrowser = jindo.$Agent().navigator();
					if(htBrowser.ie && (htBrowser.nativeVersion == 9 || htBrowser.nativeVersion == 10) && (htBrowser.version == 9 || htBrowser.version == 10)){
						this._removeEmptyTextNode_IE(elTmp);
					}
	...
	_insertColumnAfter : function(){
		...
		for(var y = 0, nYLen = this.htMap[0].length; y < nYLen; y++){
			...
			for(var x = this.htSelectionEPos.x; x >= this.htSelectionSPos.x; x--){
				...
				// [SMARTEDITORSUS-1742][SMARTEDITORSUS-1842] 배경색이 바로 반영되지 않는 오유로 인해 명시
				if(htBrowser.ie && (htBrowser.nativeVersion >= 9 || htBrowser.nativeVersion <= 11) && (htBrowser.version >= 9 || htBrowser.version <= 11)){
	...
	_insertRowBelow : function(){
		...
		for(var y = this.htSelectionSPos.y; y <= this.htSelectionEPos.y; y++){
			...
			// [SMARTEDITORSUS-1533] 문서방식이 9~10인 IE 9~10에서 빈 본문 노드가 인식된다.
			// [SMARTEDITORSUS-1842]
			if(htBrowser.ie && (htBrowser.nativeVersion >= 9 || htBrowser.nativeVersion <= 11) && (htBrowser.version >= 9 || htBrowser.version <= 11)){
			// --[SMARTEDITORSUS-1842]
				// 양식을 적용시켜 줄 대상 tr
				...
				for(var i = 0, len = aCellClone.length; i < len; i++){
					...
					// [SMARTEDITORSUS-1639] 병합 후 추가시 JS 오유가 발생하는 문제가 있어 nodeName 확인
					// [SMARTEDITORSUS-1672]
...
*/
		...
		// remove empty TR's
		// (상단 TD의 rowspan만으로 지탱되는) 빈 TR이 있을 경우 IE7 이하에서 랜더링 오유가 발생 할수 있어 빈 TR을 지워 줌
	...
	_addClassToSelection : function(){
		...
		for(var i = 0; i < this.aSelectedCells.length; i++){
			...
			// [SMARTEDITORSUS-1552] 끌기로 셀을 선택하는 중 elCell이 없는 경우 오유 발생
			if(elCell){
				// [SMARTEDITORSUS-1498][SMARTEDITORSUS-1549] 선택된 모든 셀에서 끌기가 발생하지 못하게 방지(FF, Chrome)
...
*/
	...
	_isValidTable : function(elTbl){
		...
		for(var i = 1; i < nXSize; i++){
			// 첫번째 렬과 길이가 다른 렬이 하나라도 있다면 직사각형이 아님
...
nhn.husky.HuskyCore.mixin(nhn.husky.SE2M_FontColor, {
	...
	$ON_APPLY_FONTCOLOR : function(sFontColor){
		...
		// [SMARTEDITORSUS-907] 모든 웨브열람기에서 SET_WYSIWYG_STYLE로 색상을 설정하도록 변경
		// var oAgent = jindo.$Agent().navigator();
...
nhn.husky.HuskyCore.mixin(nhn.husky.SE2M_Hyperlink, {
	...
	$ON_MSG_HYPERLINK_LAYER_SHOWN : function(){
		...
		if(oAnchor && !this.oSelection.collapsed){
			...
			// href속성에 문제가 있을 경우 례: href="http://na&nbsp;&nbsp; ver.com", IE에서 oAnchor.href 접근 시에 알수 없는 오유를 발생시킴
			try{
	...
	$ON_APPLY_HYPERLINK : function(){
		...
		// [SMARTEDITORSUS-1451] 글감에 초련결를 적용하지 않도록 처리
		...
		if(this._validateURL(sURL)){
			...
			if(this.oSelection.collapsed){
				...
				// 웨브열람기에서 제공하는 execcommand에 createLink로는 타겟을 지정할수가 없다.
				// 그렇기 때문에, 더미 URL을 createLink에 넘겨서 초련결를 먼저 걸고, 이후에 loop을 돌면서 더미 URL을 가진 A태그를 찾아서 정상 URL 및 타겟을 설정 해 준다.
				sBM = this.oSelection.placeStringBookmark();
				this.oSelection.select();
				
				// [SMARTEDITORSUS-61] TD 안에 있는 본문를 전체 선택하여 URL 변경하면 수정되지 않음 (only IE8)
				//		SE_EditingArea_WYSIWYG 에서는 IE인 경우 beforedeactivate 이벤트가 발생하면 현재의 Range를 보관하고, RESTORE_IE_SELECTION 통보문이 발생하면 보관된 Range를 적용한다.
				//		IE8 또는 IE7 호환방식이고 TD 안의 본문 전체를 선택한 경우  Bookmark 생성 후의 select()를 처리할 때
				//		HuskyRange 에서 호출되는 this._oSelection.empty(); 에서 beforedeactivate 가 발생하여 empty 처리된 selection 이 보관되는 문제가 있어 초련결가 적용되지 않음.
				//		정확한 selection 이 보관되여 EXECCOMMAND에서 초련결가 적용될수 있도록 함
				if(oAgent.ie && (oAgent.version === 8 || oAgent.nativeVersion === 8)){	// nativeVersion 으로 IE7 호환방식인 경우 확인
					this.oApp.exec("IE_FOCUS", []);
					this.oSelection.moveToBookmark(sBM);
					this.oSelection.select();
				}
				
				// createLink 이후에 이번에 생성된 A 태그를 찾을수 있도록 nSession을 포함하는 더미 초련결을 만든다.
	...
	_isExceptional : function(){
		...
		// [SMARTEDITORSUS-612] 그림 선택 후 초련결 추가했을 때 초련결가 걸리지 않는 문제
		...
		// [SMARTEDITORSUS-579] IE8 이하에서 E-mail 패턴 문자렬에 URL 초련결 못거는 이슈
...
nhn.husky.HuskyCore.mixin(nhn.husky.SE2M_QuickEditor_Common, {
	...
	setOpenType : function(sType,bBol){
		// [SMARTEDITORSUS-1213] 작성된 콘텐츠 수정 화면에서 사진이 적재되자마자 바로 사진을 클릭하면 QuickEditor를 띄우는 데 문제가 있음 
	...
	/**
	 * 레이어가 열기 할 때 실행되는 이벤트.
	 * 레이어가 처음 뜰 때
	 * 		보관된 단축건 리스트를 레이어에 등록하고 (레이어가 떠 있을때도 단축건가 먹도록 하기 위해)
	 * 		레이어에 대한 건반/마우스 이벤트를 등록한다.
	 * @param {Element} oEle
	...
	$LOCAL_BEFORE_FIRST : function(sMsg) {
		...
		new jindo.DragArea(this._qe_wrap, {
			...
			dragEnd : function(oCustomEvent){
				...
				//if(that._currentType=="review"||that._currentType=="table"){	// [SMARTEDITORSUS-153] 그림 빠른 편집기도 같은 로직으로 처리하도록 수정
...
//		var beforeY = oBefore.css("top");
		...
		// QuickEditor를 접고 펼칠 때마다 API 통신을 거치기 때문에 submit이나 beforeunload에 구애받지 않고 안정적인 자료 보관 가능
...
*/
	...
	/**
	 * 레이어를 띄우는데 레이어가 table(형타),img인지를 확인하여 id를 반환하는 함수.
	 * @param {Element} oEle
	...
	/**
	 * 빠른편집기에서 keyup시 실행되는 이벤트
	 */
	$ON_QE_IN_KEYUP : function(){
		this._in_event = true;
	},
	/**
	 * 빠른편집기에서 mousedown시 실행되는 이벤트
	 */
	$ON_QE_IN_MOUSEDOWN : function(){
		this._in_event = true;
	},
	/**
	 * 빠른편집기에서 mousewheel시 실행되는 이벤트
	 */
	...
	/**
	 * 레이어의 위치를 반환 하는 함수
	 *		고정 상태가 아니거나 최소화 상태이면 요소 위치에 빠른편집기를 띄우고
	 *		고정 상태이고 최대화 상태이면 표나 글 양식은 보관된 위치에 띄워주고, 그림는...?
	 * @param {Element} oEle
	 * @param {Element} oLayer
	 */
	get_position_layer : function(oEle , oLayer){
		...
		//if(this._currentType == "review" || this._currentType == "table"){	// [SMARTEDITORSUS-153] 그림 빠른 편집기도 같은 로직으로 처리하도록 수정
			...
			// [SMARTEDITORSUS-129] 편집령역 높이를 줄였을 때 빠른편집기가 령역을 벗어나지 않도록 처리
...
nhn.FindReplace = jindo.$Class({
	...
	find : function(sKeyword, bCaseMatch, bBackwards, bWholeWord){
		...
		// [SMARTEDITORSUS-1814] LG웨브열람기의 경우 focus를 주면 선택령역이 풀리는 문제가 있어서 LG기기가 아닌 경우만 focus를 실행하도록 수정
	...
	findNext : function(sKeyword, bCaseMatch, bBackwards, bWholeWord){
		...
		if(this.document.body.createTextRange){
			try{
				...
				}else{							// [SMARTEDITORSUS-1837] 이미 생성되여 있는 TextRange를 리용해 collapseEnd 하면 다음 문자를 찾을수 있다.
					this._range.collapse(false);
				}
	 ...
	 * selection 객체를 받아서 사용할수 있도록 private 메쏘드 추가
	...
	replaceAll : function(sOriginalWord, Replacement, bCaseMatch, bWholeWord){
		...
		oSelection.moveToBookmark(sBookmark);
		oSelection.deleteContents();	// [SMARTEDITORSUS-1591] 크롬에서 첫번째 단어가 삭제되지 않는 경우가 있으므로 select()메쏘드대신 deleteContents() 메쏘드를 호출한다.
		oSelection.removeStringBookmark(sBookmark);

		// setTimeout 없이 바로 지우면 IE8 웨브열람기가 빈번하게 죽어버림
		setTimeout(function(){
	...
	_getFirstTextNode : function(){
		...
		// 처음 노드가 본문 노드가 아니거나 bogus 노드라면 다음 본문 노드를 찾음
