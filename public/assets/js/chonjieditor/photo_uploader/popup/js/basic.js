...
jQuery(function ($) {
    ...
    var basicUrl = localStorage.getItem('apiUrl').substr(0, localStorage.getItem('apiUrl').lastIndexOf('/') + 1);

    // Change this to the location of your server-side upload handler:
    var gnu = {
        basicUrl : basicUrl,
        url : localStorage.getItem('apiUrl') + '/files/upload/public/',
        headers: {'Authorization': localStorage.getItem('token')},
        container_el : 'body',
        dreg_area : '#drag_area',
        dreg_area_list : '#drag_area > ul',
        progress_bar : '#progress .progress-bar',
        filter : /^(image\/bmp|image\/gif|image\/jpg|image\/jpeg|image\/png|video\/mp4)$/i,
        files : [],
        file_limit : 10, //한번에 올릴수 화일개수 제한
        imgw : 100,
        imgh : 70,
        ...
        _delete : function(e){
            ...
            var othis = gnu,
                ...
                // file_type = $button.attr("data-type");
            // if( delete_url ){
            //     $.ajax({
            //         type: "POST",
            //         url: othis.url + delete_url + '?type=' + file_type,
            //         data: {
            //             'op': "DELETE"
            //         }
        ...
        _add : function(e, data, preload){
            ...
            $.each(data.files, function (index, file) {
                ...
                var node = $('<div/>')
                    ...
                    //     $img = "<video class='pre_thumb_video' controls autoplay />";
                    //     $img = "<img src='./img/loading.gif' class='pre_thumb_video' />",
                    // }
        ...
        _done : function(e, data){
            var othis = this;
            $.each(data.result.uploaded, function (index, file) {
                if (file._id && file._id != "") {  //if (file.url && !file.error) {
                    var tmpUrl = gnu.basicUrl + file._id + '.' + file.name.split('.')[file.name.split('.').length - 1];
                    // var tmpUrl = gnu.basicUrl + file.name;
                    var ret = othis.get_ratio( file.width, file.height ),
                        node = $(data.context.children()[index]),
                        // size_text = file.width+" x "+file.height,
                        ...
                        if (file.mime.indexOf('video') >= 0){
                            ...
                            .append('<br>')
                            // .append($('<span/>').text(size_text))
                            .find("img.pre_thumb").remove();
                            var $video = "<video class='pre_thumb_video' controls />";
                            node
                            .prepend('<br>')
                            .prepend($video);
                            node
                            ...
                            .append('<br>')
                            // .append($('<span/>').text(size_text))
                            .find("video.pre_thumb_video").attr({"src":tmpUrl,"width":ret['width'],"height":ret['height']})
                            .end().find(".delete_img").attr({"data-delete":file._id,"data-url":tmpUrl,"data-type":file.mime});
                        } else {
                            node
                            //.wrap(link)
                            .append('<br>')
                            // .append($('<span/>').text(size_text))
                            .find("img.pre_thumb").attr({"src":tmpUrl,"width":ret['width'],"height":ret['height']})
                            .end().find(".delete_img").attr({"data-delete":file._id,"data-url":tmpUrl,"data-type":file.mime});
                        }
                } else if (file.error) {
        ...
        setMultiMediaToEditor : function(oFileInfo){
            if (!!opener && !!opener.nhn && !!opener.nhn.husky && !!opener.nhn.husky.PopUpManager) {
                //스마트 편집기 플러그인을 통해서 넣는 방법 (oFileInfo는 Array)
                opener.nhn.husky.PopUpManager.setCallback(window, 'SET_MULTIMEDIA', [oFileInfo]);
                //본문에 바로 tag를 넣는 방법 (oFileInfo는 String으로 <img src=....> )
                // opener.nhn.husky.PopUpManager.setCallback(window, 'PASTE_HTML', [oFileInfo]);
            }
        }
    ...
    $('#fileupload').fileupload({
        ...
        acceptFileTypes: /(\.|\/)(gif|jpe?g|bmp|png|mp4)$/i,
	...
	var listeners = {
		...
		fileQueueError: function(event, file, errorCode, message){
            ...
            {
                case -100 :
                    alert("화일을 "+message+"개 이하로 선택해주십시오.");
                    break;
            }
    ...
    $(gnu.dreg_area).bind('drop dragover', function (e) {
        e.preventDefault();
        if( !gnu.file_api_support && e.type == 'drop' ) alert("열람기가 해당 동작을 지원하지 않습니다.");
    });
    $("#all_remove_btn").bind("click", function(e){
        e.preventDefault();
        if( $(gnu.dreg_area_list).children().length ){
            if (confirm("추가한 매체가 있습니다.정말 삭제 하겠습니까?")){
    ...
    $("#img_upload_submit").bind("click", function(e){
        ...
        $(gnu.dreg_area_list).find(".delete_img").each( function(i, f){
            ...
            aResult[j]['sFileType'] = $(this).attr("data-type");
            j++;
        });
        if( aResult.length ){
            // gnu.setPhotoToEditor(aResult);
            gnu.setMultiMediaToEditor(aResult);
            aResult = null;
        }
        window.close();
    });
