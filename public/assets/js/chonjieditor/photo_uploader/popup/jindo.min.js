 ...
 * @function
 * $ 함수는 다음의 두 가지 역할을 한다.
 * <ul><li/>ID를 사용하여 HTML 요소를 가져온다. 매개변수를 두 개 이상 지정하면 HTML 요소를 원소로하는 배렬을 반환한다.
 * <li>또한 "<tagName>" 과 같은 형식의 문자렬을 입력하면 tagName을 가지는 객체를 생성한다.</li></ul>
 * @param {String...} sID HTML 요소의 ID. ID는 하나 이상 지정할수 있다. (1.4.6부터는 마지막 매개변수에 document을 지정할수 있다.)
 * @return {Element|Array} HTML 요소 혹은 HTML 요소를 원소로 가지는 배렬을 반환한다. 만약 ID에 해당하는 HTML 요소가 없으면 null을 반환한다.
 * @description [Lite]
 * @example
// ID를 리용하여 객체를 반환한다.
<div id="div1"></div>
...
// ID를 리용하여 여러개의 객체를 반환한다.
<div id="div1"></div>
<div id="div2"></div>

var els = $("div1","div2"); // [$("div1"),$("div2")]와 같은 결과를 반환한다.

// tagName과 같은 형식의 문자렬을 리용하여 객체를 생성한다.
var el = $("<DIV>");
var els = $("<DIV id='div1'><SPAN>hello</SPAN></DIV>");

//IE는 iframe에 추가할 요소를 생성하려고 할 때는 document를 반드시 지정해야 한다.(1.4.6 부터 지원)
 ...
 * 클라스 객체를 생성한다.
 * @extends core
 * @class $Class는 Jindo에서 객체 지향 프로그래밍(OOP)를 구현하는 객체이다. $Class.$init 메소드는 클라스를 생성할 때 클라스 인스턴스에 대한 생성자 함수를 정의한다.
 * @param {Object} oDef 클라스를 정의하는 객체. 메서드, 속성와 생성자를 정의한다. 	$staic 키워드는 인스턴스를 생성하지 않아도 사용할수 있는 메서드의 집합이다.
 * @return {$Class} 클라스 객체
 * @description [Lite]
 * @example
var CClass = $Class({
 ...
 * 클라스를 상속한다.
 * 하위 클라스는 this.$super.method 로 상위 클라스의 메서드에 접근할수 있으나, this.$super.$super.method 와 같이 한 단계 이상의 상위 클라스는 접근할수 없다.
 * @name $Class#extend 
 * @type $Class
 * @function
 * @param {$Class} superClass 수퍼 클라스 객체
 * @return {$Class} 상속된 클라스
 * @description [Lite]
 * @example
...
jindo.$Class.extend = function(superClass) { 
	...
	if (typeof superClass == "undefined" || superClass === null || !superClass.extend) {
		throw new Error("extend시 슈퍼 클라스는 Class여야 합니다.");
	}
...
/**
 ...
 부모 클라스의 메서드에 접근할 때 사용한다. 부모 클라스와 자식 클라스가 같은 이름의 메서드를 가지고 있고 $super로 그 메서드를 호출하면 자식 클라스의 메서드를 사용한다.
 @name $Class#$super
 @type $Class
 @example
	var Parent = $Class ({
	...
	oChild.sum();           // 20
	oChild.sum2();          // 20
	oChild.$super.sum();    // 30 -> 부모 클라스의 100(a)과 200(b)대신 자식 클라스의 10(a)과 20(b)을 더한다.
	oChild.$super.sum2();   // 20 -> 부모 클라스의 sum2 메서드에서 부모 클라스의 sum()이 아닌 자식 클라스의 sum()을 호출한다.
  
*/
/**
 
 * @fileOverview CSS 셀렉터를 사용한 요소 선택 엔진
 * @name cssquery.js
 * @author Hooriza
 ...
 * @return {Array} 조건에 해당하는 요소의 배렬을 반환한다.
 * @description [Lite]
 * @example
 // 문서에서 IMG 태그를 찾는다.
 var imgs = $$('IMG');
...
인용부호, [] 등 파싱에 문제가 될수 있는 부분 replace 시켜놓기
...
replace 시켜놓은 문자렬에서 Quot 을 제외하고 반환
...
유사클라스 조건 검사가 맨 뒤로 가도록 unshift 사용
...
캐싱되여 있는게 있으면 parent 가 같은건지 검사한후 aSingleQueryResult 에 대입
...
결과 캐쉬를 사용중이면 캐쉬에 저장
	 ...
	 * 특정 요소가 해당 CSS 셀렉터에 부합하는 요소인지 판단한다
	 * @remark CSS 셀렉터에 련결자는 사용할수 없음에 유의한다.
	 * @param {Element} element	검사하고자 하는 요소
	 * @param {String} selector	CSS 셀렉터
	 * @return {Boolean} 셀렉터 조건에 부합하면 true, 부합하지 않으면 false
	 * @example

// oEl 이 div 태그 또는 p 태그, 또는 align=center 인 요소인지
if (cssquery.test(oEl, 'div, p, [align=center]')) alert('해당 조건 만족');// oEl 이 div 태그 또는 p 태그, 또는 align=center 인 요소인지
if (cssquery.test(oEl, 'div, p, [align=center]')) alert('해당 조건 만족');
	 ...
	 * cssquery 에 결과 캐쉬를 사용할것인지 지정하거나 확인한다.
	 ...
	 * @param {Boolean} flag	사용할것 인지 여부 (생략시 사용 여부만 반환)
	 ...
	 * CSS 셀렉터를 사용하여 DOM 에서 원하는 요소를 하나만 얻어낸다. 반환하는 값은 배렬이 아닌 객체 또는 null 이다.
	 * @remark 결과를 하나만 얻어내면 이후의 모든 탐색 작업을 중단하기 때문에 결과가 하나라는 보장이 있을때 빠른 속도로 결과를 얻어올 수 있다.
	 * @param {String} selector	CSS 셀렉터
	 * @param {Document | Element} el	탐색을 진행하는 기준이 되는 요소 또는 문서 (생략시 현재 문서의 document 객체)
	 * @param {Object} 오브젝트에 onTimeOffCache를 true로 하면 해당 쿼리는 cache를 사용하지 않는다.
	 * @return {Element} 선택된 요소
...
cssquery.debug(false);
	 ...
	 * @param {Boolean} flag	true 로 설정하면 _cssquery_UID 가 나오지 않음
	 ...
	 * 하지만 ID가 없는 요소를 기준 요소로 넣었을 때 기준 요소에 임의의 아이디가 들어간다.
 ...
 * @fileOverview $Agent의 생성자 및 메서드를 정의한 화일
 ...
 * navigator 메서드는 웨브 브라우저의 정보 객체를 반환한다.
 * @return {Object} 웨브 브라우저 정보를 저장하는 객체. <br>
 * object는 브라우저 이름과 판본을 속성으로 가진다. 브라우저 이름은 영어 소문자로 표시하며, 사용자의 브라우저와 일치하는 브라우저 이름은 true를 가진다.
 ...
 * flash 메서드는 플래시에 대한 정보 객체를 반환한다.
 * @return {Object} Flash 정보 객체. <br>
 * object.installed는 플래시 플레이어 설치 여부를 boolean 값으로 가지고 object.version은 플래시 플레이어의 판본을 가진다. 플래시 판본을 탐지하지 못하면 flash.version은 -1의 값을 가진다.
 * @example
var oFlash = $Agent.flash();
oFlash.installed  // 플래시 플레이어를 설치했다면 true
oFlash.version  // 플래시 플레이어의 판본. 
 ...
 * silverlight 메서드는 실버라이트(Silverlight)에 대한 정보 객체를 반환한다.
 * @returns {Object} Silverlight 정보 객체. <br>
 * object.installed은 실버라이트 플레이어 설치 여부를 boolean 값으로 가지고 object.version은 실버라이트 플레이어의 판본을 가진다. 플레이어의 판본을 탐지하지 못하면 object.version의 값은 -1이 된다.
 * @example
var oSilver = $Agent.silverlight();
oSilver.installed  // Silverlight 플레이어를 설치했다면 true
oSilver.version  // Silverlight 플레이어의 판본. 
 ...
 * @fileOverview $A의 생성자 및 메서드를 정의한 화일
 * @name array.js
 ...
 * $A 객체를 생성하여 반환한다.
 * @extends core
 * @class	$A 클라스는 배렬(Array)을 래핑(wrapping)하여 배렬을 다루기 위한 여러가지 메서드를 제공한다.<br>
 * 여기서 래핑이란 자바스크립트의 함수를 감싸 본래 함수의 기능에 새로운 확장 속성을 추가하는 것을 말한다.
 * @param 	{Array|$A} array 배렬. 만약 매개 변수를 생략하면 빈 배렬을 가진 새로운 $A 객체를 반환한다.
 * @constructor
 * @description [Lite]
 * @author Kim, Taegon
 *
 ...
 * toString 메서드는 내부 배렬을 문자렬로 변환한다. 자바스크립트의 Array.toString 을 사용한다.
 * @return {String} 내부 배렬을 변환한 문자렬.
 * @description [Lite]
 *
 * @example
var zoo = ["zebra", "giraffe", "bear", "monkey"];
 ...
 * 인덱스로 배렬의 원소 값을 조회한다.
 * @param {Number} nIndex 조회할 배렬의 인덱스. 인덱스는 0부터 시작한다.
 * @return {Value} 배렬에서의 해당 인덱스의 원소 값.
 * @description [Lite]
 * @since 1.4.2 부터 지원
 *
 * @example
 ...
 * 내부 배렬의 크기를 지정하거나 반환한다.
 * @param 	{Number} [nLen]	지정할 배렬의 크기.<br>
 * nLen 이 기존 배렬의 크기보다 크면 oValue 매개 변수의 값을 배렬의 마지막에 덧붙인다.<br>
 * nLen 이 기존 배렬의 크기보다 작으면 nLen 번째 이후의 원소는 제거한다.
 * @param 	{Value} [oValue] 새로운 원소를 추가할 때 사용할 초기값
 * @return 	{Number|$A} 매개 변수를 모두 생략하면 현재 내부 배렬의 크기를 반환하고,<br>
 * 매개 변수를 지정한 경우에는 내부 배렬을 변경한 $A 객체를 반환한다.
 *
 * @example
var zoo = ["zebra", "giraffe", "bear", "monkey"];
...
// 배렬의 크기 지정 (원소가 삭제되는 경우)
$A(zoo).length(2);
// 결과 : ["zebra", "giraffe"]

// 배렬의 크기 지정 (원소가 추가되는 경우)
 ...
 * @return {Boolean} 배렬에서 매개 변수의 값과 동일한 원소를 찾으면 true를, 찾지 못하면 false를 반환한다.
 * @see $A#indexOf
 * @description [Lite]
 *
 * @example
 ...
 * 배렬에서 특정 값을 검색하고 검색한 원소의 인덱스를 반환한다.
 * @param {Value} oValue 검색할 값
 * @return {Number} 찾은 원소의 인덱스. 인덱스는 0 부터 시작한다. 매개 변수와 동일한 원소를 찾지 못하면 -1 을 반환한다.
 * @see $A#has
 * @description [Lite]
 *
 * @example
 ...
 * @example
var waNum = $A([1, 2, 3]);
waNum.$value(); // 원래의 배렬인 [1, 2, 3]이 반환된다
 ...
 * 내부 배렬에 하나 이상의 원소를 추가한다.
 * @param {oValue1, ..., oValueN} oValueN 추가할 N 개의 값
 * @return {Number} 하나 이상의 원소를 추가한 내부 배렬의 크기
 * @description [Lite]
 *
 * @example
var arr = $A([1,2,3]);
...
// 원소 추가
arr.push(4);	// 결과 : 4 반환, 내부 배렬은 [1,2,3,4]로 변경 됨
arr.push(5,6);	// 결과 : 6 반환, 내부 배렬은 [1,2,3,4,5,6]로 변경 됨
...
arr.pop(); // 결과 : 5 반환, 내부 배렬은 [1,2,3,4]로 변경 됨
 ...
 * 내부 배렬의 모든 원소를 앞으로 한 칸씩 이동한다. 내부 배렬의 첫번째 원소는 삭제된다.
 * @return {Value} 삭제한 첫번째 원소.
 * @see $A#pop
 * @see $A#unshift
 * @description [Lite]
 * @example
...
arr.shift(); // 결과 : 'Melon' 반환, 내부 배렬은 ["Grape", "Apple", "Kiwi"]로 변경 됨.
 ...
 * 내부 배렬의 맨 앞에 하나 이상의 원소를 삽입한다.
 * @param {oValue1, ..., oValueN} oValueN 삽입할 하나 이상의 값
 * @return {Number} 원소를 추가한 후의 배렬의 크기
 * @description [Lite]
 * @example
var arr = $A([4,5]);

arr.unshift('c');		// 결과 : 3 반환, 내부 배렬은 ["c", 4, 5]로 변경 됨.
arr.unshift('a', 'b');	// 결과 : 5 반환, 내부 배렬은 ["a", "b", "c", 4, 5]로 변경 됨.
 ...
 * @return {$A} 내부 배렬의 일부를 추출한 새로운 $A 객체.<br>
 * nStart 값이 0 보다 작거나 혹은 nStart 값이 nEnd 값 보다 크거나 같으면 빈 배렬을 가지는 $A 객체를 반환한다.
 * @description [Lite]
 *
 * @example
var arr = $A([12, 5, 8, 130, 44]);
var newArr = arr.slice(1,3);
// 잘라낸 배렬인 [5, 8]를 래핑한 $A 객체를 반환한다. (원래의 배렬은 변화 없음)
 ...
 * 이 값과 oValueN 를 생략하면 nIndex 번째 원소부터 배렬의 마지막 원소까지 삭제한다.<br>
 * 이 값을 0 혹은 지정하지 않고 oValueN 에 값을 지정하면 nIndex 번째 위치에 oValueN 값이 추가된다.
 * @param {Value1, ...,ValueN} [oValueN] 삭제한 배렬에 추가할 하나 이상의 값. nIndex 값의 인덱스부터 추가된다.
...
// arr의 내부 배렬은 ["angel", "clown", "drum", "mandarin", "surgeon"]로 인덱스 2에 drum이 추가 됨
// removed의 내부 배렬은 []로 삭제된 원소가 없음

removed = arr.splice(3, 1);
// arr의 내부 배렬은 ["angel", "clown", "drum", "surgeon"]로 mandarin이 삭제 됨
// removed의 내부 배렬은 삭제된 원소 ["mandarin"]를 가짐

removed = arr.splice(2, 1, "trumpet", "parrot");
// arr의 내부 배렬은 ["angel", "clown", "trumpet", "parrot", "surgeon"]로 drum이 삭제되고 새로운 원소가 추가 됨
// removed의 내부 배렬은 삭제된 원소 ["drum"]을 가짐

removed = arr.splice(3);
// arr의 내부 배렬은 ["angel", "clown", "trumpet"]로 인덱스 3부터 마지막 원소가 삭제되였음
// removed의 내부 배렬은 삭제된 원소 ["parrot", "surgeon"]을 가짐
 ...
 * 배렬의 모든 원소를 제거하고, 빈 배렬로 만든다.
 * @return {$A} 배렬의 원소가 제거된 $A 객체
 * @description [Lite]
 *
 * @example
var arr = $A([1, 2, 3]);
 ...
 * @remark 내부적으로는 강제로 례외를 발생시키는 구조이므로, try ~ catch 령역에서 이 메소드를 실행하면 정상적으로 동작하지 않을 수 있다.
 *
 * @description [Lite]
 * @see $A#Continue
 * @see $A#forEach
 ...
 * @fileOverview $A의 확장 메서드를 정의한 화일
 ...
 * 배렬의 모든 원소를 순회하면서 콜백 함수를 실행한다.<br>
 * 콜백 함수의 실행 결과를 배렬의 원소에 설정한다.
 *
 * @param {Function}	fCallback	순회하면서 실행할 콜백 함수.<br>
 * <br>
 * 콜백 함수는 fCallback(value, index, array) 의 형식을 가진다. <br>
 * value 는 배렬이 가진 원소의 값을 가지고,<br>
 * index 는 해당 원소의 인덱스를 가지고,<br>
 * array 는 배렬 그 자체를 가리킨다.<br>
 * <br>
 * 콜백 함수에서 반환하는 값을 원소의 값으로 설정한다.
 *
 * @param {Object} [oThis]	콜백 함수가 객체의 메서드일 때 콜백 함수 내부에서 사용할 this
 ...
 * 배렬의 모든 원소를 순회하면서 콜백 함수를 실행한다. 실행이 끝나면 filter 메서드는 콜박 함수를 만족하는 원소로 이루어진 새로운 $A 객체를 반환한다.

 * @param {Function} fCallback	순회하면서 실행할 콜백 함수.<br>
 * <br>
 * 콜백 함수는 fCallback(value, index, array)의 형식으로 작성해야 한다. 여기서
 * value 는 배렬이 가진 원소의 값, index 는 해당 원소의 인덱스, array 는 원본 배렬이다.<br>
 * <br>
 * 콜백 함수는 Boolean 을 반환해야한다. 만약 반환 값이 true 인 원소는 새로운 배렬의 원소가 된다.
 *
 * @param {Object} oThis	콜백 함수가 객체의 메서드일 때 콜백 함수 내부에서 사용할 this
 * @return {$A}	콜백 함수의 반환 값이 true 인 원소로 이루어진 새로운 $A 객체
 * @see $A#forEach
 * @see $A#map
 *
 * @example
 ...
 * 배렬의 모든 원소를 순회하면서 콜백 함수를 실행한다. 동시에 배렬의 모든 원소가 콜백 함수를 만족하는지(콜백 함수가 true를 반환하는지) 검사한다. <br>
 * 만약 모든 원소가 콜백 함수를 만족하면 every 메서드는 true를 반환한다.
 *
 * @param {Function} fCallback	순회하면서 실행할 콜백 함수.<br>
 * <br>
 * 콜백 함수는 fCallback(value, index, array) 의 형식으로 작성해야 한다. 여기서
 * value 는 배렬이 가진 원소의 값, index 는 해당 원소의 인덱스, array 는 원본 배렬이다.<br>
 * <br>
 * 콜백 함수는 Boolean 을 반환해야한다.<br>
 *
 * @param {Object} oThis	콜백 함수가 객체의 메서드일 때 콜백 함수 내부에서 사용할 this
 * @return {Boolean} 콜백 함수의 반환 값이 모두 true 이면 true 를, 그렇지 않으면 false 를 반환한다.
 * @see $A#some
 *
 * @example
 ...
 * <br>
 * 콜백 함수는 fCallback(value, index, array) 의 형식으로 작성해야 한다. 여기서
 * value 는 배렬이 가진 원소, index 는 해당 원소의 인덱스, array 는 원본 배렬이다.<br>
 * <br>
 * 콜백 함수는 Boolean 을 반환해야한다.<br>
 *
 * @param {Object} oThis	콜백 함수가 객체의 메서드일 때 콜백 함수 내부에서 사용할 this
 * @return {Boolean} 콜백 함수의 반환 값이 true 인 원소가 있으면 true 를, 하나도 없으면 false 를 반환한다.
 * @see $A#every
 *
 * @example
 ...
 * 배렬에서 매개 변수와 같은 값을 제외하여 새로운 $A 객체를 만든다.
 *
 * @param {Value, ..., ValueN} oValueN 배렬에서 제외할 값
 * @return {$A} 배렬에서 특정 값을 제외한 새로운 $A 객체
 *
 * @example
var arr = $A([12, 5, 8, 130, 44]);
 ...
 * @fileOverview $Ajax의 생성자 및 메서드를 정의한 화일
 * @name Ajax.js
 ...
 * $Ajax는 봉사기와 브라우저 사이의 비동기 통신, 즉 Ajax 통신을 지원한다. $Ajax는 XHR(XMLHTTPRequest)을 사용한 기본적인 방식과 함께 다른 호스트(Host) 사이의 통신을 위한 여러 방식을 제공한다.
 ...
 *
 * @extends core
 * @class $Ajax 클라스는 다양한 개발 환경에서 Ajax 요청과 응답을 쉽게 구현하기 위한 메서드를 제공한다.<br>
 *
 * $Ajax를 초기화할 때 사용하는 매개 변수는 다음과 같다.
 *
 * @param {String}   url			  Ajax 요청을 보낼 봉사기 측 URL.<br>
 * @param {Object}   option		      $Ajax 에서 사용하는 콜백 함수, 통신 방식 등과 같은 다양한 정보를 정의한다.<br>
 * <br>
 * option 객체의 속성와 사용법은 아래에서 설명한다.<br>
<table>
	...
	<tbody>
			...
			<td>
				...
				<ul>
					<li><strong>xhr</strong>
							브라우저에 내장된 XMLHttpRequest 객체를 리용하여 Ajax 요청을 처리한다.<br>
							응답으로는 text, xml, json 방식 모두 사용이 가능하며, 요청 실패 시 HTTP 응답코드를 통해 원인 파악이 가능하다.<br>
							단, Cross-Domain 에서는 사용할수 없다.
					</li>
					<li><strong>iframe</strong>
							iframe 을 프록시로 사용하여 Ajax 요청을 처리한다. Cross-Domain 에서 사용한다.<br>
							로컬(요청 하는 쪽)과 원격(요청 받는 쪽)에 모두 프록시용 HTML 화일을 만들어<br>
							iframe 에서 원격 프록시에 요청하면 원격 프록시에서 원격 도메인의 페지에 XHR 로 Ajax 요청을 한다.<br>
							응답을 받은 원격 프록시에서 로컬 프록시로 응답을 전달하면 로컬 프록시에서 최종적으로 콜백 함수(onload)를 호출하여 처리된다.<br>
							<ul type="disc">
								<li>원격 프록시 화일 : ajax_remote_callback.html</li>
								<li>로컬 프록시 화일 : ajax_local_callback.html</li>
							</ul>
							※ IE 에서는 "딱.딱." 하는 페지 이동음이 들릴 수도 있다 (요청당 2회).
					</li>
					<li><strong>jsonp</strong>
							JSON 과 &lt;script&gt; 태그를 사용하여 사용하여 Ajax 요청을 처리한다. Cross-Domain 에서 사용한다.<br>
							&lt;script&gt; 태그를 동적으로 생성하여, 요청할 원격 페지를 스크립트에 삽입하여 GET 방식으로 요청을 전송한다.<br>
							요청 시에 콜백 함수를 매개 변수로 넘기면 원격 페지에서 전달받은 콜백 함수명으로 아래와 같이 응답을 보낸다.<br>
							<ul type="disc"><li>function_name(...결과 값...)</li></ul>
							응답은 콜백 함수(onload)에서 처리된다.<br>
							※ GET 방식만 가능하므로, 전송 자료 길이는 URL 에 허용하는 길이로 제한된다.
					</li>
					<li><strong>flash</strong>
						플래시 객체를 사용하여 Ajax 요청을 처리한다. Cross-Domain 에서 사용한다.<br>
						봉사기의 루트에 crossdomain.xml 이 존재해야 하며 접근 권한을 설정해야 사용할수 있다.<br>
		...
		<td style="font-weight:bold;">proxy</td>
			<td>String</td>
			<td>
				로컬 프록시 화일(ajax_local_callback.html)의 경로.<br>
		...
		<tr>
			...
			<td>
				...
				<br>
				jsonp 방식에서 Ajax 요청 시, 콜백 함수 이름에 랜덤한 아이디 값을 덧붙여 만든 콜백 함수 이름을 봉사기로 전달한다.<br>
				이 때 랜덤한 값을 아이디로 사용하여 넘기므로 요청 URL이 매번 새롭게 생성되여 캐쉬 봉사기가 아닌 봉사기로 직접 자료를 요청하게 된다.<br>
				아이디 값을 지정하면 랜덤한 아이디 값으로 콜백 함수 이름을 생성하지 않으므로<br>
				캐쉬 봉사기를 사용하여 그에 대한 히트율을 높이고자 할 때 아이디를 지정하여 사용할수 있다.
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td>
				요청 헤더를 전송할지 여부.<br>
				type 이 "flash" 일 때 사용하며, 봉사기에서 접근 권한을 설정하는 crossdomain.xml 에<br>
				...
				그래서 allow-header가 설정되여 있지 않다면 반드시 false로 설정해야 한다.<br>
		...
		<tr>
			...
			<td>Boolean</td>
			<td>
				type 이 "flash" 일 때 사용하며, 요청한 자료 안에 utf-8 이 아닌 다른 인코딩이 되여 있을때 false 로 지정한다.<br>
		...
		<tr>
			...
			<td>Boolean</td>
			<td>
				요청 시 봉사기로 전달할 자료를 Body 령역에 전달할 지의 여부.<br>
 ...
 *
 * @example
// 'Get List' 버튼 클릭 시, 봉사기에서 자료를 받아와 리스트를 구성하는 례제
// (1) 봉사기 페지와 봉사 페지가 같은 도메인에 있는 경우 - xhr
 ...
 * @example
// 'Get List' 버튼 클릭 시, 봉사기에서 자료를 받아와 리스트를 구성하는 례제
// (2) 봉사기 페지와 봉사 페지가 같은 도메인에 있는 경우 - iframe
		...
		<script type="text/javascript" language="javascript">
			function getList() {
				var oAjax = new $Ajax('http://server.com/some/some.php', {
					type : 'iframe',
					method : 'get',			// GET 방식으로 통신
											// POST로 지정하면 원격 프록시 화일에서 some.php 로 요청 시에 POST 방식으로 처리
					onload : function(res){	// 요청이 완료되면 실행될 콜백 함수
						$('list').innerHTML = res.text();
					},
					// 로컬 프록시 화일의 경로.
					// 반드시 정확한 경로를 지정해야 하며, 로컬 도메인의 경로라면 어디에 두어도 상관 없음
					// (※ 원격 프록시 화일은 반드시  원격 도메인 봉사기의 도메인 루트 상에 두어야 함)
 ...
 * @example
// 'Get List' 버튼 클릭 시, 봉사기에서 자료를 받아와 리스트를 구성하는 례제
// (3) 봉사기 페지와 봉사 페지가 같은 도메인에 있는 경우 - jsonp
		...
		<script type="text/javascript" language="javascript">
			function getList(){
				var oAjax = new $Ajax('http://server.com/some/some.php', {
					...
					callbackname: 'ajax_callback_fn'	// 봉사기에서 사용할 콜백 함수이름을 가지는 매개 변수 이름 (생략 시 '_callback')
				});
				oAjax.request();
			}
		</script>
 ...
 * @example
// 'Get List' 버튼 클릭 시, 봉사기에서 자료를 받아와 리스트를 구성하는 례제
// (4) 봉사기 페지와 봉사 페지가 같은 도메인에 있는 경우 - flash
		...
		<script type="text/javascript" language="javascript">
			function getList(){
				var oAjax = new $Ajax('http://server.com/some/some.php', {
					...
					decode : true,			// 요청한 자료 안에 utf-8 이 아닌 다른 인코딩이 되여 있을때 false. (생략 시 true)
					onload : function(res){	// 요청이 완료되면 실행될 콜백 함수
						$('list').innerHTML = res.text();
					},
				});
 ...
 * request 메서드는 Ajax 요청을 봉사기에 전송한다.<br>
 * 요청에 사용할 매개 변수는 $Ajax 생성자에서 설정하거나 option 메서드에서 변경할수 있다.<br>
 *
 * @remark 요청 형식(type)이 "flash" 이면 이 메소드를 실행하기 전에 body 태그 내부에서 $Ajax.SWFRequest.write() 명령어를 반드시 실행해야 한다.
 *
 * @param {Object} oData 봉사기로 전송할 자료.
 * @return {$Ajax} $Ajax 객체
 * @description [Lite]
 * @example
 *
...
ajax.request( {key1:"value1", key2:"value2"} );	// 봉사기에 전송할 자료를 매개변수로 넘긴다.
...
XHR GET 방식 요청인 경우 URL에 파라메터 추가
 ...
 * @return {Boolean} 현재 대기 중이면 true 를, 그렇지 않으면 false를 반환한다.
 ...
 * abort 메서드는 봉사기로 전송한 Ajax 요청을 취소한다. Ajax 요청의 응답 시간이 길거나 강제로 Ajax 요청을 취소할 경우 사용한다.
 ...
 * 이름과 값을 원소로 가지는 객체를 사용하면 두 개 이상의 정보를 한 번에 설정할수 있다.
 *
 * @param {String | Object} name <br>
 * 매개 변수의 형식은 문자렬 혹은 객체를 사용한다.<br>
 * <br>
 * 문자렬을 매개 변수로 사용하면 다음과 같이 동작한다..<br>
 * 1. value 매개 변수를 생략하면 이름에 해당하는 $Ajax의 속성 값을 반환한다.<br>
 * 2. value 매개 변수를 설정하면 이름에 해당하는 $Ajax의 속성에 value를 값으로 설정한다.<br>
 * <br>
 * 객체인 경우에는 속성 이름으로 정보를 찾아 속성의 값으로 설정한다. 객체에 두 개 이상의 속성을 지정하면 여러 속성 값을 한 번에 설정할수 있다.
 * @param {String}  [value] 새로 설정할 정보의 값. name 매개 변수가 문자렬인 경우에만 사용된다.
 * @return {String|$Ajax}  정보의 값을 가져올 때는 문자렬을, 값을 설정할 때는 $Ajax 객체를 반환한다.
 * @description [Lite]
 * @example
var ajax = $Ajax("http://www.remote.com", {
	type : "xhr",
...
var request_type = ajax.option("type");					// type 인 xhr 을 반환한다.
 ...
 * 헤더를 설정하려면 헤더의 이름과 값을 각각 매개 변수로 사용하거나, 헤더의 이름과 값을 원소로 가지는 객체를 매개 변수로 사용한다. 만약 여러 개의 속성을 가진 객체를 사용하면 두 개 이상의 헤더를 한 번에 설정할수 있다.<br>
 ...
 * <br>
 * 문자렬을 매개 변수로 사용하면 다음과 같이 동작한다. <br>
 * 1. value 매개 변수를 생략하면 HTTP 요청 헤더에서 문자렬과 일치하는 속성값을 찾는다.<br>
 * 2. value 매개 변수를 설정하면 HTTP 요청 헤더에서 문자렬과 일치하는 속성에 value를 값으로 설정한다.<br>
 * <br>
 * 객체인 경우에는 속성 이름으로 정보를 찾아 속성의 값으로 설정한다. 객체에 두 개 이상의 속성을 지정하면 HTTP 요청 헤더에서 여러 속성 값을 한 번에 설정할수 있다.
 * @param {Value} [value] 설정할 헤더 값. name 매개 변수가 문자렬인 경우에만 사용된다.
 * @return {String|$Ajax} 정보의 값을 가져올 때는 문자렬을, 값을 설정할 때는 $Ajax 객체를 반환한다.
 * @description [Lite]
 * @example
 ...
 * @class $Ajax.Response 객체를 생성하여 반환한다.<br>
...
var oAjax = new $Ajax('some.xml', {
	...
	method : 'get',
	onload : function(res){
		var elData = cssquery.getSingle('data', res.xml());	// 응답을 XML 객체로 반환한다
 ...
 * 응답을 문자렬로 반환한다.
 * @return {String} 응답 문자렬. XHR의 responseText 와 유사하다.
 * @description [Lite]
 *
 * @example
// some.php
...
var oAjax = new $Ajax('some.xml', {
	...
	method : 'get',
	onload : function(res){
		$('list').innerHTML = res.text();	// 응답을 문자렬로 반환한다
	},
}).request();
 ...
 * 응답 문자렬을 자동으로 JSON 객체로 변환하여 반환한다. 변환 과정에서 오유가 발생하면 빈 객체를 반환한다.
 * @description [Lite]
 *
 * @example
// some.php
...
var oAjax = new $Ajax('some.php', {
	...
	onload : function(res){
		var welList = $Element('list').empty();
		var jsonData = res.json();	// 응답을 JSON 객체로 반환한다
 ...
 * 응답 헤더를 가져온다. 매개 변수를 전달하지 않으면 헤더 전체를 반환한다.
 * @param {String} name 가져올 응답 헤더의 이름
 * @return {String|Object} 매개 변수가 있을 때는 해당하는 헤더 값을, 그렇지 않으면 헤더 전체를 반환한다.
 * @description [Lite]
 *
 * @example
var oAjax = new $Ajax('some.php', {
	...
	method : 'get',
	onload : function(res){
		res.header();					// 응답 헤더 전체를 반환한다.
		res.header("Content-Length")	// 응답 헤더에서 "Content-Length" 의 값을 반환한다.
	},
}).request();
 ...
 * @fileOverview $Ajax의 확장 메서드를 정의한 화일
 * @name Ajax.extend.js
 ...
 * Ajax 요청 형식 별로 요청 객체를 생성하기 위한 상위 객체로 사용한다.
 ...
 * $Ajax.JSONPRequest 객체를 생성하여 반환한다.
 * @extends $Ajax.RequestBase
 * @class Ajax 요청 형식(type)이 jsonp 인 요청 객체를 생성하며, jindo.$Ajax 생성자에서 요청 객체 생성 시 사용한다.
 ...
 * $Ajax.SWFRequest 객체를 생성하여 반환한다.
 * @extends $Ajax.RequestBase
 * @class Ajax 요청 형식(type)이 flash 인 요청 객체를 생성하며, jindo.$Ajax 생성자에서 요청 객체 생성 시 사용한다.
...
jindo.$Ajax.SWFRequest = jindo.$Class({
 ...
 자료 안에 utf-8이 아닌 다른 인코딩일때 디코딩을 안하고 바로 text에 저장.
 ...
 * Ajax 요청 형식(type)이 flash 일 때 request 메소드가 호출되기 전에 반드시 한 번 실행해야 한다.<br>
 * <br>
 * 요청 형식이 flash 이면 모든 통신은 플래시 객체를 통하여 주고 받으며 Ajax 호출을 하기 전에 반드시 플래시 객체를 초기화해야한다.<br>
 * $Ajax.SWFRequest.write 메서드를 호출하면 통신을 위한 플래시 객체를 문서 내에 추가한다.
 *
 * @param {String} [swf_path] 통신을 담당할 플래시 화일의 경로. 생략하면 기본 값은 "./ajax.swf" 이다.
 *
 * @remark 해당 메서드는 <body> 태그 내에 작성되여야 한다.
...
//		throw new Error("Check your flash file!. Unload flash on a page.");
		// [SMARTEDITORSUS-334] Flash 를 지원하지 않는 환경(ex. iOS)에서 경고창을 띄우지 않도록 림시적으로 주석 처리함
 ...
 * $Ajax.FrameRequest 객체를 생성하여 반환한다.
 * @extends $Ajax.RequestBase
 * @class Ajax 요청 형식(type)이 iframe 인 요청 객체를 생성하며, jindo.$Ajax 생성자에서 요청 객체 생성 시 사용한다.
 ...
 * $Ajax 객체를 순서대로 호출할수 있는 기능을 제공한다.
 * @class $Ajax.Queue는 Ajax 요청을 큐에 담아 큐에 들어온 순서대로 처리한다.
 * @param {Object} option $Ajax.Queue 에서 봉사기에 요청할 때 사용하는 다양한 정보를 정의한다.
	<ul>
		<li>
 ...
 * @return {Value | $Ajax.Queue} 값을 가져오는 경우 해당 값을, 설정하는 경우 $Ajax.Queue 객체를 반환한다.
 * @example
var oAjaxQueue = new $Ajax.Queue({
	useResultAsParam : true
});

oAjaxQueue.option("useResultAsParam");	// useResultAsParam 옵션 값인 true 를 반환한다.
 ...
 * @fileOverview $H의 생성자 및 메서드를 정의한 화일
 * @name hash.js
 ...
 * $H 해시 객체를 반환한다
 * @class $H 클라스는 키와 값을 원소로 가지는 렬거형 배렬인 해시를 구현하고, 해시를 다루기 위한 여러 가지 위한 메서드를 제공한다.
 ...
 * @remark 필터 콜백 함수의 결과가 true인 원소만 수집한다. 콜백 함수는 형식은 례제를 참고한다.
 * @example
 ...
 * map 메서드는 해시 객체의 원소를 인수로 콜백 함수를 실행하고, 함수의 반환 값을 해당 원소의 값으로 지정한다.
 * @param {Function} callback 콜백 함수
 * @param {Object} thisObject 콜백 함수의 this
 * @return {$H} 값을 변경한 해시 객체
 * @remark 콜백 함수는 형식은 례제를 참고한다.
 * @example
function callback(value, key, object ) {
   // value    해시의 값
   // key      해시의 고유한 키 혹은 이름
 ...
 * @param {String} value 해시 테블에 추가할 값
 ...
 * remove 메서드는 해시 테블의 원소를 제거한다.
 * @param {String} key 제거할 원소의 키
 * @return {void} 제거한 키 값
 * @example
var h = $H({one:"first", two:"second", three:"third"});
h.remove ("two");
// h의 해시 테블은 {one:"first", three:"third"}
 ...
 * search 메서드는 해시 테블에서 인수로 지정한 값을 찾는다.
 ...
 * hasKey 메서드는 해시 테블에 인수로 지정한 건이 있는지 찾는다.
 * @param {String} key 해시 테블에서 검색할 키
 ...
 * hasValue 메서드는 해시 테블에 인수로 지정한 값이 있는지 확인한다.
 * @param {String} value 해시 테블에서 검색할 값
 ...
 * keys 메서드는 해시 키의 배렬을 반환한다.
 * @return {Array} 해시 키의 배렬
 * @example
var h = $H({one:"first", two:"second", three:"third"});
h.keys ();
// ["one", "two", "three"]
 ...
 * values 메서드는 해시 값의 배렬을 반환한다.
 * @return {Array} 해시 값의 배렬
 * @example
var h = $H({one:"first", two:"second", three:"third"});
h.values();
// ["first", "second", "third"]
 ...
 * @remark forEach, filter, map와 같은 루프를 중단한다. 강제로 exception을 발생시키므로 try ~ catch 령역에서 이 메소드를 실행하면 정상적으로 동작하지 않을 수 있다.
 * @example
 ...
 * @remark forEach, filter, map와 같은 루프 실행 도중에 현재 루프를 중단하고 다음으로 넘어간다. 강제로 exception을 발생시키므로 try ~ catch 령역에서 이 메소드를 실행하면 정상적으로 동작하지 않을 수 있다.
 * @example
$H({a:1, b:2, c:3}).forEach(function(v,k,o) {
   ...
   if (v % 2 == 0) $H.Continue();
 ...
 * @fileOverview $Json의 생성자 및 메서드를 정의한 화일
 * @name json.js
 ...
 * @param {Object | String} sObject 객체, 혹은 JSON으로 인코딩 가능한 문자렬.
 * @return {$Json} 인수를 인코딩한 $Json 객체.
 * @remark XML 형태의 문자렬을 사용하여 $Json 객체를 생성하려면 $Json#fromXML 메서드를 사용한다.
 * @example
 ...
 * fromXML 메서드는 XML 형태의 문자렬을 $Json 객체로 인코딩한다.
 * @param {String} sXML $Json  객체로 인코딩할 XML 형태의 문자렬
 ...
 * get 메서드는 path에 해당하는 $Json 객체의 값을 반환한다.
 * @param {String} sPath path 문자렬
 * @return {Array} 지정된 path에 해당하는 value를 원소로 가지는 배렬
 * @example
 ...
 * toString 메서드는 $Json 객체를 JSON 문자렬로 반환한다.
 * @return {String} JSON 문자렬
 * @example
 ...
 * toXML 메서드는 $Json 객체를 XML 형태의 문자렬로 반환한다.
 * @return {String} XML 형태의 문자렬
 * @example
 ...
 * toObject 메서드는 $Json 객체 원래의 JSON 자료 객체를 반환한다.
 * @return {Object} 원래의 자료 객체
 * @example
var json = $Json({foo:1, bar: 31});
json.toObject();
 ...
 * @fileOverview $Cookie의 생성자 및 메서드를 정의한 화일
 * @name cookie.js
 ...
 * $Cookie 객체를 생성한다.
 * @class $Cookie 클라스는 쿠키(Cookie)를 추가, 수정, 혹은 삭제하거나 쿠키의 값을 가져온다.
 * @constructor
 ...
 * keys 메서드는 쿠키 이름을 원소로 가지는 배렬을 반환한다.
 * @return {Array} 쿠키 이름을 원소로 가지는 배렬
 * @example
var cookie = $Cookie();
 ...
 * get 메서드는 쿠키 이름에 해당하는 쿠키 값을 가져온다. 값이 존재하지 않는다면 null을 반환한다.
 ...
 * @param {Number} [nDays] 쿠키 유효 시간. 유효 시간은 일단위로 설정한다. 유효시간을 생략했다면 쿠키는 웨브 브라우저가 완료되면 없어진다.
...
/**
  ...
  * @fileOverview $Element의 생성자 및 메서드를 정의한 화일
  * @name element.js
 ...
 * @class $Element 클라스는 HTML 요소를 래핑(wrapping)하며, 요소를 다루기 위한 메서드를 제공한다.<br>
 ...
 * <br>
 * $Element 는 문자렬, HTML 요소, 혹은 $Element 를 매개 변수로 지정할수 있다.<br>
 * <br>
 * 매개 변수가 문자렬이면 두 가지 방식으로 동작한다.<br>
 * 만일 "&lt;tagName&gt;" 과 같은 형식의 문자렬이면 tagName을 가지는 객체를 생성한다.<br>
 * 그 이외의 경우 문자렬을 id 로 하는 HTML 요소를 사용하여 $Element 를 생성한다.<br>
 * <br>
 * 매개 변수가 HTML 요소이면 HTML 요소를 래핑하여 $Element 를 생성한다.<br>
 * <br>
 * 매개 변수가 $Element 이면 전달된 매개 변수를 그대로 반환하며 undefined 혹은 null 인 경우에는 null 을 반환한다.
 * @return {$Element} 생성된 $Element 객체
 *
 * @example
var element = $Element($("box"));	// HTML 요소를 매개 변수로 지정
var element = $Element("box");		// HTML 요소의 id를 매개 변수로 지정
var element = $Element("<DIV>");	// 태그를 매개 변수로 지정, DIV 요소를 생성하여 래핑함          
 ...
 * @return {HTML Element} 래핑된 원래의 요소
 * @description [Lite]
 *
 * @example
var element = $Element("sample_div");
element.$value(); // 원래의 요소가 반환된다
 ...
 * HTML 요소의 display 속성을 확인하거나 display 속성을 설정하기 위해 사용한다.
 *
 * @param {Boolean} [bVisible] 화면에 보여줄지의 여부<br>
 * 매개 변수를 생략하면 HTML 요소의 현재 display 속성을 확인하여 true/false로 반환한다.(none 이면 false)<br>
 ...
 *
 * @description [Lite]
 * @since 1.1.2부터 설정 기능을 사용할수 있다.
 * @since 1.4.5부터 bVisible 매개 변수의 값이 true 인 경우 sDisplay 매개 변수의 값으로 display 속성을 설정할수 있다.
 * @see $Element#show
 * @see $Element#hide
 * @see $Element#toggle
 *
 ...
 * HTML 요소가 화면에 보이도록 display 속성을 변경한다.
...
IE에서 sDisplay값이 비정상적일때 block로 설정한다.
 ...
 * HTML 요소가 화면에 보이지 않도록 display 속성을 none 으로 변경한다.
 ...
 * HTML 요소의 display 속성을 변경하여 요소를 화면에 보이거나, 보이지 않게 한다.
 ...
 * css 메서드는 문자렬, Object, 혹은 $H 를 매개 변수로 지정할수 있다.<br>
 * <br>
 * 매개 변수가 문자렬이면 조회 혹은 수정할 속성 명으로 아래 두 가지 방식으로 동작한다.<br>
 * 두 번째 매개 변수인 sValue 매개 변수를 생략하면 CSS 속성의 값을 가져온다.<br>
 * 두 번째 매개 변수인 sValue 매개 변수에 값이 있으면 CSS 속성의 값을 sValue 값으로 설정한다.<br>
 * <br>
 * Object 혹은 $H 객체를 사용하면 두 개 이상의 CSS 속성을 한꺼번에 설정할수 있다.<br>
 * 객체의 속성 이름으로 CSS 속성을 찾아 속성 값으로 설정한다.
 * @param {String | Number} [sValue] CSS 속성에 설정할 값.<br>
 * 단위가 필요한 값은 수자 혹은 단위를 포함한 문자렬을 사용한다.
 * @return {String | $Element} 값을 가져올 때는 값을 문자렬로 반환하고, 값을 설정할 때는 값을 설정한 현재의 $Element 를 반환한다.
 * @description [Lite]
 * @example
<style type="text/css">
	#btn {
 ...
 * DOM 요소의 HTML 속성을 가져오거나 설정한다.
 * 하나의 인자만 사용하면 해당 HTML 속성의 속성 값을 가져온다. 해당 속성이 없다면 null을 반환한다.
 * 두 개의 인자를 사용하면 첫번째 인자에 해당하는 HTML 속성의 속성 값을 두 번째 인자의 값으로 설정한다.
 * 첫번째 인자로 Object 혹은 $H 객체를 사용하면 두 개 이상의 HTML 속성을 한꺼번에 정의할수 있다.
 * @param {String|Object|$H} sName HTML 속성 이름 혹은 설정값 객체
 * @param {String|Number} [sValue] 설정 값. 설정 값을 null로 지정하면 해당 HTML 속성을 삭제한다.
 * @return {String|$Element} 값을 가져올 때는 String 설정 값을, 값을 설정할 때는 값을 설정한 현재의 $Element를 반환한다.
 * @description [Lite]
 *
 * @example
 ...
 * HTML 요소의 너비를 가져오거나 설정한다.
 * @remark width 메서드는 HTML 요소의 실제 너비를 가져온다. 브라우저마다 Box 모델의 크기 계산 방법이 다르므로 CSS의 width 속성 값과 width 메서드의 반환 값은 서로 다를 수 있다.
 * @param {Number} [width]	설정할 너비 값. 단위는 px 이며 매개 변수의 값은 수자로 지정한다.<br>
 * 매개 변수를 생략하면 너비 값을 가져온다.
 * @return {Number | $Element} 값을 가져오는 경우에는 HTML 요소의 실제 너비를, 값을 설정하는 경우에는 너비 값이 변경된 현재의 $Element 객체를 반환한다.
 * @description [Lite]
 * @see $Element#height
 *
 * @example
 ...
 * HTML 요소의 높이를 가져오거나 설정한다.
 * @remark height 메서드는 HTML 요소의 실제 높이를 가져온다. 브라우저마다 Box 모델의 크기 계산 방법이 다르므로 CSS의 height 속성 값과 height 메서드의 반환 값은 서로 다를 수 있다.
 * @param {Number} [height]	설정할 높이 값. 단위는 px 이며 매개 변수의 값은 수자로 지정한다.
 * @return {Number | $Element} 값을 가져오는 경우에는 HTML 요소의 실제 높이를, 값을 설정하는 경우에는 높이 값이 변경된 현재의 $Element 객체를 반환한다.
 * @description [Lite]
 * @see $Element#width
 *
 * @example
 ...
 * HTML 요소의 클라스명을 설정하거나 반환한다.
 * @param {String} [sClass] 설정할 클라스명<br>
 * 매개 변수를 생략하면 HTML 요소의 현재의 클라스명을 반환한다.<br>
 * 매개 변수를 지정하면 클라스명을 설정한다. 여러개의 클라스명을 설정하려면 공백으로 구분한다.
 * @return {String | $Element} 클라스명을 조회하는 경우에는 클라스명을 반환한다.<br>
 * 클라스가 두 개 이상이면 클라스명을 공백으로 구분하여 공백을 포함한 문자렬로 반환한다.<br>
 * 클라스명을 설정한 경우에는 변경된 현재의 $Element 객체를 반환한다.
 * @description [Lite]
 ...
 * HTML 요소에서 특정 클라스를 사용하고 있는지 확인한다.
 * @param {String} sClass 확인할 클라스명
 * @return {Boolean} 클라스 사용 여부
 * @description [Lite]
 ...
 * HTML 요소에 클라스를 추가한다.
 * @param {String} sClass 추가할 클라스명. 여러개의 클라스명을 추가하려면 공백으로 구분한다.
 * @return {$Element} 클라스가 추가된 현재의 $Element 객체
 * @description [Lite]
 * @see $Element#className
 * @see $Element#hasClass
 * @see $Element#removeClass
 ...
 * HTML 요소에서 특정 클라스를 제거한다.
 * @param {String} sClass 제거할 클라스명. 여러개의 클라스명을 제거하려면 공백으로 구분한다.
 * @return {$Element} 클라스가 제거된 현재의 $Element 객체
 * @description [Lite]
 * @see $Element#className
 * @see $Element#hasClass
 * @see $Element#addClass
 ...
 * HTML 요소에 클라스가 이미 적용되여 있으면 제거하고 만약 없으면 추가한다.
 * @param {String} sClass 추가 혹은 제거할 클라스명
 * @param {String} [sClass2] 추가 혹은 제거할 클라스명<br>
 * sClass2 매개 변수를 생략하면 HTML 요소에서 sClass 매개 변수 값의 클라스를 사용 중인지 확인한다.<br>
 * 만약 사용하고 있다면 해당 클라스를 제거하고, 사용하고 있지 않다면 추가한다.<br>
 * 두 개의 매개 변수를 모두 사용하면 두 클라스 중에서 사용하고 있는 것을 제거하고 나머지를 추가한다.
 * @return {$Element} 클라스가 추가 혹은 제거된 현재의 $Element 객체
 ...
 * HTML 요소의 내부 HTML(innerHTML) 을 가져오거나 설정한다.
 * @param {String} [sHTML] 설정할 HTML 문자렬<br>
 * 매개 변수를 생략하면 내부 HTML 을 조회하고, 매개 변수를 지정하면 매개 변수의 값으로 내부 HTML을 변경한다.
 * @return {String | $Element} 값을 조회하는 경우에는 HTML 요소의 내부 HTML을 반환하고,<br>
...
jindo.$Element.prototype.html = function(sHTML) {
	...
	IE 나 FireFox 의 일부 상황에서 SELECT 태그나 TABLE, TR, THEAD, TBODY 태그에 innerHTML 을 설정해도
...
* ie에서 select테그일 경우 option중 selected가 되여 있는 option이 있는 경우 중간에
* selected가 되여 있으면 그 다음 부터는 계속 selected가 true로 되여 있어
* 해결하기 위해 cloneNode를 리용하여 option을 카피한 후 selected를 변경함. - mixed
 ...
 * HTML 요소의 외부 HTML(outerHTML) 을 반환한다.
 ...
 * HTML 요소를 HTML 문자렬로 변환하여 반환한다. (outerHTML 메서드와 동일)
 * @return {String} 외부 HTML
 * @see $Element#outerHTML
 ...
 * @fileOverview $Element의 확장 메서드를 정의한 화일
 ...
 * HTML 요소를 서서히 나타나게 한다. (Fade-in 효과)
 *
 * @param {Number} duration HTML 요소가 완전히 나타날 때까지 걸리는 시간. 단위는 초를 사용한다.
 * @param {Function} [callback] HTML 요소가 완전히 나타난 후에 실행할 콜백 함수.
 * @return {$Element} 현재의 $Element 객체
 *
 * @remark IE6 에서 filter 사용 시 해당 요소가 position 속성을 가지고 있으면 사라지는 문제가 있기 때문에 HTML 요소에 position 속성이 없어야 한다.
 ...
 * HTML 요소를 서서히 사라지게 한다. (Fade-out 효과)<br>
 * HTML 요소가 완전히 사라지면 요소의 display 속성은 none 으로 변한다.
 *
 * @param {Number} duration HTML 요소 완전히 사라질 때까지 걸리는 시간. 단위는 초를 사용한다.
 * @param {Function} [callback] HTML 요소가 완전히 사라진 후에 실행할 콜백 함수.
 * @return {$Element} 현재의 $Element 객체
 *
 * @remark webkit기반의 브라우저(safari5+, mobile safari, chrome, mobile webkit), opera10.60+ 에서는 CSS3 transition을 사용한다. 그 이외의 브라우저에서는 자바스크립트를 사용한다.
 *
 ...
 * HTML 요소의 위치를 가져오거나 설정한다.<br>
 * <br>
 * 매개 변수를 생략하면 위치 값을 가져온다.<br>
 * 매개 변수를 지정하면 HTML 요소의 위치를 설정한다.<br>
 * 기준점은 브라우저 문서의 왼쪽 상단이다.
 *
 * @param {Number} [nTop] 문서의 맨 위에서 HTML 요소 맨 위까지의 거리. 단위는 px
 * @param {Number} [nLeft] 문서의 왼쪽 가장자리에서 HTML 요소 왼쪽 가장자리까지의 거리. 단위는 px
 * @return {$Element | Object} 위치를 설정하면 위치 값이 변경된 $Element 객체를 반환하고,<br>
 * 위치를 가져오면 HTML 요소의 top, left 위치 값을 객체로 반환한다.
 *
 * @remark HTML 요소가 보이는 상태에서 적용해야 한다. 요소가 화면에 보이지 않으면 offset 의 사용이 정확하지 않다.
 * @remark 일부 브라우저와 일부 상황에서 inline 요소에 대한 위치가 올바르게 얻어지지 않는 문제가 있으며 이 경우 해당 요소를 position:relative; 로 바꿔주는 식으로 해결할수 있다.
 * @author Hooriza
 *
 * @example
<style type="text/css">
 ...
 * 문자렬 내의 JavaScript를 실행한다.<br>
 * &lt;script&gt; 태그가 포함된 문자렬을 매개 변수로 지정하면 &lt;script&gt; 안에 있는 내용을 파싱하여 eval 를 수행한다.
 *
 * @param {String} sHTML &lt;script&gt; 태그가 포함된 HTML 문자렬
 * @return {$Element} 현재의 $Element 객체를 반환
 *
 * @example
// script 태그가 포함된 문자렬을 지정
 ...
 * HTML 요소에 마지막 자식 노드를 추가한다.
 *
 * @param {$Element | HTML Element | String} oElement 추가할 HTML 요소. 문자렬, HTML 요소, 혹은 $Element 을 매개 변수로 지정할수 있다.<br>
 * <br>
 * 매개 변수가 문자렬이면 해당 문자렬을 id 로 하는 HTML 요소를 마지막 자식노드로 추가한다.<br>
 * 매개 변수가 HTML 요소이면 해당 요소를 마지막 자식노드로 추가한다.<br>
 * 매개 변수가 $Element 이면 $Element 객체 내부의 HTML 요소를 마지막 자식노드로 추가한다.
 ...
 *
 * @example
// id 가 sample1 인 HTML 요소에
// id 가 sample2 인 HTML 요소를 추가
 ...
 * HTML 요소에 첫번째 자식 노드를 추가한다.
 *
 * @param {$Element | HTMLElement | String} oElement 추가할 HTML 요소. 문자렬, HTML 요소, 혹은 $Element 을 매개 변수로 지정할수 있다.<br>
 * <br>
 * 매개 변수가 문자렬이면 해당 문자렬을 id 로 하는 HTML 요소를 첫번째 자식노드로 추가한다.<br>
 * 매개 변수가 HTML 요소이면 해당 요소를 첫번째 자식노드로 추가한다.<br>
 * 매개 변수가 $Element 이면 $Element 객체 내부의 HTML 요소를 첫번째 자식노드로 추가한다.
 * @return {$Element} $Element 객체 자신
 *
 * @see $Element#append
 * @see $Element#before
 ...
 *
 * @example
// id 가 sample1 인 HTML 요소에서
// id 가 sample2 인 HTML 요소를 첫번째 자식노드로 이동
 ...
 * $Element 객체 내부의 HTML 요소를 매개 변수로 지정한 요소로 대체한다.
 *
 * @param {$Element | HTML Element | String} oElement 대체할 HTML 요소. 문자렬, HTML 요소, 혹은 $Element 을 매개 변수로 지정할수 있다.<br>
 * <br>
 * 매개 변수가 문자렬이면 해당 문자렬을 id 로 하는 HTML 요소로 대체한다.<br>
 * 매개 변수가 HTML 요소이면 해당 요소로 대체한다.<br>
 * 매개 변수가 $Element 이면 $Element 객체 내부의 HTML 요소로 대체한다.
 * @return {$Element} HTML 요소가 대체 된 $Element 객체를 반환
 *
 * @example
// id 가 sample1 인 HTML 요소에서
// id 가 sample2 인 HTML 요소로 대체
 ...
 * HTML 요소를 다른 HTML 요소의 마지막 자식노드로 추가한다.
 *
 * @param {$Element | HTML Element | String} oElement 부모가 될 HTML 요소. <br>
 * <br>
 * 문자렬, HTML 요소, 혹은 $Element 을 매개 변수로 지정할수 있다.<br>
 * <br>
 * 매개 변수가 문자렬이면 해당 문자렬을 id 로 하는 HTML 요소를 부모로 한다.<br>
 * 매개 변수가 HTML 요소이면 해당 요소를 부모로 한다.<br>
 * 매개 변수가 $Element 이면 $Element 객체 내부의 HTML 요소를 부모로 한다.
 * @return {$Element} 인자로 받은 요소
 *
 * @see $Element#append
 * @see $Element#prepend
 * @see $Element#after
 ...
 *
 * @example
// id 가 sample2 인 HTML 요소에
// id 가 sample1 인 HTML 요소를 추가
 ...
 * HTML 요소를 다른 HTML 요소의 첫번째 자식노드로 추가한다.
 *
 * @param {$Element | HTML Element | String} oElement 부모가 될 HTML 요소. 문자렬, HTML 요소, 혹은 $Element 을 매개 변수로 지정할수 있다.<br>
 * <br>
 * 매개 변수가 문자렬이면 해당 문자렬을 id 로 하는 HTML 요소를 부모로 한다.<br>
 * 매개 변수가 HTML 요소이면 해당 요소를 부모로 한다.<br>
 * 매개 변수가 $Element 이면 $Element 객체 내부의 HTML 요소를 부모로 한다.
 ...
 * HTML 요소 바로 앞에 HTML 요소를 추가한다.
 *
 * @param {$Element | HTML Element | String} oElement 추가할 HTML 요소.<br>
 * <br>
 * 문자렬, HTML 요소, 혹은 $Element 을 매개 변수로 지정할수 있다.<br>
 * <br>
 * 매개 변수가 문자렬이면 해당 문자렬을 id 로 하는 HTML 요소를 추가한다.<br>
 * 매개 변수가 HTML 요소이면 해당 요소를 추가한다.<br>
 * 매개 변수가 $Element 이면 $Element 객체 내부의 HTML 요소를 추가한다.
 ...
 *
 * @example
// id 가 sample1 인 HTML 요소 앞에
// id 가 sample2 인 HTML 요소를 추가 함
 ...
 * HTML 요소의 상위 요소 노드를 검색한다.
 *
 * @param {Function} [pFunc] 상위 요소 노드의 검색 조건을 지정한 콜백 함수.<br>
 * 매개 변수를 생략하면 부모 요소 노드를 반환하고,<br>
 * 매개 변수로 콜백 함수를 지정하면 콜백 함수의 실행 결과가 true 인 상위 요소 노드의 배렬을 반환한다.<br>
 * 콜백 함수에 매개 변수로 탐색 중인 상위 요소 노드의 $Element 객체를 넘긴다.
 * @param {Number} [limit] 탐색할 상위 요소 노드의 뎁스.<br>
 * 매개 변수를 생략하면 모든 상위 요소 노드를 탐색한다.<br>
 * pFunc 매개 변수를 null로 설정하고 limit 매개 변수를 설정하면 제한된 뎁스의 상위 요소 노드를 조건 없이 검색한다.
 * @return {$Element | Array} 부모 요소 노드 혹은 상위 요소 노드의 배렬.<br>
 * 매개 변수를 생략하여 부모 요소 노드를 반환하는 경우 $Element 형식으로 반환한다.<br>
 * 그 이외에는 검색된 요소 노드를 $Element의 배렬로 반환한다.
 *
 * @see $Element#child
 * @see $Element#prev
 * @see $Element#next
...
<script type="text/javascript">
	...
	// id가 div1인 DIV를 래핑한 $Element를 원소로 하는 배렬을 반환

	parent = welTarget.parent(function(v){
	        return v.hasClass("sample");
	    }, 1);
	// id가 div3인 DIV를 래핑한 $Element를 원소로 하는 배렬을 반환
</script>
 ...
 * HTML 요소의 하위 요소 노드를 검색한다.
 *
 * @param {Function} [pFunc] 하위 요소 노드의 검색 조건을 지정한 콜백 함수.<br>
 * 매개 변수를 생략하면 자식 요소 노드의 배렬을 반환하고,<br>
 * 매개 변수로 콜백 함수를 지정하면 콜백 함수의 실행 결과가 true 인 하위 요소 노드의 배렬을 반환한다.<br>
 * 콜백 함수에 매개 변수로 탐색 중인 하위 요소 노드의 $Element 객체를 넘긴다.
 * @param {Number} [limit] 탐색할 하위 요소 노드의 뎁스.<br>
 * 매개 변수를 생략하면 모든 하위 요소 노드를 탐색한다.<br>
 * pFunc 매개 변수를 null로 설정하고 limit 매개 변수를 설정하면 제한된 뎁스의 하위 요소 노드를 조건 없이 검색한다.
 * @return {$Element | Array} 자식 요소 노드  혹은 조건에 맞는 하위 노드의 $Element의 배렬을 반환한다.
 *
 * @see $Element#parent
 * @see $Element#prev
 * @see $Element#next
...
<script type="text/javascript">
	...
	// id가 div7인 DIV를 래핑한 $Element를 원소로 하는 배렬을 반환
 ...
 * HTML 요소의 이전에 나오는 형제 요소 노드를 검색한다.
 *
 * @param {Function} [pFunc] 이전 형제 요소 노드의 검색 조건을 지정한 콜백 함수.<br>
 * 매개 변수를 생략하면 바로 이전의 형제 요소 노드를 반환하고,<br>
 * 매개 변수로 콜백 함수를 지정하면 콜백 함수의 실행 결과가 true 인 형제 요소 노드의 배렬을 반환한다.<br>
 * 콜백 함수에 매개 변수로 탐색 중인 형제 요소 노드를 넘긴다. ($Element 객체가 아님)
 * @return {$Element | Array} 바로 전의 형제 요소 노드를 가리키는 $Element 혹은 조건에 맞는 형제 노드의 $Element의 배렬을 반환한다.
 *
 * @see $Element#parent
 * @see $Element#child
 * @see $Element#next
...
<script type="text/javascript">
	...
	// id가 sample_div3인 DIV를 래핑한 $Element를 원소로 하는 배렬을 반환
</script>
 ...
 * HTML 요소의 다음에 나오는 형제 요소 노드를 검색한다.
 *
 * @param {Function} [pFunc] 다음 형제 요소 노드의 검색 조건을 지정한 콜백 함수.<br>
 * 매개 변수를 생략하면 바로 다음의 형제 요소 노드를 반환하고,<br>
 * 매개 변수로 콜백 함수를 지정하면 콜백 함수의 실행 결과가 true 인 형제 요소 노드의 배렬을 반환한다.<br>
 * 콜백 함수에 매개 변수로 탐색 중인 형제 요소 노드를 넘긴다. ($Element 객체가 아님)
 * @return {$Element | Array} 바로 다음의 형제 요소 노드를 가리키는 $Element 혹은 조건에 맞는 형제 노드의 $Element의 배렬을 반환한다.
 *
 * @see $Element#parent
 * @see $Element#child
 * @see $Element#prev
...
<script type="text/javascript">
	...
	// id가 sample_div8인 DIV를 래핑한 $Element를 원소로 하는 배렬을 반환
</script>
 ...
 * HTML 요소의 첫번째 자식 요소 노드를 반환한다.
 *
 * @return {$Element} 첫번째 자식 요소 노드
 * @since 1.2.0
 *
 * @see $Element#parent
 * @see $Element#child
 ...
 * HTML 요소의 마지막 자식 요소 노드를 반환한다.
 *
 * @return {$Element} 마지막 자식 요소 노드
 * @since 1.2.0
 *
 * @see $Element#parent
 * @see $Element#child
 ...
 * HTML 요소의 부모 요소 노드를 확인한다.
 *
 * @param {HTML Element | String | $Element} element 부모 노드인지 확인할 HTML 요소.<br>
 * <br>
 * 문자렬, HTML 요소, 혹은 $Element 을 매개 변수로 지정할수 있다.<br>
 * <br>
 * 매개 변수가 문자렬이면 해당 문자렬을 id 로 하는 HTML 요소를 확인한다.<br>
 * 매개 변수가 HTML 요소이면 해당 요소를 확인한다.<br>
 * 매개 변수가 $Element 이면 $Element 객체 내부의 HTML 요소를 확인한다.
 * @return {Boolean} 매개 변수가 부모 요소 노드이면 true 를, 그렇지 않으면 false 를 반환한다.
 *
 ...
 * HTML 요소의 자식 요소 노드를 확인한다.
 *
 * @param {HTML Element | String | $Element} element 자식 노드인지 확인할 HTML 요소. 문자렬, HTML 요소, 혹은 $Element 을 매개 변수로 지정할수 있다가매개 변수가 문자렬이면 해당 문자렬을 id 로 하는 HTML 요소를 확인한다.<br>
 * 매개 변수가 HTML 요소이면 해당 요소를 확인한다.<br>
 * 매개 변수가 $Element 이면 $Element 객체 내부의 HTML 요소를 확인한다.
 * @return {Boolean} 매개 변수가 자식 요소 노드이면 true 를, 그렇지 않으면 false 를 반환한다.
 *
 ...
 * 현재의 HTML 요소와 동일한 요소인지 확인한다.
 *
 * @remark DOM3의 API중 isSameNode와 같은 함수로 레퍼런스까지 확인 함수이다.
 * @remark isEqualNode와는 다른 함수이기 때문에 헷갈리지 않도록 한다.
 *
 * @param {HTML Element | String | $Element} element 같은 HTML 요소인지 확인할 HTML 요소. 문자렬, HTML 요소, 혹은 $Element 을 매개 변수로 지정할수 있다.<br>
 * <br>
 * 매개 변수가 문자렬이면 해당 문자렬을 id 로 하는 HTML 요소를 확인한다.<br>
 * 매개 변수가 HTML 요소이면 해당 요소를 확인한다.<br>
 * 매개 변수가 $Element 이면 $Element 객체 내부의 HTML 요소를 확인한다.
 * @return {Boolean} 매개 변수가 같은 HTML 요소이면 true 를, 그렇지 않으면 false 를 반환한다.
 *
 * @example
 ...
 * @return {$Element} 이벤트가 발생한 HTML 요소
 *
 * @since WebKit 계열에서는 이벤트 객체의 keyCode 가 read-only 인 관계로 key 이벤트를 발생시킬 경우 keyCode 값이 설정되지 않는다. 1.4.1 부터 keyCode 값을 설정할수 있다.
 *
 * @example
 ...
 * HTML 요소의 특정 자식 노드를 제거한다. 제거되는 자식 요소 노드의 이벤트 핸들러도 제거한다.
 *
 * @param {HTML Element | String | $Element} oChild 제거할 자식 요소 노드.<br>
 * <br>
 * 문자렬, HTML 요소, 혹은 $Element 을 매개 변수로 지정할수 있다.<br>
 * <br>
 * 매개 변수가 문자렬이면 해당 문자렬을 id 로 하는 HTML 요소를 제거한다.<br>
 * 매개 변수가 HTML 요소이면 해당 요소를 제거한다.<br>
 * 매개 변수가 $Element 이면 $Element 객체 내부의 HTML 요소를 제거한다.
 * @return {$Element} 자식 노드를 모두 제거한 현재의 $Element 객체
 *
 * @see $Element#empty
 * @see $Element#leave
 ...
 * HTML 요소를 부모 요소 노드에서 제거한다.<br>
 * HTML 엘러먼트에 등록된 이벤트 핸들러도 제거한다.
 *
 * @return {$Element} 부모 요소 노드에서 제거 된 현재의 $Element 객체
 *
 * @see $Element#empty
 * @see $Element#remove
 *
 ...
 * HTML 요소를 다른 HTML 요소로 감싼다.
 *
 * @param {String | HTML Element | $Element} wrapper 감쌀 HTML 요소. 문자렬, HTML 요소, 혹은 $Element 을 매개 변수로 지정할수 있다.<br>
 * <br>
 * 매개 변수가 문자렬이면 해당 문자렬을 id 로 하는 HTML 요소를 사용한다.<br>
 * 매개 변수가 HTML 요소이면 해당 요소를 사용한다.<br>
 * 매개 변수가 $Element 이면 $Element 객체 내부의 HTML 요소를 사용한다.
 * @return {$Element} 새로운 HTML 요소로 감싼 $Element 개체
 *
 * @example
 ...
 * HTML 요소의 텍스트 노드가 브라우저에서 한 줄로 보이도록 길이를 조절한다.
 *
 * @remark 이 메서드는 HTML 요소가 텍스트 노드만을 포함한다고 가정한다. 따라서, 이 외의 상황에서의 동작은 보장하지 않는다.
 * @remark 브라우저에서의 HTML 요소의 너비를 기준으로 텍스트 노드의 길이를 정하므로 HTML 요소는 반드시 보이는 상태여야 한다.
 * @remark 화면에 전체 텍스트 노드가 보였다가 줄어드는 경우가 있다. 이런 경우 HTML 요소에서 overflow:hidden 속성을 활용한다.
 *
 * @param {String} [stringTail] 말줄임 표시자. <br>
 * 매개 변수에 지정한 문자렬을 텍스트 노드 맨 끝에 붙이고 텍스트 노드의 길이를 조절한다.<br>
...
<div style="width:300px; border:1px solid #CCCCCC; padding:10px">
	<span id="sample_span">NHN은 검색과 게임을 양축으로 혁신적이고 편리한 온라인 봉사를 꾸준히 선보이며 디지털 라이프를 선도하고 있습니다.</span>
</div>
 ...
 * HTML 요소에서 매개 변수가 몇 번째 자식 요소 노드인지 확인하여 인덱스를 반환한다.
 *
 * @param {String | HTML Element | $Element} element 확인할 HTML 요소. 문자렬, HTML 요소, 혹은 $Element 을 매개 변수로 지정할수 있다.<br>
 * <br>
 * 매개 변수가 문자렬이면 해당 문자렬을 id 로 하는 HTML 요소를 사용한다.<br>
 * 매개 변수가 HTML 요소이면 해당 요소를 사용한다.<br>
 * 매개 변수가 $Element 이면 $Element 객체 내부의 HTML 요소를 사용한다.
 ...
 * HTML 요소에서 특정 CSS 셀렉터(selector)를 만족하는 하위 요소 노드를 찾는다.
 *
 * @param {String} sSelector CSS 셀렉터
 * @return {Array} CSS 셀렉터 조건을 만족하는 HTML 요소의 배렬을 반환한다.<br>
 * 만족하는 HTML 요소가 존재하지 않으면 빈 배렬을 반환한다.
 *
 * @see $Element#query
...
<script type="text/javascript">
	...
	// <div class="pink"></div>와 <div class="pink"></div>를 원소로 하는 배렬을 반환
 ...
 * HTML 요소에서 특정 CSS 셀렉터(selector)를 만족하는 첫번째 하위 요소 노드를 반환한다.
 *
 * @param {String} sSelector CSS 셀렉터
 * @return {HTML Element} CSS 셀렉터 조건을 만족하는 첫번째 HTML 요소.<br>
 * 만족하는 HTML 요소가 존재하지 않으면 null 을 반환한다.
 *
 * @see $Element#test
 * @see $Element#queryAll
 *
...
<script type="text/javascript">
	...
	// 첫번째 <div class="pink"></div> DIV 요소를 반환
 ...
 * HTML 요소에서 특정 CSS 셀렉터(selector)를 만족하는지 확인한다.
 *
 * @param {String} sSelector CSS 셀렉터
 ...
 * HTML 요소를 기준으로 XPath 문법을 사용하여 해당하는 HTML 요소를 가져온다.
 *
 * @remark 지원하는 문법이 무척 제한적으로 특수한 경우에서만 사용하는 것을 권장한다.
 *
 * @param {String} sXPath XPath 문법
 * @return {Array} XPath 에 해당하는 HTML 요소를 원소로 하는 배렬을 반환한다.
 *
 * @example
<div id="sample">
	<div>
 ...
 * HTML 요소 내부 HTML 의 가장 뒤에 HTML 을 덧붙인다.
 *
 * @param {String} sHTML 덧붙일 HTML 문자렬
 ...
 * HTML 요소 내부 HTML 의 가장 앞에 HTML 을 삽입한다.
 *
 * @param {String} sHTML 삽입할 HTML 문자렬
 * @return {$Element} 1.4.8부터 내부 HTML 을 변경한 현재의 $Element 객체를 반환한다.
 * @since 1.4.6부터 사용 가능.
 ...
 * 이벤트 델리게이션이란 이벤트 버블링을 리용하여 효율적으로 이벤트를 관리하는 방법이다.<br>
 ...
 *
 * @param {String | Function} vFilter 원하는 HTML 요소에 대해서만 이벤트를 실행하도록 하기 위한 필터이다.<br>
 * <br>
 * 필터에는 CSS 셀렉터와 함수의 두 가지 형식이 있다.<br>
 * <br>
 * 필터를 CSS 셀렉터를 사용하려면 문자렬을 매개 변수로 지정한다.<br>
 * <br>
 * 필터를 함수로 사용하려면 Boolean 을 반환하는 함수를 매개 변수로 지정한다.<br>
 * 필터 함수의 첫번째 매개 변수는 HTML 요소 자신이고, 두 번째 매개 변수는 이벤트가 발생한 HTML 요소이다.
 *
 * @param {Function} fpCallback 필터에서 true 가 반환된 경우 실행되는 콜백 함수이다.<br>
 * 매개 변수로 이벤트 객체가 지정된다.
 *
 * @return {$Element} $Element 객체를 반환한다.
 * @since 1.4.6부터 사용 가능.
 ...
 * @example
	...
	$Element("parent").delegate("click",
		...
		function(eEvent){	// 콜백 함수
			alert("odd 클라스를 가진 li가 클릭 될 때 실행");
		});
 * @example
	<ul id="parent">
		<li class="odd">1</li>
 ...
 * 이벤트를 바인딩 하는 함수.
 * @param {Element} oEle 요소
 * @param {Boolean} sEvent 이벤트 형식.
 ...
 * 딜리게이션으로 실행되여야 할 함수를 추가 하는 함수.
 * @param {String} sEvent 이벤트 형식.
 * @param {String|Function} vFilter cssquery,function 이렇게 2가지 형식이 들어옴.
 ...
 * 파라메터로 들어오는 요소 부터 자신의 요소 까지의 요소를 구하는 함수.
 * @param {Element} 요소.
 * @returns {Array} 배렬 객체.
 * @ignore
 ...
 * @fileOverview $Fn의 생성자 및 메서드를 정의한 화일
 * @name function.js
 ...
 * $Fn 객체를 반환한다.
 * @extends core
 * @class $Fn 클라스는 자바스크립트 Function 객체의 래퍼(Wrapper) 클라스이다.
 * @constructor
 * @param {Function | String} func
 * <br>
 * Function 객체 혹은 함수의 매개변수를 나타내는 문자렬
 * @param {Object | String} thisObject
 * <br>
 * 함수가 특정 객체의 메서드일 때 해당 객체도 같이 전달한다. 혹은 함수의 몸체를 나타내는 문자렬.
 * @return {$Fn} $Fn 객체
 * @see $Fn#toFunction
 * @description [Lite]
 * @example
 ...
 * bind 메서드는 함수가 객체의 메소드로 동작하도록 묶인 Function 객체를 반환한다.
...
// 함수에서 참조 하는 값들은 해당 함수를 생성 할때의 값이 아니라 함수 실행 시점의 값이 사용 되므로 이때 bind를 리용한다.
for(var i=0; i<2;i++){
	aTmp[i] = function(){alert(i);}
}

for(var n=0; n<2;n++){
	aTmp[n](); // 수자 2만 두번 alert된다.
}
...
for(var n=0; n<2;n++){
	aTmp[n](); // 수자 0, 1이 alert된다.
}

 * @example
//클라스 생성 시 함수를 매개변수로 할 때 scope를 맞춰주기 위해 bind를 사용한다.
var MyClass = $Class({
	fFunc : null,
	$init : function(func){
		this.fFunc = func;
 ...
 * attach 메서드는 함수를 특정 요소의 이벤트 핸들러로 할당한다.<br>
 * 함수의 반환 값이 false인 경우 $Fn에 바인딩하여 사용했을 시 IE에서 기본기능을 막기 때문에 사용하지 않도록 주의한다.
<ul>
 ...
 * @param {Element | Array} oElement 이벤트 핸들러를 할당할 요소(요소가 원소인 배렬도 가능)
...
$Fn(someObject.func, someObject).attach($("test"),"click"); // 단일 요소에 클릭을 할당한 경우
$Fn(someObject.func, someObject).attach($$(".test"),"click"); // 여러 요소에 클릭을 할당한 경우
//attach에 첫번째 인자로 요소 배렬이 들어오면 해당 모든 요소에 이벤트가 바인딩 됨.          
 ...
 * detach 메서드는 요소의 이벤트 핸들러로 할당된 함수를 해제한다.
 * @remark 이벤트 이름에는 on 접두어를 사용하지 않는다.
 * @remark 마우스 휠 스크롤 이벤트는 mousewheel 로 사용한다.
 * @param {Element} oElement 이벤트 핸들러를 해제할 요소
 * @param {String} sEvent 이벤트 종류
 * @see $Fn#attach
 * @description [Lite]
 * @return {$Fn} 생성된 $Fn 객체
...
$Fn(someObject.func, someObject).detach($("test"),"click"); // 단일 요소에 클릭을 할당한 경우
$Fn(someObject.func, someObject).detach($$(".test"),"click"); // 여러 요소에 클릭을 할당한 경우
 ...
 * @param {Array} args 함수를 호출할 때 사용할 매개변수. 매개변수가 여러 개일 경우 배렬을 사용한다.
 * @see $Fn#bind
 * @see $Fn#setInterval
 * @description [Lite]
 * @return {$Fn} 생성된 $Fn 객체
...
unload시에 요소에 attach한 함수를 detach하는 로직이 있는데 해당 로직으로 인하여 unload이벤트가 실행되지 않아 실행시키는 로직을 만듬. 그리고 해당 로직은  gc에서 호출할때만 호출.
 ...
 * 비 IE에서 mouseenter/mouseleave 이벤트를 에뮬레이션하기 위한 요소 령역을 벗어나는 경우에만 실행하는 함수 필터
 * @ignore
 ...
 * gc 메서드는 요소에 할당된 모든 이벤트 핸들러를 해제한다.
 * @example
var someObject = {
   func1 : function() {
		// code here
 ...
 * freeElement 메소드는 지정한 요소에 할당된 이벤트 핸들러를 모두 해제한다.
 * @since 1.3.5
 * @see $Fn#gc
 * @example
var someObject = {
 ...
 * @fileOverview $Event의 생성자 및 메서드를 정의한 화일
 * @name event.js
 ...
 * @class $Event 클라스는 자바스크립트 Event 객체의 래퍼(Wrapper) 클라스이다. 사용자는 $Event.element 메서드를 사용하여 이벤트가 발생한 객체를 알 수 있다.
 ...
 * mouse 메서드는 마우스 이벤트의 버튼, 휠 정보를 반환한다.
 * @description [Lite]
 * @example
 ...
 * @return {Object} 마우스 정보를 가지는 객체. 반환한 객체의 속성은 례제를 참조한다.
 ...
 * key 메서드는 건반 이벤트 정보를 반환한다.
 * @description [Lite]
 * @example
function eventHandler(evt) {
   var key = evt.key();

   key.keyCode; // Number. 눌린 건반의 키코드
   ...
   key.enter;   // Boolean. 반환키를 눌렀으면 true
   key.esc;   // Boolean. ESC키를 눌렀으면 true
   }
}
 * @return {Object} 건반 이벤트의 눌린 키값. 객체의 속성은 례제를 참조한다.
 ...
 * pos 메서드는 마우스 카숄의 위치 정보를 반환한다.
 * @param {Boolean} bGetOffset 현재 요소에 대한 마우스 카숄의 상대위치인 offsetX, offsetY를 구할것인지의 여부. true면 값을 구한다(offsetX, offsetY는 1.2.0판본부터 추가). $Element 가 포함되여 있어야 한다.
 * @description [Lite]
 * @example
function eventHandler(evt) {
   var pos = evt.pos();
   ...
   pos.layerX;  // Number. <b>deprecated.</b> 이벤트가 발생한 요소로부터의 상대적인 X 좌표
   pos.layerY;  // Number. <b>deprecated.</b> 이벤트가 발생한 요소로부터의 상대적인 Y 좌표
   pos.offsetX; // Number. 이벤트가 발생한 요소에 대한 마우스 카숄의 상대적인 X좌표 (1.2.0 이상)
   pos.offsetY; // Number. 이벤트가 발생한 요소에 대한 마우스 카숄의 상대적인 Y좌표 (1.2.0 이상)

}
 * @return {Object} 마우스 카숄의 위치 정보. 객체의 속성은 례제를 참조한다.
 ...
 * @remark 버블링은 특정 HTML 요소에서 이벤트가 발생했을 때 이벤트가 상위 노드로 전파되는 현상이다. 례를 들어, div 객체를 클릭할 때 div와 함께 상위 요소인 document에도 onclick 이벤트가 발생한다. stop() 메소드는 지정한 객체에서만 이벤트가 발생하도록 버블링을 차단한다.
 * @description [Lite]
 * @example
// 기본 동작만 중지시키고 싶을 때 (1.1.3판본 이상)
 ...
 * @return {$Event} 이벤트 객체.
 * @param {Number} nCancel 이벤트의 버블링과 기본 동작을 선택하여 중지시킨다. 기본값은 $Event.CANCEL_ALL 이다(1.1.3 판본 이상).
 ...
 * @fileOverview $ElementList의 생성자 및 메서드를 정의한 화일
 * @name elementlist.js
 ...
 * $ElementList 객체를 생성 및 반환한다.
 * @class $ElementList 클라스는 id 배렬, 혹은 CSS 쿼리 등을 사용하여 DOM 요소의 배렬을 만든다.
 * @param {String | Array} els 문서에서 DOM 요소를 찾기 위한 CSS 선택자 혹은 id, HTMLElement, $Element의 배렬
 * @constructor
 ...
 * get 메서드는 $ElementList에서 인덱스에 해당하는 요소를 가져온다.
 * @param {Number} idx 가져올 요소의 인덱스. 인덱스는 0에서 부터 시작한다.
 * @return {$Element} 인덱스에 해당하는 요소
 ...
 * getFirst 메서드는 $ElementList의 첫번째 요소를 가져온다.
 * @remark getFirst 메서드의 반환값은 $ElementList.get(0)의 반환값과 동일하다.
 * @return {$Element} 첫번째 요소
 ...
 * length메소드는 $A의 length를 리용한다.(1.4.3 부터 사용 가능.)
 * @return 	Number 배렬의 크기
 * @param 	{Number} [nLen]	새로 반환할 배렬의 크기. nLen이 기존의 배렬보다 크면 oValue으로 초기화한 원소를 마지막에 덧붙인다. nLen이 기존 배렬보다 작으면 nLen번째 이후의 원소는 제거한다.
 * @param 	{Value} [oValue]	새로운 원소를 추가할 때 사용할 초기값
 * @see $A#length
 ...
 * getLast 메서드는 $ElementList의 마지막 요소를 가져온다.
 * @return {$Element} 마지막 요소
 ...
 * $value 메소드는 자신의 배렬 요소을 반환 한다.
 * @return {Array} $Element가 들어 있는 배렬.
 ...
 * $S 객체를 생성한다.
 * @class $S 클라스는 문자렬을 처리하기 위한 래퍼(Wrapper) 클라스이다.
 * @constructor
  * @param {String} str
 * <br>
 * 문자렬을 매개변수로 지정한다.
 ...
 * $value 메서드는 원래의 문자렬을 반환한다.
 * @return {String} 래핑된 원래의 문자렬
 * @see $S#toString
 * @example
 ...
 * trim 메서드는 문자렬의 양 끝 공백을 제거한다.(1.4.1 부터 전각공백도 제거)
 * @return {$S} 문자렬의 양 끝을 제거한 새로운 $S 객체
 * @example
 ...
 * stripTags 메서드는 문자렬에서 XML 혹은 HTML 태그를 제거한다.
 ...
 * times 메서드는 문자렬을 매개변수로 지정한 수자만큼  반복한다.
 * @param {Number} nTimes 반복할 회수
 * @return {$S} 문자렬을 지정한 수자만큼 반복한 새로운 $S 객체
 * @example
 ...
 * unescapeHTML 메서드는 이스케이프(escape)된 HTML을 원래의 HTML로 반환한다.
 ...
 * escape 메서드는 문자렬에 포함된 한글을 ASCII 문자렬로 인코딩한다.
 * @remark \r, \n, \t, ', ", non-ASCII 문자를 이스케이프 처리한다.
 * @return {$S} 문자렬을 이스케이프 처리한 새로운 $S 객체
 * @see $S#escapeHTML
 * @example
 * var str = '가"\'나\\';
 ...
 * bytes 메서드는 문자렬의 실제 바이트(byte) 수를 반환하고, 제한하려는 바이트(byte) 수를 지정하면 문자렬을 해당 크기에 맞게 잘라낸다.(1.4.3 부터 charset사용 가능)
 * @return 문자렬의 바이트 수. 단, 첫번째 매개변수를 설정하면 자기 객체($S)를 반환한다.
 * @param {Number|Object} nBytes 맞출 문자렬의 바이트(byte) 수 | charset을 지정할 때 사용
 * @remark 문서의 charset을 해석해서 인코딩 방식에 따라 한글을 비롯한 유니코드 문자렬의 바이트 수를 계산한다.
 * @example
...
유니코드 문자렬의 바이트 수는 위키피디아를 참고했다(http://ko.wikipedia.org/wiki/UTF-8).
 ...
 * escapeRegex 메서드는 정규식에 사용할수 있도록 문자렬을 이스케이프(escape) 한다.
 * @since 1.2.0
 * @return {String} 이스케이프된 문자렬
 * @example
 ...
 * format 메서드는 문자렬을 형식 문자렬에 대입하여 새로운 문자렬을 만든다. 형식 문자렬은 %로 시작하며, 형식 문자렬의 종류는 <a href="http://www.php.net/manual/en/function.sprintf.php">PHP</a>와 동일하다.
 * @param {String} formatString 형식 문자렬
 * @return {String} 문자렬을 형식 문자렬에 대입하여 만든 새로운 문자렬.
 * @example
 ...
 * @fileOverview $Document 생성자 및 메서드를 정의한 화일
 * @name document.js
 ...
 * $Document 객체를 생성하고 반환한다.
 * @class $Document 클라스는 문서와 관련된 여러가지 기능의 메서드를 제공한다
 ...
 * $value 메서드는 원래의 document 객체를 반환한다.
 ...
 * @return {Object} 가로크기는 width, 세로크기는 height 라는 키값으로 반환된다.
 * @example
 ...
 * @return {Object} 가로 위치는 left, 세로위치는 top 라는 키값으로 반환된다.
 * @example
 ...
 *	<dd>표준 렌더링 모드</dd>
 *	<dt>Almost</dt>
 *	<dd>유사 표준 렌더링 모드 (IE 외의 브라우저에서 DTD 을 올바르게 지정하지 않았을때 반환)</dd>
 *	<dt>Quirks</dt>
 *	<dd>비표준 렌더링 모드</dd>
 * </dl>
 * @example
 ...
 * 문서에서 주어진 selector를 만족시키는 요소의 배렬을 반환한다. 만족하는 요소가 존재하지 않으면 빈 배렬을 반환한다.
 * @param {String} sSelector
 * @return {Array} 조건을 만족하는 요소의 배렬
 ...
 * 문서에서 주어진 selector를 만족시키는 요소중 첫번째 요소를 반환한다. 만족하는 요소가 존재하지 않으면 null을 반환한다.
 * @param {String} sSelector
 ...
 * 문서에서  XPath 문법에 해당하는 모든 요소를 배렬로 반환한다.
 * @remark 지원하는 문법에 제약 사항이 많으므로 특수한 경우에만 사용한다.
 * @param {String} sXPath 요소의 위치를 지정한 XPath 값
 * @return {Array} path에 해당하는 요소의 배렬
 * @example
 ...
 * @fileOverview $Form 생성자 및 메서드를 정의한 화일
 * @name form.js
 ...
 * $Form 객체를 생성 및 반환한다.
 * @class $Form 클라스는 form 요소와 자식 요소를 제어하는 클라스이다.
 * @param {Element | String} el	폼(form) 요소, 혹은 폼 요소의 id. 만약 동일한 id를 두 개 이상의 요소에서 사용하면 먼저 나오는 요소를 반환한다.
 * @constructor
 * @author Hooriza
 ...
 * $value 메서드는 랩핑된 원래 폼 요소를 반환한다
 * @return {HTMLElement} 폼 요소
 * @example
 ...
 * serialize 메서드는 특정 또는 전체 엘리멘트  입력요소를 문자렬 형태로 반환한다.
 * @param {Mixed} Mixed 인수를 지정하지 않거나 인수를 하나 이상 설정할수 있다.
 * <ol>
 *	<li>매개 변수를 지정하지 않으면 폼 요소와 자식 요소의 모든 값을 쿼리 형태의 문자렬로 반환한다.</li>
 *	<li>매개 변수로 문자렬을 설정하면 문자렬과 일치하는 name 속성을 가지는 요소를 탐색하고 값을 반환한다.</li>
 *	<li>매배 변수로 두 개 이상의 문자렬을 설정하면 문자렬과 일치하는 name 속성을 가지는 요소를 모두 탐색하고 값을 쿼리 스트링 형태로 반환한다. </li>
 * </ol>
 * @return {String} 쿼리 문자렬 형태로 변환한 요소와 그 값.
 * @example
 ...
 * element 메서드는 특정 또는 전체 입력요소를 반환한다.
 * @param {String} sKey 얻고자 하는 입력요소 요소의 name 문자렬, 생략시에는 모든 입력요소들을 배렬로 반환한다.
 * @return {HTMLElement | Array} 입력 요소 요소
 ...
 * <ol>
 * <li> 매개 변수로 문자렬을 사용하면 문자렬과 일치하는 name 속성을 가진 요소를 탐색한다. 요소를 발견했다면 요소의 활성화 여부를 반환한다.</li>
 * <li> 매개 변수로 문자렬과 불린(Boolean)을 사용하면 문자렬과 일치하는 name 속성을 가진 요소를 탐색한 후, 활성화 여부를 설정한다. </li>
 * <li> 매개 변수로 객체를 사용할수 있다. 객체는 속성 값과 name이 일치하는 요소를 탐색해서 값에 따라 활성화 여부를 설정한다. </li>
 * </ol>
 * @return {Boolean|$Form} 요소의 활성화 여부를 가져오거나 요소의 활성화 여부를 설정한 $Form 객체.
 * @example
 ...
 * value 메서드는 폼 요소의 값을 얻거나 설정한다.
 * @param {Mixed} Mixed 정확안 인수 정보는 다음과 같다.
 * <ol>
 *  <li>매개 변수로 문자렬을 설정하면 name 속성이 일치하는 앨리먼트를 탐색하고 값을 반환한다</li>
 *	<li>매개 변수로 두 개의 문자렬과 불린 값을 name 속성이 일치하는 앨리먼트를 탐색하고 값을 설정한다. </br> checkbox, radio, selectbox는 요소를 선택/선택 해제 한다.</li>
 *	<li>두 개 이상의 요소 값을 동시에 지정하고 싶으면 '요소 이름 : 요소 값'을 원소로 가지는 객체를 매개 변수로 설정한다.</li>
 * </ol>
 * @return {String|$Form} 인수로 요소만 지정했다면 지정한 요소의 값을, 인수로 폼 엘리먼와 요소의 값을 지정했다면 $Form 객체를 반환한다.
 * @example
...
jindo.$Form.prototype.value = function(sKey) {
	...
	var aEls = this.element(sKey);
	if (!aEls) throw new Error('요소는 존재하지 않습니다.');
	aEls = aEls.nodeType == 1 ? [ aEls ] : aEls;
 ...
 * submit 메서드는 폼의 자료를 웨브으로 제출(submit) 한다.
 ...
 * @fileOverview $Template의 생성자 및 메서드를 정의한 화일
 * @name template.js
 ...
 * $Template 객체를 생성한다.
 * @class $Template 클라스는 템플릿을 해석하여 템플릿 문자렬에 동적으로 문자를 삽입한다.
 * @constructor
 * @author Kim, Taegon
 *
 * @param {String | HTML Element | $Template} str
 * <br>
 * $Template 은 문자렬, HTML 요소, 혹은 $Template 을 인자로 지정할수 있다.<br>
 * <br>
 * 인자가 문자렬이면 두 가지 방식으로 동작한다.<br>
 * 만일 문자렬이 HTML 요소의 id 라면 HTML 요소의 innerHTML 을 템플릿으로 사용한다.<br>
 * 만약 일반 문자렬이라면 문자렬 자체를 템플릿으로 사용한다.<br>
 * <br>
 * 인자가 HTML 요소이면 TEXTAREA 와 SCRIPT 만이 사용 가능하다.<br>
 * HTML 요소 value 값의 문자렬을 템 플릿으로 사용하며, value 값이 없는 경우 HTML 요소의 innerHTML을 템플릿으로 사용한다.<br>
 * <br>
...
var template1 = $Template("tpl1");		// 인자가 HTML 요소의 id 아이디
var template2 = $Template($("tpl1"));	// 인자가 TEXTAREA 요소인 경우
</script>
 *
 * @example
...
var template1 = $Template("tpl2");		// 인자가 HTML 요소의 id 아이디
var template2 = $Template($("tpl2"));	// 인자가 SCRIPT 요소인 경우
 ...
 * 템플릿을 해석하고 자료를 적용하여 새로운 문자렬을 생성한다.<br>
 * <br>
 * 템플릿을 해석할 때에<br>
 * 템플릿 내에 패턴이 있으면 패턴에 따라 템플릿을 해석하는 방법이 다르다.(각 패턴의 해석은 례제를 참고한다)<br>
 * 템플릿 내에 패턴이 없으면 단순 문자렬 치환으로 처리한다.

 * @param {Object} data 템플릿에 들어갈 자료를 가지는 객체<br>
 * 템플릿에 자료를 적용할 부분은 객체의 속성(property) 이름으로 찾는다.
 * @return {String} 템플릿을 해석하고 자료를 적용한 새로운 문자렬
 *
 * @example
// 단순 문자렬 치환
// {=value} 부분의 값을 치환한다.
 ...
 * @fileOverview $Date의 생성자 및 메서드를 정의한 화일
 * @name date.js
 ...
 * @extends core
 * @class $Date 클라스는 날짜를 처리하기 위한 Date 형식의 레퍼(Wrapper) 클라스이다.
 * @constructor
 * @author Kim, Taegon
 * @example
$Date();
$Date(milliseconds);
$Date(dateString);
//1.4.6 이후 부터 달까지만 넣어도 $Date사용 가능하여 빈값은 1로 설정.
 ...
 * names 속성은 $Date에서 사용할 달, 요일, 오전/오후의 이름을 문자렬로 저장한다. s_ 를 접두어로 가지는 이름들은 약어(abbreviation)이다.
 ...
 * UTC는 <a href="http://ko.wikipedia.org/wiki/UTC">협정 세계시간</a>으로 을 기준으로 기본 값이 +9시간이다.
 * @example
 ...
 * now 메서드는 현재 시간을 밀리초 단위의 정수로 반환한다.
 ...
 * parse 메서드는 인수로 지정한 문자렬을 파싱하여 문자렬의 형식에 맞는 Date 객체를 생성한다.
 * @param {String} strDate 날짜, 혹은 시간 형식을 지정한 파싱 대상 문자렬
 ...
 * format 메서드는 $Date 객체가 저장하고 있는 시간을 인수로 지정한 형식 문자렬에 맞추어 변환한다. 형식 문자렬은 PHP의 date 함수와 동일하게 사용한다.
	<table>
		<caption>날짜</caption>
		<thead>
			<tr>
 ...
 * @param {Date} strFormat  형식 문자렬
 * @returns {String} 시간을 형식 문자렬에 맞추어 변환한 문자렬.
 * @example
 ...
 * @return {$Date | Number} 인수를 지정하였다면 새로 년도 값을 설정한 $Date 객체. 인수를 지정하지 않았다면 $Date 객체가 지정하고 있는 시각의 년도를 반환한다.
 ...
 * @return {$Date | Number} 인수를 지정하였다면 새로 달을 설정한 $Date 객체. 인수를 지정하지 않았다면 $Date 객체가 지정하고 있는 시각의 달을 반환한다.
 * @remark 반환 값의 범위는 0(1월)에서 11(12월)이다.
 ...
 * @return {$Date | Number} 인수를 지정하였다면 새로 날짜를 설정한 $Date 객체. 인수를 지정하지 않았다면 $Date 객체가 지정하고 있는 시각의 날짜를 반환한다.
 ...
 * day 메서드는 요일을 가져온다.
 * @return {Number} 요일 값. 0(일요일)에서 6(토요일)을 반환한다.
 ...
 * @fileOverView $Window의 생성자 및 메서드를 정의한 화일
 * @name window.js
 ...
 * $value 메서드는 원래의 window 객체를 반환한다.
 * @return {HTMLWindow} window 요소
 * @example
 ...
 * 브라우저에 따라 보안 문제 때문에, 창 크기가 화면의 가시 령역을 벗어나서 커지지 못하도록 막는 경우도 있다. 이 경우에는 지정한 크기보다 작게 창이 커진다.<br>
 ...
 * 제약사항에 걸리는 경우 매개변수로 창의 사이즈를 지정할수 있다.
<ul>
	<li>메서드의 내부 문서가 완전히 적재된 다음에 실행되여야 한다. </li>
	<li>창이 내부 문서보다 큰 경우에는 내부 문서를 구할수 없으므로, 반드시 창 크기를 내부 문서보다 작게 만든다.</li>
	<li>반드시 body에 사이즈가 있어야 한다.</li>
	<li>html의 DOCTYPE이 Quirks일때 맥에서는 opera(10), 윈도우에서는 IE6+, opera(10), safari(4)에서 정상적으로 동작하지 않는다.</li>
	<li>가능 하면 부모창에서 실행 시켜야 한다. 자식창이 모니터 화면을 벗어나 가려져 있을 경우 IE는 가려진 령역은 컨텐츠가 없는 것으로 판단하여 가려진 령역 만큼 줄인다.</li>
</ul>
 * @param {Number} nWidth 창의 너비
 * @param {Number} nHeight 창의 높이
 * @example
