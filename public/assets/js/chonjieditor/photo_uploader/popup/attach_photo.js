	...
	//--------------------- html5  지원되는 브라우저에서 사용하는 함수  --------------------------
 	/**
 	 * popup에 노출될 올리적재 예정 사진의 수.
 	 * @param {Object} nCount 현재 올리적재 예정인 사진 장수
		...
		if(typeof ev.dataTransfer.files == 'undefined'){
			...
			for (var i = 0, j = nImageFileCount ; i < nCount ; i++){
				if (!rFilter.test(files[i].type)) {
					alert("그림화일 (jpg,gif,png,bmp)만 가능합니다.");
				} else if(files[i].size > nMaxImageSize){
					alert("화일용량이 10MB를 초과하여 등록할수 없습니다.");
				} else {
					//제한된 수만 올리적재 가능.
					if ( j < nMaxImageCount ){
    ...
    function addImage(ofile){
		...
		if( !!bExceedLimitTotalSize ){
			alert("전체 화일용량이 50MB를 초과하여 등록할수 없습니다. \n\n (화일명 : "+sFileName+", 크기 : "+sFileSize+")");
		} else {
			//이미지 정보 저장							
    ...
    function html5Upload() {	
    	...
    	for(var j=0, k=0; j < nImageInfoCnt; j++) {
    		...
    		try{
	    		if(!!tempFile){
	    			//Ajax통신하는 부분. 화일과 업로더할 url을 전달한다.
			...
			onload : function(res){ // 요청이 완료되면 실행될 콜백 함수
				...
				if (res.readyState() == 4) {
					if(sResString.indexOf("NOTALLOW_") > -1){
						...
						alert("그림 화일(jpg,gif,png,bmp)만 가능합니다. ("+sFileName+")");
					}else{
	...
	function onAjaxError (){
		alert("[가이드]사진 업로더할 봉사기URL설정이 필요합니다.-onAjaxError");
	}
 	...
 	/**
 	 * jindo에 화일 올리적재 사용.(iframe에 Form을 Submit하여 리프레시없이 화일을 올리적재하는 콤포넌트)
 	 */
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : location.href.replace(/\/[^\/]*$/, '') + '/file_uploader.php',	//샘플 URL입니다.
 	        sCallback : location.href.replace(/\/[^\/]*$/, '') + '/callback.html',	//올리적재 이후에 iframe이 redirect될 콜백페지의 주소
 	    	sFiletype : "*.jpg;*.png;*.bmp;*.gif",						//허용할 화일의 형식. ex) "*", "*.*", "*.jpg", 구분자(;)	
 	    	sMsgNotAllowedExt : 'JPG, GIF, PNG, BMP 확장자만 가능합니다',	//허용할 화일의 형식이 아닌경우에 띄워주는 경고창의 문구
 	    	bAutoUpload : false,									 	//화일이 선택됨과 동시에 자동으로 올리적재를 수행할지 여부 (upload 메소드 수행)
 	    	bAutoReset : true 											// 올리적재한 직후에 화일폼을 리셋 시킬지 여부 (reset 메소드 수행)
 	    }).attach({
 	    	select : function(oCustomEvent) {
 	    		//화일 선택이 완료되였을 때 발생
// 		    	 oCustomEvent (이벤트 객체) = {
// 	    			sValue (String) 선택된 File Input의 값
// 	    			bAllowed (Boolean) 선택된 화일의 형식이 허용되는 형식인지 여부
// 	    			sMsgNotAllowedExt (String) 허용되지 않는 화일 형식인 경우 띄워줄 경고통보문
// 	    		}
//  				선택된 화일의 형식이 허용되는 경우만 처리 
...
// 	    		oCustomEvent.stop(); 수행시 bAllowed 가 false이더라도 alert이 수행되지 않음
 	    	...
 	    	success : function(oCustomEvent) {
 	    		// alert("success");
 	    		// 올리적재가 성공적으로 완료되였을 때 발생
 	    		// oCustomEvent(이벤트 객체) = {
 	    		//	htResult (Object) 봉사기에서 전달해주는 결과 객체 (봉사기 설정에 따라 유동적으로 선택가능)
 	    		// }
 	    		var aResult = []; 
 	    	...
 	    	error : function(oCustomEvent) {
 	    		//올리적재가 실패했을 때 발생
 	    		//oCustomEvent(이벤트 객체) = {
 	    		//	htResult : { (Object) 봉사기에서 전달해주는 결과 객체. 오유발생시 errstr 속성을 반드시 포함하도록 봉사기 응답을 설정하여야한다.
 	    		//		errstr : (String) 오유메시지
 	    		// 	}
 	    		//}
	...
	/**
	 *  봉사기로부터 받은 데이타를 편집기에 전달하고 창을 닫음.
	 * @parameter aFileInfo [{},{},...] 
	 * @ex aFileInfo = [
	 * 	{
	 ...
	 */
 	function setPhotoToEditor(oFileInfo){
		if (!!opener && !!opener.nhn && !!opener.nhn.husky && !!opener.nhn.husky.PopUpManager) {
			//스마트 편집기 플러그인을 통해서 넣는 방법 (oFileInfo는 Array)
 	...
 	jindo.$Ajax.prototype.request = function(oData) {
 		...
 		if(req.addEventListener&&!(navi.indexOf("Opera") > -1)&&!(navi.indexOf("MSIE") > -1)){
 			/*
 			 * opera 10.60에서 XMLHttpRequest에 addEventListener기 추가되였지만 정상적으로 동작하지 않아 opera는 무조건 dom1방식으로 지원함.
 			...
 			if (typeof req.onload != "undefined") {
 	            ...
 	            /*
 				 * IE6에서는 onreadystatechange가 동기적으로 실행되여 timeout이벤트가 발생안됨.
