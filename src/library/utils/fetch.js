...
// import { WorkflowConstants } from '../redux/constants/workflowConstants';

// normal axios call without saga [vdg - 2020/05/19]
export const executeQuery = ({ method, url, data, params, success, fail, reExecute, timeOut }) => {
  ...
  .catch((err) => {
    ...
    if ( reExecute ) {
      setTimeout(() => {
        executeQuery({ method, url, data, params, success, fail, reExecute, timeOut });
      }, timeOut || 1000 );
    } else {
      fail(err.response, err);
    }
  })
}
